...
def demonstrate_working_crawling():
    ...
    login_response = requests.post(
        f"{API_BASE}/api/auth/login", json={"username": "admin", "password": "admin123"}
    )
    ...
    for i in range(30):  # Wait up to 60 seconds
            ...
            if results_response.status_code == 200:
                ...
                if isinstance(results, dict):
                    ...
                    if "discovered_urls" in results:
                        print(
                            f"   Total URLs discovered: {len(results['discovered_urls'])}"
                        )
        ...
        print(
            f"   Status: {current_job['status'] if current_job else 'unknown'} (waiting...)"
        )
