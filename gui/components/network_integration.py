...
class NetworkIntegrationManager(QObject):
    ...
    async def test_current_configuration(self) -> Dict[str, Any]:
        ...
        if any(service['working'] for service in results.values() if isinstance(service, dict)):
            results['overall_status'] = 'connected'
        elif any(service['available'] for service in results.values() if isinstance(service, dict)):
