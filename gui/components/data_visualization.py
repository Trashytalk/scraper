...
class NetworkLayoutEngine:
    ...
    def add_link(self, link: SiteLink):
        ...
        if (
            self.graph
            and link.source_url in self.nodes
            and link.target_url in self.nodes
        ):
            self.graph.add_edge(
                link.source_url,
                link.target_url,
        ...
        try:
            if algorithm == "spring":
                pos = nx.spring_layout(
                    self.graph, dim=3 if self.mode == "3d" else 2, k=1.0, iterations=50
                )
                ...
                if self.mode == "3d":
                    ...
                    for node_id in list(
                        pos.keys()
                    ):  # Convert to list to avoid dict size change during iteration
                        depth = (
                            self.nodes[node_id].crawl_depth
...
class VisualizationRenderer(QWidget):
    ...
    def _draw_arrow_head(
        self, painter: QPainter, sx: float, sy: float, tx: float, ty: float
    ):
    ...
    def _get_node_color(self, node: SiteNode) -> QColor:
        ...
        if self.color_scheme == "status":
            ...
            max_depth = max(
                [n.crawl_depth for n in self.layout_engine.nodes.values()], default=1
            )
...
class SiteVisualizationWidget(QWidget):
    ...
    def setup_ui(self):
        ...
        self.layout_combo.addItems(
            ["spring", "circular", "hierarchical", "force_directed"]
        )
    ...
    def update_statistics(self):
        ...
        avg_response_time = (
            sum(node.response_time for node in nodes.values()) / total_nodes
            if total_nodes > 0
            else 0
        )
        success_rate = (
            sum(1 for node in nodes.values() if node.status_code == 200)
            / total_nodes
            * 100
            if total_nodes > 0
            else 0
...
Node Type: {node.node_type}
    ...
    def export_visualization(self):
        ...
        try:
            ...
            filename = (
                f"site_visualization_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
            )
    ...
    def get_visualization_data(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "statistics": {
                ...
                "unique_domains": len(
                    set(node.domain for node in self.layout_engine.nodes.values())
                ),
            },
        }
