...
class SpiderFootIntegration(OSINTModule):
    ...
    async def _start_scan(self, target: OSINTTarget) -> Optional[str]:
        ...
        try:
            ...
            async with self.session.post(
                f"{self.base_url}/startscan", data=scan_data
            ) as response:
        ...
        while (datetime.now() - start_time).seconds < max_wait:
            try:
                ...
                async with self.session.get(
                    f"{self.base_url}/scansummary", params={"id": scan_id}
                ) as response:
                    if response.status == 200:
    ...
    def _get_modules_for_target(self, target_type: str) -> List[str]:
        ...
        module_map = {
            ...
            "username": ["sfp_whatsmyname", "sfp_social", "sfp_github", "sfp_twitter"],
        }
...
class SocialMediaAnalyzer(OSINTModule):
    ...
    async def _enrich_profile(
        self, session: aiohttp.ClientSession, profile: SocialMediaProfile
    ):
    ...
    def _extract_github_bio(self, html: str) -> str:
        """Extract GitHub bio"""
        match = re.search(
            r'<div[^>]*class="[^"]*user-profile-bio[^"]*"[^>]*>([^<]+)</div>', html
        )
        return match.group(1).strip() if match else ""
...
class OSINTIntegrationWidget(QWidget):
    ...
    def create_target_tab(self) -> QWidget:
        ...
        self.target_input.setPlaceholderText(
            "domain.com, email@example.com, username, IP address..."
        )
        ...
        self.targets_table.setHorizontalHeaderLabels(
            ["Target", "Type", "Priority", "Added", "Status"]
        )
    ...
    def create_investigation_tab(self) -> QWidget:
        ...
        for module in self.modules:
            checkbox = QCheckBox(
                f"{module.name} - {module.__class__.__doc__ or 'No description'}"
            )
    ...
    def create_social_tab(self) -> QWidget:
        ...
        self.social_tree.setHeaderLabels(
            ["Platform", "Username", "Display Name", "Verified"]
        )
    ...
    def create_reports_tab(self) -> QWidget:
        ...
        self.report_type.addItems(
            ["Summary", "Detailed", "Social Media", "Risk Assessment"]
        )
    ...
    def connect_signals(self):
        ...
        for name, checkbox in self.module_checkboxes.items():
            checkbox.stateChanged.connect(
                lambda state, n=name: self.toggle_module(n, state)
            )
    ...
    def update_targets_table(self):
        ...
        for i, target in enumerate(self.targets):
            ...
            self.targets_table.setItem(
                i, 3, QTableWidgetItem(target.created_at.strftime("%Y-%m-%d %H:%M"))
            )
    ...
    def update_results_table(self):
        ...
        for i, result in enumerate(self.results):
            ...
            self.results_table.setItem(
                i, 4, QTableWidgetItem(f"{result.confidence:.2f}")
            )
            self.results_table.setItem(
                i, 5, QTableWidgetItem(result.discovered_at.strftime("%Y-%m-%d %H:%M"))
            )
            self.results_table.setItem(
                i, 6, QTableWidgetItem("Yes" if result.verified else "No")
            )
        ...
        report_data = {
            ...
            "summary": {
                ...
                "high_risk_results": len(
                    [r for r in self.results if r.risk_level in ["high", "critical"]]
                ),
