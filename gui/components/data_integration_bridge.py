...
try:
    from .advanced_entity_graph import (
        AdvancedEntityGraphSystem, NodeType, RelationshipType,
        EntityNode, EntityRelationship, advanced_entity_graph
    )
    ...
    from business_intel_scraper.backend.db.quality_models import (
        EntityRecord, ProvenanceRecord, DataSource, QualityAssessment
    )
...
class DataIntegrationBridge:
    ...
    async def sync_entities_to_graph(self, entity_types: List[str] = None) -> Dict[str, int]:
        ...
        try:
            ...
            with get_db_session() as session:
                for entity_type in entity_types:
                    ...
                    synced_count = await self._sync_entity_type(session, entity_type, mapping)
                    sync_results[entity_type] = synced_count
                    
                    logger.info(f"Synced {synced_count} {entity_type} entities to graph")
    ...
    async def _sync_entity_type(self, session: Session, entity_type: str, 
                               mapping: IntegrationMapping) -> int:
        ...
        try:
            ...
            for entity in entities:
                ...
                confidence = 1.0
                if mapping.confidence_field and hasattr(entity, mapping.confidence_field):
                    confidence = getattr(entity, mapping.confidence_field, 1.0)
                
                # Get data sources
                data_sources = []
                if mapping.data_source_field and hasattr(entity, mapping.data_source_field):
    ...
    async def _sync_relationships(self, session: Session) -> int:
        ...
        try:
            ...
            from business_intel_scraper.backend.db.models import Company, Location
    ...
    async def sync_quality_assessments_to_graph(self) -> int:
        ...
        try:
            from business_intel_scraper.backend.db.utils import get_db_session
            from business_intel_scraper.backend.db.quality_models import QualityAssessment
            ...
            with get_db_session() as session:
                ...
                logger.info(f"Updated {updated_count} graph nodes with quality assessments")
    ...
    async def sync_provenance_to_graph(self) -> int:
        ...
        try:
            from business_intel_scraper.backend.db.utils import get_db_session
            from business_intel_scraper.backend.db.quality_models import ProvenanceRecord
                ...
                for node_type, nodes in nodes_by_type.items():
                    for i, node1 in enumerate(nodes):
                        for j, node2 in enumerate(nodes[i+1:], i+1):
                            similarity_score = self._calculate_entity_similarity(node1, node2)
                            ...
                            if similarity_score > 0.8:  # High similarity threshold
                                duplicates.append({
                                    ...
                                    'matching_properties': self._get_matching_properties(node1, node2),
                                    'recommended_action': 'merge' if similarity_score > 0.9 else 'review'
                                })
    ...
    def _calculate_entity_similarity(self, node1: EntityNode, node2: EntityNode) -> float:
    ...
    def _get_matching_properties(self, node1: EntityNode, node2: EntityNode) -> List[str]:
    ...
    async def get_integration_status(self) -> Dict[str, Any]:
        ...
        status = {
            'graph_system_connected': bool(self.graph_system.connector and self.graph_system.connector.connected),
        ...
        if hasattr(self.graph_system.connector, 'nodes_data'):
            nodes_data = self.graph_system.connector.nodes_data
            relationships_data = getattr(self.graph_system.connector, 'relationships_data', {})
