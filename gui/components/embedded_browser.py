...
from PyQt6.QtCore import QUrl, pyqtSignal, QThread, Qt
import json
import time
...
class BrowserRecorder:
    ...
    def record_action(self, action_type: str, **kwargs: Any) -> None:
        ...
        action = BrowserAction(
            timestamp=time.time() - self.start_time, action_type=action_type, **kwargs
        )
...
class BrowserPlayer(QThread):
    ...
    def __init__(
        self, browser_view: QWebEngineView, actions: List[BrowserAction]
    ) -> None:
        super().__init__()
    ...
    def add_new_tab(self, url: str = "about:blank") -> EmbeddedBrowser:
        ...
        browser.web_view.titleChanged.connect(
            lambda title: self.setTabText(
                index, title[:20] + "..." if len(title) > 20 else title
            )
        )
...
class BrowserDockWidget(QDockWidget):
    ...
    def __init__(
        self, title: str = "Browser", parent: Optional[QWidget] = None
    ) -> None:
