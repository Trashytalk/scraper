        ...
        try:
            for circuit in self.controller.get_circuits():
                circuit_info = {
                    ...
                    "path": [
                        f"{relay.fingerprint} ({relay.nickname})"
...
class TORFailureHandler:
    ...
    def handle_circuit_failure(
        self, circuit_id: str, config: CircuitConfig
    ) -> Optional[str]:
        ...
        try:
            ...
            if circuit and circuit.path:
                ...
                self.failure_counts[exit_node] = (
                    self.failure_counts.get(exit_node, 0) + 1
                )
                ...
                if self.failure_counts[exit_node] >= self.max_failures:
                    ...
                    logger.info(
                        f"Blacklisted exit node {exit_node} after {self.max_failures} failures"
                    )
    ...
    def _modify_config_for_retry(self, config: CircuitConfig) -> CircuitConfig:
        ...
        if new_config.exit_nodes:
            new_config.exit_nodes = [
                node
                for node in new_config.exit_nodes
...
class TORWidget(QWidget):
    ...
    def create_circuit(self):
        ...
        config = CircuitConfig(path_length=self.path_length_spin.value())
        ...
        if result["success"]:
            if result["is_tor"]:
                self.status_label.setText(
                    f"TOR working - IP: {result['ip']} ({result.get('country', 'Unknown')})"
                )
    ...
    def refresh_circuits(self):
        ...
        for row, circuit in enumerate(circuits):
            ...
            self.circuit_table.setItem(
                row, 2, QTableWidgetItem(" → ".join(circuit["path"]))
            )
            ...
            close_btn.clicked.connect(
                lambda checked, cid=circuit["id"]: self.close_circuit(cid)
            )
