...
class AdvancedParser:
    ...
    def parse_file(
        self, file_path: str, format_type: FileFormat, rules: List[ParsingRule]
    ) -> Dict[str, Any]:
    ...
    def _parse_text_file(
        self, file_path: str, rules: List[ParsingRule]
    ) -> Dict[str, Any]:
    ...
    def _apply_parsing_rules(
        self, content: str, rules: List[ParsingRule]
    ) -> Dict[str, Any]:
        ...
        try:
            if "ner" in self.transformers_models:
                ...
                return [
                    ...
                    if entity["score"] >= rule.confidence_threshold
                ]
...
class BatchProcessor(QThread):
    ...
    def __init__(
        self, files: List[str], rules: List[ParsingRule], parser: AdvancedParser
    ) -> None:
        super().__init__()
        self.files = files
        self.rules = rules
    ...
    def run(self) -> None:
        ...
        for i, file_path in enumerate(self.files):
            ...
            self.progress_updated.emit(
                int((i / total_files) * 100), f"Processing {Path(file_path).name}"
            )
...
class ParsingWidget(QWidget):
    ...
    def create_models_tab(self) -> QWidget:
        ...
        self.spacy_model_combo.addItems(
    ...
    def refresh_rules_table(self) -> None:
        ...
        for row, rule in enumerate(self.parsing_rules):
            ...
            self.rules_table.setItem(
                row, 1, QTableWidgetItem(rule.pattern[:50] + "...")
            )
            self.rules_table.setItem(row, 2, QTableWidgetItem(rule.extraction_type))
            self.rules_table.setItem(
                row, 3, QTableWidgetItem(f"{rule.confidence_threshold:.2f}")
            )

            enabled_cb = QCheckBox()
            enabled_cb.setChecked(rule.enabled)
            enabled_cb.stateChanged.connect(
                lambda state, r=row: self.toggle_rule_enabled(r, state)
            )
    ...
    def start_batch_processing(self) -> None:
        ...
        self.batch_processor.processing_completed.connect(
    ...
    def add_batch_result(self, file_path: str, results: Dict[str, Any]) -> None:
        ...
        if "error" in results:
            ...
            result_summary += (
                f"Extracted data points: {len(results.get('extracted_data', {}))}\n"
            )
    ...
    def batch_processing_completed(self, total: int, successful: int) -> None:
        ...
        self.batch_status.setText(
            f"Completed: {successful}/{total} files processed successfully"
        )
