        ...
        try:
            if self.operation == "full_analytics":
                ...
                analytics = loop.run_until_complete(
                    self.graph_system.perform_analytics()
                )
...
class RelationshipAddDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        self.relationship_type_combo.addItems(
            [rel_type.value for rel_type in RelationshipType]
        )
    ...
    def get_relationship_data(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "relationship_type": RelationshipType(
                self.relationship_type_combo.currentText()
            ),
            "properties": properties,
            "confidence": self.confidence_slider.value() / 100.0,
...
class AdvancedEntityGraphWidget(QWidget):
    ...
    def create_left_panel(self) -> QWidget:
        ...
        self.run_analytics_btn.setStyleSheet(
            "padding: 8px; font-weight: bold; background-color: #3498db; color: white;"
        )
        ...
        self.color_combo.addItems(
            ["node_type", "centrality", "confidence", "community"]
        )
    ...
    def create_right_panel(self) -> QWidget:
        ...
        try:
            ...
            self.viz_placeholder.setHtml(
                "<h3>Visualization</h3><p>Graph visualization will appear here.</p>"
            )
    ...
    def setup_connections(self):
        ...
        # Quick analytics buttons
        self.centrality_btn.clicked.connect(
            lambda: self.run_quick_analytics("centrality")
        )
        self.communities_btn.clicked.connect(
            lambda: self.run_quick_analytics("communities")
        )
        self.anomalies_btn.clicked.connect(
            lambda: self.run_quick_analytics("anomalies")
        )
        ...
        try:
            ...
            if success:
                self.status_label.setText(
                    f"Added relationship: {relationship_data['relationship_type'].value}"
                )
                self.update_relationship_list()
                self.update_visualization()
            else:
...
ORDER BY length(path)
        ...
        if template_name in templates:
            ...
            if template_name in [
                ...
                self.params_edit.setText(
                    '{"entity_id": "COMPANY_001", "max_hops": 3, "limit": 100}'
                )
    ...
    def update_visualization(self):
        ...
        try:
            ...
            html_content = self.graph_system.create_visualization(
                layout, color_by, show_labels
            )
        ...
        try:
            ...
            for row, (entity_id, node) in enumerate(nodes_data.items()):
                ...
                self.entities_table.setItem(
                    row, 1, QTableWidgetItem(node.node_type.value)
                )
                self.entities_table.setItem(row, 2, QTableWidgetItem(node.display_name))
                self.entities_table.setItem(
                    row, 3, QTableWidgetItem(f"{node.confidence:.2f}")
                )
        ...
        try:
            ...
            for row, (rel_key, relationship) in enumerate(relationships_data.items()):
                self.relationships_table.setItem(
                    row, 0, QTableWidgetItem(relationship.source_id)
                )
                self.relationships_table.setItem(
                    row, 1, QTableWidgetItem(relationship.target_id)
                )
                self.relationships_table.setItem(
                    row, 2, QTableWidgetItem(relationship.relationship_type.value)
                )
                self.relationships_table.setItem(
                    row, 3, QTableWidgetItem(f"{relationship.confidence:.2f}")
                )
    ...
    def format_analytics_results(self, analytics: GraphAnalytics) -> str:
        ...
        if analytics.centrality_measures:
            ...
            for node in sorted(nodes):
                ...
                betweenness = analytics.centrality_measures.get("betweenness", {}).get(
                    node, 0
                )
                pagerank = analytics.centrality_measures.get("pagerank", {}).get(
                    node, 0
                )
