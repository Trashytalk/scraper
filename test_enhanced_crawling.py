...
def test_enhanced_crawling_features():
    ...
    async def run_domain_test():
        result = await engine.intelligent_crawl(
            "https://httpbin.org/", "basic", config_domain  # Smaller domain for testing
        )
    ...
    async def run_image_test():
        result = await engine.intelligent_crawl(
            "https://en.wikipedia.org/wiki/Photography", "basic", config_images
        )
    ...
    try:
        ...
        if cache_table_exists:
            ...
            cursor.execute(
                "SELECT url, crawled_at FROM crawl_cache ORDER BY crawled_at DESC LIMIT 5"
            )
    ...
    def analyze_crawl_status(result):
        ...
        print(
            f"   🌐 Domain crawling: {'✅' if config.get('crawl_entire_domain') else '❌'}"
        )
        ...
        total_attempts = summary.get("pages_processed", 0) + summary.get(
            "errors_encountered", 0
        )
        success_rate = (
            (summary.get("pages_processed", 0) / total_attempts * 100)
            if total_attempts > 0
            else 0
        )
    ...
    features_tested = [
        ...
        (
            "Data Persistence",
            cache_table_exists if "cache_table_exists" in locals() else False,
        ),
