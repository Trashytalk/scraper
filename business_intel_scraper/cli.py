...
try:
    from .backend.cli.phase2_cli import (
        setup_dom_change_parser,
        setup_spider_update_parser,
    )
    from .backend.cli.phase2_cli import (
        check_dom_changes_command,
        list_dom_changes_command,
    )
    from .backend.cli.phase2_cli import (
        analyze_page_command,
        generate_dom_report_command,
    )
    from .backend.cli.phase2_cli import (
        check_spider_updates_command,
        spider_update_status_command,
    )
...
def main() -> None:
    ...
    if PHASE3_AVAILABLE:
        phase3_parser = sub.add_parser(
            "phase3", help="Phase 3: Advanced ML-Powered Discovery"
        )
        phase3_subparsers = phase3_parser.add_subparsers(
            dest="phase3_cmd", required=True
        )

        # Content analysis
        analyze_parser = phase3_subparsers.add_parser(
            "analyze-content", help="Analyze content using ML"
        )
        analyze_parser.add_argument("url", help="URL to analyze")
        analyze_parser.add_argument(
            "--html-file", type=str, help="HTML file to analyze instead of fetching URL"
        )
        analyze_parser.add_argument(
            "--output", "-o", type=str, help="Output file for analysis results"
        )
        analyze_parser.add_argument(
            "--detailed", action="store_true", help="Show detailed feature analysis"
        )

        # Quality assessment
        quality_parser = phase3_subparsers.add_parser(
            "assess-quality", help="Assess data quality using ML"
        )
        ...
        quality_parser.add_argument(
            "--output", "-o", type=str, help="Output file for quality report"
        )
        quality_parser.add_argument(
            "--detailed", action="store_true", help="Show detailed quality metrics"
        )

        # Pattern learning
        learn_parser = phase3_subparsers.add_parser(
            "learn-patterns", help="Learn patterns from spider execution"
        )
        learn_parser.add_argument("session_file", help="Session data file")
        learn_parser.add_argument(
            "--output", "-o", type=str, help="Output file for learned patterns"
        )

        # Source discovery
        discover_parser = phase3_subparsers.add_parser(
            "discover-sources", help="Discover related sources using ML"
        )
        ...
        discover_parser.add_argument(
            "--output", "-o", type=str, help="Output file for discovered sources"
        )

        # Selector optimization
        optimize_parser = phase3_subparsers.add_parser(
            "optimize-selectors", help="Optimize extraction selectors"
        )
        optimize_parser.add_argument("url", help="URL to optimize selectors for")
        optimize_parser.add_argument(
            "selectors", nargs="+", help="Selectors to optimize"
        )
        optimize_parser.add_argument(
            "--output", "-o", type=str, help="Output file for optimization results"
        )

        # Learning cycle
        cycle_parser = phase3_subparsers.add_parser(
            "run-learning-cycle", help="Run adaptive learning cycle"
        )

        # Insights report
        insights_parser = phase3_subparsers.add_parser(
            "generate-insights", help="Generate ML insights report"
        )
        insights_parser.add_argument(
            "--days", type=int, default=30, help="Number of days to include in report"
        )
        insights_parser.add_argument(
            "--output", "-o", type=str, help="Output file for insights report"
        )

        # System status
        status_parser = phase3_subparsers.add_parser(
            "status", help="Show Phase 3 system status"
        )
    ...
    # Route Phase 2 commands
        ...
        try:
            ...
            if args.phase == "2" and not PHASE2_AVAILABLE:
                ...
                print(
                    "📦 Install required dependencies: pip install scikit-learn pandas numpy"
                )
                sys.exit(1)
        ...
        if PHASE3_AVAILABLE:
            try:
                ...
                if args.phase3_cmd == "analyze-content":
                    ...
                    print(
                        "💡 Use: python -m business_intel_scraper.backend.cli.phase3_commands analyze-content <url>"
                    )
                    ...
                    print("⚠️  Phase 3 CLI integration in progress")
                    print(
                        "💡 Use: python -m business_intel_scraper.backend.cli.phase3_commands assess-quality <data_file>"
                    )
                    ...
                    print("⚠️  Phase 3 CLI integration in progress")
                    print(
                        "💡 Use: python -m business_intel_scraper.backend.cli.phase3_commands learn-patterns <session_file>"
                    )
                    ...
                    print("⚠️  Phase 3 CLI integration in progress")
                    print(
                        "💡 Use: python -m business_intel_scraper.backend.cli.phase3_commands discover-sources <seed_url>"
                    )
                    ...
                    print("⚠️  Phase 3 CLI integration in progress")
                    print(
                        "💡 Use: python -m business_intel_scraper.backend.cli.phase3_commands optimize-selectors <url> <selectors>"
                    )
                    ...
                    print("⚠️  Phase 3 CLI integration in progress")
                    print(
                        "💡 Use: python -m business_intel_scraper.backend.cli.phase3_commands run-learning-cycle"
                    )
                    ...
                    print("⚠️  Phase 3 CLI integration in progress")
                    print(
                        "💡 Use: python -m business_intel_scraper.backend.cli.phase3_commands generate-insights"
                    )
                    ...
                    try:
                        from .backend.discovery.ml_content_analysis import (
                            content_analyzer,
                        )
                        from .backend.discovery.data_quality_assessment import (
                            quality_assessor,
                        )
                        from .backend.discovery.intelligent_pattern_recognition import (
                            pattern_recognizer,
                        )
