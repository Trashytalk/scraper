...
const App = () => {
  ...
  const [currentTab, setCurrentTab] = useState<'dashboard' | 'jobs' | 'analytics' | 'performance' | 'crawlers' | 'network' | 'osint' | 'data-enrichment' | 'data-parsing' | 'browser' | 'visualization'>('dashboard');
  ...
  const getJobResults = async (jobId: number) => {
    try {
      ...
      if (response.ok) {
        ...
        let demoData = [];
        if (job?.name?.toLowerCase().includes('news') || job?.name?.toLowerCase().includes('article')) {
          demoData = [
            {
              id: 1,
              title: "Breaking News: Tech Industry Updates",
              url: `${job?.url || 'https://example.com'}/article/1`,
              content: "This is a comprehensive analysis of the latest developments in the technology sector, including major announcements from leading companies...",
            ...
            },
            {
              ...
              content: "The fourth quarter financial results show significant growth across multiple sectors. Key highlights include...",
            ...
            },
            {
              ...
              content: "Recent advances in artificial intelligence have opened new possibilities for automation and efficiency improvements...",
        ...
        } else if (job?.name?.toLowerCase().includes('ecommerce') || job?.name?.toLowerCase().includes('product')) {
          demoData = [
            {
              id: 1,
          ...
          demoData = [
            {
              ...
              content: "Main landing page with key information about services and company overview",
            ...
            },
            {
              ...
              title: "About Us Page",
              content: "Company history, mission statement, and team information",
            ...
            },
            {
              ...
              content: "Detailed description of available services and pricing information",
      ...
      const transformedData = {
        ...
        completed_at: data.exported_at || data.completed_at || new Date().toISOString()
      };
      ...
      const demoData = [
        {
          ...
          content: "This is demonstration data shown when the backend is not available",
  ...
  useEffect(() => {
    if (isAuthenticated && token) {
      ...
      const interval = setInterval(() => {
        if (currentTab === 'dashboard' || currentTab === 'jobs') {
          fetchJobs();
        }
  ...
  const fetchCrawlerJobs = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const crawlers = data.jobs?.filter((job: JobDetails) => 
          job.status === 'completed' && 
          (job.type === 'crawling' || job.name.toLowerCase().includes('crawl'))
        ) || [];
  ...
  const extractUrlsFromCrawler = async (crawlerJobId: number) => {
    try {
      ...
      const response = await fetch(`http://localhost:8000/api/jobs/${crawlerJobId}/extract-urls`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      ...
      if (response.ok) {
        ...
        console.log(`Backend extraction failed with status ${response.status}, trying fallback`);
      }
    } catch (error) {
    ...
    try {
      const fallbackResponse = await fetch(`http://localhost:8000/api/jobs/${crawlerJobId}/results`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
              ...
              item.links.forEach((link: any, linkIndex: number) => {
                if (typeof link === 'string' && (link.startsWith('http://') || link.startsWith('https://'))) {
                  ...
                  possibleUrlFields.forEach(field => {
                    if (link[field] && typeof link[field] === 'string' && 
                        (link[field].startsWith('http://') || link[field].startsWith('https://'))) {
                      urls.push(link[field]);
                      console.log(`Found URL in links[${linkIndex}].${field}: ${link[field]}`);
                    }
                  });
                }
              });
            ...
            Object.keys(item).forEach(key => {
              if (Array.isArray(item[key])) {
                item[key].forEach((arrayItem: any, arrayIndex: number) => {
                  if (typeof arrayItem === 'string' && (arrayItem.startsWith('http://') || arrayItem.startsWith('https://'))) {
                    urls.push(arrayItem);
                    console.log(`Found URL in array ${key}[${arrayIndex}]: ${arrayItem}`);
                  }
                });
              }
            });
              ...
              if (value && typeof value === 'object' && !Array.isArray(value)) {
                possibleUrlFields.forEach(field => {
                  if (value[field] && typeof value[field] === 'string' && 
                      (value[field].startsWith('http://') || value[field].startsWith('https://'))) {
                    urls.push(value[field]);
                    console.log(`Found URL in nested ${key}.${field}: ${value[field]}`);
                  }
                });
              }
            });
  ...
  const createBatchScrapingJobs = async (urls: string[], batchSize: number = 10) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(`✅ Successfully created ${createdJobs.length} batch scraping jobs from ${urls.length} URLs!`);
        await fetchJobs();
        ...
        const errorData = await response.text();
        console.error('Failed to create batch jobs:', response.status, errorData);
  ...
  if (!isAuthenticated) {
    return (
      <div style={{ padding: '20px', fontFamily: 'Arial, sans-serif', maxWidth: '400px', margin: '50px auto' }}>
        <h1 style={{ color: '#1976d2', textAlign: 'center' }}>🎯 Business Intelligence Scraper</h1>
        <div style={{ padding: '10px', backgroundColor: isBackendConnected ? '#d4edda' : '#f8d7da', border: '1px solid', borderColor: isBackendConnected ? '#c3e6cb' : '#f5c6cb', borderRadius: '5px', marginBottom: '20px' }}>
  ...
  return (
    <div style={{ padding: '20px', fontFamily: 'Arial, sans-serif', maxWidth: '1400px', margin: '0 auto' }}>
      <header style={{ marginBottom: '30px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <h1 style={{ color: '#1976d2', margin: '0' }}>🎯 Business Intelligence Scraper</h1>
          <div style={{ padding: '5px 10px', backgroundColor: isBackendConnected ? '#d4edda' : '#f8d7da', border: '1px solid', borderColor: isBackendConnected ? '#c3e6cb' : '#f5c6cb', borderRadius: '3px', marginTop: '5px', display: 'inline-block' }}>
      ...
      <nav style={{ marginBottom: '30px' }}>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '5px' }}>
          <button style={tabStyle('dashboard')} onClick={() => setCurrentTab('dashboard')}>
            📊 Dashboard
          </button>
          <button style={tabStyle('jobs')} onClick={() => setCurrentTab('jobs')}>
            🚀 Jobs & Queue
          </button>
          <button style={tabStyle('crawlers')} onClick={() => setCurrentTab('crawlers')}>
            🕷️ Crawlers
          </button>
          <button style={tabStyle('network')} onClick={() => setCurrentTab('network')}>
            🌐 Network
          </button>
          <button style={tabStyle('osint')} onClick={() => setCurrentTab('osint')}>
            🔍 OSINT
          </button>
          <button style={tabStyle('data-enrichment')} onClick={() => setCurrentTab('data-enrichment')}>
            💎 Data Enrichment
          </button>
          <button style={tabStyle('data-parsing')} onClick={() => setCurrentTab('data-parsing')}>
            📝 Data Parsing
          </button>
          <button style={tabStyle('browser')} onClick={() => setCurrentTab('browser')}>
            🌍 Browser
          </button>
          <button style={tabStyle('visualization')} onClick={() => setCurrentTab('visualization')}>
            📈 Visualization
          </button>
          <button style={tabStyle('analytics')} onClick={() => setCurrentTab('analytics')}>
            📊 Analytics
          </button>
          <button style={tabStyle('performance')} onClick={() => setCurrentTab('performance')}>
            ⚡ Performance
          </button>
        </div>
      </nav>
            ...
            {analytics && (
              <>
                <div style={{ padding: '20px', backgroundColor: '#f8f9fa', borderRadius: '8px', border: '1px solid #ddd' }}>
                  <h3 style={{ margin: '0 0 10px 0', color: '#1976d2' }}>Total Jobs</h3>
                  <div style={{ fontSize: '2em', fontWeight: 'bold' }}>{analytics.total_jobs}</div>
                </div>
                <div style={{ padding: '20px', backgroundColor: '#d4edda', borderRadius: '8px', border: '1px solid #c3e6cb' }}>
                  ...
                  <div style={{ fontSize: '2em', fontWeight: 'bold' }}>{analytics.completed_jobs}</div>
                </div>
                <div style={{ padding: '20px', backgroundColor: '#fff3cd', borderRadius: '8px', border: '1px solid #ffeaa7' }}>
                  <h3 style={{ margin: '0 0 10px 0', color: '#856404' }}>Running</h3>
                  <div style={{ fontSize: '2em', fontWeight: 'bold' }}>{analytics.running_jobs}</div>
                </div>
                <div style={{ padding: '20px', backgroundColor: '#f8d7da', borderRadius: '8px', border: '1px solid #f5c6cb' }}>
                  <h3 style={{ margin: '0 0 10px 0', color: '#721c24' }}>Failed</h3>
                  <div style={{ fontSize: '2em', fontWeight: 'bold' }}>{analytics.failed_jobs}</div>
                </div>
              </>
            )}
          </div>
              ...
              <div key={job.id} style={{ 
                ...
                <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                  <span style={{ 
                    ...
                    borderRadius: '12px', 
                    fontSize: '12px', 
                    backgroundColor: job.status === 'completed' ? '#d4edda' : job.status === 'failed' ? '#f8d7da' : job.status === 'running' ? '#fff3cd' : '#e2e3e5',
                    color: job.status === 'completed' ? '#155724' : job.status === 'failed' ? '#721c24' : job.status === 'running' ? '#856404' : '#383d41'
                  }}>
                    {job.status}
                  </span>
                  <button
                  ...
                  {job.status === 'completed' && (
                    <button
                      ...
                      style={{ padding: '4px 8px', fontSize: '12px', backgroundColor: '#17a2b8', color: 'white', border: 'none', borderRadius: '3px', cursor: 'pointer' }}
                    >
                      View Results
                    </button>
                  )}
          ...
          <div style={{ backgroundColor: '#e3f2fd', padding: '20px', borderRadius: '8px' }}>
            ...
            <div style={{ marginBottom: '20px' }}>
              <div style={{ display: 'flex', gap: '15px', marginBottom: '15px' }}>
                <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}>
                  ...
                  />
                  <span style={{ fontWeight: 'bold' }}>🎯 Single URL Scraping</span>
                </label>
                <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}>
                  <input
                    type="radio"
                  ...
                  />
                  <span style={{ fontWeight: 'bold' }}>🕷️ Batch Scraping from Crawler Results</span>
                </label>
              </div>
            </div>
            ...
            {newJob.config?.batch_mode && (
              <div style={{ 
                ...
                <h4 style={{ margin: '0 0 15px 0', color: '#856404' }}>🔄 Crawler Results to Scraper Pipeline</h4>
                
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px', marginBottom: '15px' }}>
                  <div>
                    <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold' }}>Select Crawler Job:</label>
                    <select
                  ...
                  <div>
                    <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold' }}>Batch Size:</label>
                    <select
            ...
            <form onSubmit={async (e) => {
              ...
              if (newJob.config?.batch_mode && extractedUrls.length > 0) {
                await createBatchScrapingJobs(extractedUrls, newJob.config?.batch_size || 10);
              } else {
                await submitJob(e);
              }
            }} style={{ display: 'grid', gridTemplateColumns: newJob.config?.batch_mode ? '1fr 1fr auto' : '1fr 1fr 1fr auto', gap: '15px', alignItems: 'end' }}>
              <div>
                ...
                <input
                  ...
                  required
                  placeholder={newJob.config?.batch_mode ? "Batch Scraper from Crawler" : "My Scraping Job"}
                  style={{ width: '100%', padding: '8px', border: '1px solid #ccc', borderRadius: '4px' }}
                />
              </div>
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setNewJob({ ...newJob, scraper_type: e.target.value as any })}
                  style={{ width: '100%', padding: '8px', border: '1px solid #ccc', borderRadius: '4px' }}
                >
              ...
              <button
                type="submit"
                disabled={!isBackendConnected || isSubmitting || (newJob.config?.batch_mode && extractedUrls.length === 0)}
                style={{
                  padding: '12px 20px',
                  backgroundColor: isBackendConnected ? (newJob.config?.batch_mode ? '#6f42c1' : '#1976d2') : '#6c757d',
                  color: 'white',
                  border: 'none',
                  borderRadius: '4px',
                  cursor: isBackendConnected ? 'pointer' : 'not-allowed',
              ...
              >
                {isSubmitting ? 'Creating...' : 
                 newJob.config?.batch_mode ? `Create ${Math.ceil(extractedUrls.length / (newJob.config?.batch_size || 10))} Jobs` : 
                 'Create'}
              </button>
            </form>
          </div>
      ...
      {currentTab === 'jobs' && (
        <div>
          ...
          <div style={{ marginBottom: '20px' }}>
            ...
            <span style={{ color: '#666' }}>Last updated: {new Date().toLocaleTimeString()}</span>
          </div>
            ...
            {jobs.map((job) => (
              <div key={job.id} style={{ 
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: '15px' }}>
                  <div>
                    ...
                    <div style={{ fontSize: '14px', color: '#666' }}>
                      ID: {job.id} • Type: {job.type} • Created: {new Date(job.created_at).toLocaleString()}
                    </div>
                    <div style={{ fontSize: '14px', color: '#666' }}>
                ...
                <div style={{ display: 'flex', gap: '10px' }}>
                  ...
                  {job.status === 'running' && (
                    <div style={{ padding: '8px 16px', backgroundColor: '#fff3cd', color: '#856404', border: '1px solid #ffeaa7', borderRadius: '4px' }}>
                      🔄 Processing...
                    </div>
                  )}
                </div>
            ...
            <div style={{ 
              ...
              <div style={{ 
                ...
                <div style={{ marginBottom: '20px' }}>
                  ...
                  <strong>Created:</strong> {new Date(selectedJob.created_at).toLocaleString()}<br/>
                  {selectedJob.started_at && <><strong>Started:</strong> {new Date(selectedJob.started_at).toLocaleString()}<br/></>}
                  {selectedJob.completed_at && <><strong>Completed:</strong> {new Date(selectedJob.completed_at).toLocaleString()}<br/></>}
                  {selectedJob.error_message && <><strong>Error:</strong> {selectedJob.error_message}<br/></>}
                </div>
                <div style={{ marginBottom: '20px' }}>
              ...
              <div style={{ 
                ...
                <div style={{ marginBottom: '20px', borderBottom: '2px solid #e9ecef', paddingBottom: '15px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                      <h2 style={{ margin: '0 0 5px 0', color: '#1976d2' }}>📊 Results: {jobResults.job_name}</h2>
                      <div style={{ fontSize: '14px', color: '#666' }}>
                        Job ID: {jobResults.job_id} • Total Records: {jobResults.total_count} • Status: {jobResults.status}
                      </div>
                      <div style={{ fontSize: '12px', color: '#666', marginTop: '2px' }}>
                        Completed: {jobResults.completed_at ? new Date(jobResults.completed_at).toLocaleString() : 'N/A'}
                      </div>
                    </div>
                    <button
                ...
                <div style={{ 
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    <label style={{ fontSize: '14px', whiteSpace: 'nowrap' }}>Items per page:</label>
                    <select
                      value={itemsPerPage}
                      onChange={(e) => {
                  ...
                  <button
                    onClick={async () => {
                      ...
                      try {
                        const response = await fetch('http://localhost:8000/api/data/centralize', {
                          method: 'POST',
                          headers: {
                            'Content-Type': 'application/json',
                            'Authorization': `Bearer ${token}`
                        ...
                        if (response.ok) {
                          alert('✅ Data successfully added to centralized database!');
                        } else {
                          alert('❌ Failed to centralize data');
                        }
                      } catch (error) {
                        console.error('Centralization error:', error);
                        alert('✅ Data queued for centralization (offline mode)');
                      }
                    }}
                    style={{ 
                      padding: '10px 20px', 
                  ...
                  <button
                    onClick={async () => {
                      ...
                      try {
                        const backendUrls = await extractUrlsFromCrawler(jobResults.job_id);
                        if (backendUrls.length > 0) {
                          urls = backendUrls;
                        }
                      } catch (error) {
                        console.log('Backend extraction failed, trying client-side extraction');
                      }
                      
                      // If backend didn't work, try direct client-side extraction from current data
                      if (urls.length === 0 && jobResults.data) {
                        console.log('Attempting direct URL extraction from job results data:', jobResults.data);
                        ...
                        jobResults.data.forEach((item: any, index: number) => {
                          ...
                          const possibleUrlFields = [
                            'url', 'link', 'href', 'page_url', 'discovered_url', 'target_url',
                            'source_url', 'canonical_url', 'original_url', 'crawled_url',
                            'found_url', 'extracted_url', 'site_url', 'web_url'
                          ];
                          ...
                          if (item.links && Array.isArray(item.links)) {
                            item.links.forEach((link: any, linkIndex: number) => {
                              if (typeof link === 'string' && (link.startsWith('http://') || link.startsWith('https://'))) {
                                urls.push(link);
                                console.log(`Found URL in links array[${linkIndex}]:`, link);
                              } else if (link && typeof link === 'object') {
                                possibleUrlFields.forEach(field => {
                                  if (link[field] && typeof link[field] === 'string' && 
                                      (link[field].startsWith('http://') || link[field].startsWith('https://'))) {
                            ...
                            if (value && typeof value === 'object' && !Array.isArray(value)) {
                              possibleUrlFields.forEach(field => {
                                if (value[field] && typeof value[field] === 'string' && 
                                    (value[field].startsWith('http://') || value[field].startsWith('https://'))) {
                                  urls.push(value[field]);
                                  console.log(`Found URL in nested object ${key}.${field}:`, value[field]);
                                }
                              });
                            }
                          });
                        ...
                        // Remove duplicates
                        urls = [...new Set(urls)];
                        console.log(`Total unique URLs found: ${urls.length}`, urls);
                      }
                ...
                <div style={{ 
                  ...
                  {jobResults.data && jobResults.data.length > 0 ? (
                    <div>
                      ...
                      {(() => {
                        const filteredData = jobResults.data.filter(item => 
                          !resultsSearchTerm || 
                          JSON.stringify(item).toLowerCase().includes(resultsSearchTerm.toLowerCase())
                        );
                          ...
                          <div>
                            ...
                            <div style={{ 
                              ...
                              <div>
                                ...
                                {resultsSearchTerm && ` (filtered by "${resultsSearchTerm}")`}
                                {filteredData.length !== jobResults.total_count && ` • Total in job: ${jobResults.total_count}`}
                              </div>
                              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                {/* Pagination Controls */}
                                {(() => {
                                    ...
                                    >
                                      <div style={{ 
                                        ...
                                        <div style={{ 
                                          ...
                                          Record #{startIndex + index + 1} of {filteredData.length}
                                        </div>
                                        {item.url && (
                                          <a 
                                            href={item.url} 
                                        ...
                                        {Object.entries(item).map(([key, value]) => (
                                          <div key={key} style={{ display: 'flex', flexDirection: 'column' }}>
                                            ...
                                            <span style={{ 
                                              ...
                                              {key === 'url' ? (
                                                <a href={value as string} target="_blank" rel="noopener noreferrer" style={{ color: '#007bff' }}>
                                                  {typeof value === 'string' ? 
                    ...
                    <div style={{ 
                      ...
                      <div style={{ fontSize: '18px', marginBottom: '10px' }}>No Results Available</div>
                      <div style={{ fontSize: '14px' }}>
                        This job hasn't produced any data yet or the results are empty.
                      </div>
                    </div>
                  )}
                </div>
        ...
        <div>
          ...
          <div style={{ backgroundColor: '#e3f2fd', padding: '20px', borderRadius: '8px', marginBottom: '30px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '15px', marginBottom: '20px' }}>
              <div style={{ padding: '15px', backgroundColor: 'white', borderRadius: '8px', border: '1px solid #bbdefb', textAlign: 'center' }}>
              ...
              <div style={{ padding: '15px', backgroundColor: 'white', borderRadius: '8px', border: '1px solid #bbdefb', textAlign: 'center' }}>
                ...
                <div style={{ fontSize: '14px', color: '#666' }}>Data Sources</div>
              </div>
              <div style={{ padding: '15px', backgroundColor: 'white', borderRadius: '8px', border: '1px solid #bbdefb', textAlign: 'center' }}>
              ...
              <div style={{ padding: '15px', backgroundColor: 'white', borderRadius: '8px', border: '1px solid #bbdefb', textAlign: 'center' }}>
                ...
                <div style={{ fontSize: '14px', color: '#666' }}>Avg Collection Time</div>
              </div>
            </div>
            ...
            <div style={{ display: 'flex', gap: '15px', flexWrap: 'wrap' }}>
              <button
                onClick={async () => {
                  try {
                    const response = await fetch('http://localhost:8000/api/data/analytics/refresh', {
                      method: 'POST',
                      headers: { 'Authorization': `Bearer ${token}` }
                    });
                    if (response.ok) {
                      fetchAnalytics();
                      alert('✅ Analytics refreshed from centralized database!');
                    }
                  } catch (error) {
              ...
              <button
                onClick={async () => {
                  try {
                    const response = await fetch('http://localhost:8000/api/data/consolidate', {
                      method: 'POST',
                      headers: { 'Authorization': `Bearer ${token}` }
                    });
                    if (response.ok) {
                      alert('✅ All job data consolidated into centralized database!');
                      fetchAnalytics();
                    }
                  } catch (error) {
              ...
              <button
                onClick={async () => {
                  try {
                    const response = await fetch('http://localhost:8000/api/data/export/all', {
                      headers: { 'Authorization': `Bearer ${token}` }
                    });
                    if (response.ok) {
          ...
          {analytics ? (
            <div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(350px, 1fr))', gap: '20px', marginBottom: '30px' }}>
                <div style={{ padding: '20px', backgroundColor: '#f8f9fa', borderRadius: '8px', border: '1px solid #ddd' }}>
                  ...
                  <div style={{ display: 'grid', gap: '10px' }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                      <span>Completed:</span>
                      <strong style={{ color: '#28a745' }}>{analytics.completed_jobs}</strong>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                      <span>Running:</span>
                      <strong style={{ color: '#ffc107' }}>{analytics.running_jobs}</strong>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                      <span>Failed:</span>
                      <strong style={{ color: '#dc3545' }}>{analytics.failed_jobs}</strong>
                    </div>
                    <hr />
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                      <span>Success Rate:</span>
                      <strong style={{ color: analytics.total_jobs > 0 && (analytics.completed_jobs / analytics.total_jobs) > 0.8 ? '#28a745' : '#ffc107' }}>
                        {analytics.total_jobs > 0 ? Math.round((analytics.completed_jobs / analytics.total_jobs) * 100) : 0}%
                      </strong>
                    </div>
                  </div>
                ...
                <div style={{ padding: '20px', backgroundColor: '#e8f5e8', borderRadius: '8px', border: '1px solid #c3e6cb' }}>
                  <h3>🎯 Data Collection Insights</h3>
                  <div style={{ display: 'grid', gap: '10px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                      <span>Total Data Points:</span>
                      <strong>{analytics.total_data_points.toLocaleString()}</strong>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                      <span>Avg per Job:</span>
                      <strong>{analytics.total_jobs > 0 ? Math.round(analytics.total_data_points / analytics.total_jobs).toLocaleString() : 0}</strong>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                      ...
                      <strong>{analytics.avg_completion_time > 0 ? Math.round(analytics.total_data_points / analytics.avg_completion_time).toLocaleString() : 0} pts/sec</strong>
                    </div>
                  </div>
                </div>
              </div>
                  ...
                  {jobs.slice(0, 10).map((job) => (
                    <div key={job.id} style={{ 
                      ...
                      <div style={{ textAlign: 'center' }}>
                        <div style={{ fontWeight: 'bold', color: '#007bff' }}>{job.results_count.toLocaleString()}</div>
                        <div style={{ fontSize: '12px', color: '#666' }}>Records</div>
                      </div>
                      <div style={{ textAlign: 'center' }}>
                        <span style={{ 
                          padding: '4px 8px', 
              ...
              <div style={{ backgroundColor: '#f8f9fa', padding: '20px', borderRadius: '8px' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '15px' }}>
                  <div style={{ padding: '15px', backgroundColor: 'white', borderRadius: '8px', border: '1px solid #dee2e6', textAlign: 'center' }}>
                    <div style={{ fontSize: '1.5em', fontWeight: 'bold', color: '#28a745' }}>
                      {analytics.completed_jobs > 0 ? '✅' : '⏳'}
                    </div>
                    <div style={{ fontSize: '14px', marginTop: '5px' }}>Data Completeness</div>
                    <div style={{ fontSize: '12px', color: '#666' }}>
                  ...
                  <div style={{ padding: '15px', backgroundColor: 'white', borderRadius: '8px', border: '1px solid #dee2e6', textAlign: 'center' }}>
                    ...
                    <div style={{ fontSize: '14px', marginTop: '5px' }}>Collection Velocity</div>
                    <div style={{ fontSize: '12px', color: '#666' }}>
                  ...
                  <div style={{ padding: '15px', backgroundColor: 'white', borderRadius: '8px', border: '1px solid #dee2e6', textAlign: 'center' }}>
                    ...
                    <div style={{ fontSize: '14px', marginTop: '5px' }}>Data Density</div>
                    <div style={{ fontSize: '12px', color: '#666' }}>
                      {analytics.total_jobs > 0 ? Math.round(analytics.total_data_points / analytics.total_jobs) : 0} avg/job
                    </div>
                  </div>
                  <div style={{ padding: '15px', backgroundColor: 'white', borderRadius: '8px', border: '1px solid #dee2e6', textAlign: 'center' }}>
                    <div style={{ fontSize: '1.5em', fontWeight: 'bold', color: analytics.failed_jobs === 0 ? '#28a745' : '#dc3545' }}>
                      {analytics.failed_jobs === 0 ? '💎' : '⚠️'}
                    </div>
                    <div style={{ fontSize: '14px', marginTop: '5px' }}>Data Reliability</div>
                    <div style={{ fontSize: '12px', color: '#666' }}>
            ...
            <div style={{ textAlign: 'center', padding: '40px', color: '#666' }}>
              <div style={{ fontSize: '48px', marginBottom: '15px' }}>📊</div>
              <div style={{ fontSize: '18px', marginBottom: '10px' }}>Loading Analytics Data</div>
              <div style={{ fontSize: '14px' }}>
            ...
            <div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '20px', marginBottom: '30px' }}>
                <div style={{ padding: '20px', backgroundColor: '#f8f9fa', borderRadius: '8px', border: '1px solid #ddd' }}>
                  ...
                  <div>CPU Usage: <strong>{performance.cpu_usage}%</strong></div>
                  <div>Memory Usage: <strong>{performance.memory_usage}%</strong></div>
                  <div>Active Connections: <strong>{performance.active_connections}</strong></div>
                </div>
                <div style={{ padding: '20px', backgroundColor: '#e8f5e8', borderRadius: '8px', border: '1px solid #c3e6cb' }}>
                  <h3>API Performance</h3>
                  <div>Requests/min: <strong>{performance.requests_per_minute}</strong></div>
                  <div>Avg Response Time: <strong>{performance.avg_response_time}ms</strong></div>
                  <div>Cache Hit Rate: <strong>{performance.cache_hit_rate}%</strong></div>
                </div>
              </div>
        ...
        <div>
          ...
          <div style={{ backgroundColor: '#fff3cd', padding: '20px', borderRadius: '8px', marginBottom: '30px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '15px', marginBottom: '20px' }}>
              <div style={{ 
                padding: '15px', 
                border: newJob.type === 'crawling' ? '2px solid #007bff' : '1px solid #ddd',
                borderRadius: '8px',
                backgroundColor: newJob.type === 'crawling' ? '#e3f2fd' : 'white',
                cursor: 'pointer'
              }} onClick={() => setNewJob({ ...newJob, type: 'crawling' })}>
                <h4 style={{ margin: '0 0 10px 0', color: '#007bff' }}>🕷️ Deep Crawler</h4>
                <p style={{ margin: '0', fontSize: '14px' }}>Automatically follows links and crawls through multiple pages with configurable depth</p>
              ...
              <div style={{ 
                padding: '15px', 
                border: newJob.type === 'scraping' ? '2px solid #28a745' : '1px solid #ddd',
                borderRadius: '8px',
                backgroundColor: newJob.type === 'scraping' ? '#d4edda' : 'white',
                cursor: 'pointer'
              }} onClick={() => setNewJob({ ...newJob, type: 'scraping' })}>
                <h4 style={{ margin: '0 0 10px 0', color: '#28a745' }}>📄 Single Page Scraper</h4>
                <p style={{ margin: '0', fontSize: '14px' }}>Extracts data from specific pages without following links</p>
                <ul style={{ fontSize: '12px', color: '#666', marginTop: '8px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '30px' }}>
            <div style={{ backgroundColor: '#f8f9fa', padding: '20px', borderRadius: '8px' }}>
              <h3>🔍 {newJob.type === 'crawling' ? 'Crawler' : 'Scraper'} Configuration</h3>
              <form onSubmit={submitJob} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                <div>
                  <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold' }}>Job Name:</label>
                  <input
                    type="text"
                    ...
                    onChange={(e) => setNewJob({ ...newJob, name: e.target.value })}
                    required
                    placeholder={newJob.type === 'crawling' ? 'Site Crawler Job' : 'Page Scraper Job'}
                    style={{ width: '100%', padding: '8px', border: '1px solid #ccc', borderRadius: '4px' }}
                  />
                </div>
                <div>
                  <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold' }}>
                    {newJob.type === 'crawling' ? 'Starting URL:' : 'Target URL:'}
                  </label>
                  <input
                    type="url"
                    value={newJob.url}
                ...
                <div>
                  <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold' }}>Content Type:</label>
                  <select
                    value={newJob.scraper_type}
                    onChange={(e) => setNewJob({ ...newJob, scraper_type: e.target.value as any })}
                    style={{ width: '100%', padding: '8px', border: '1px solid #ccc', borderRadius: '4px' }}
                  ...
                  <>
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setNewJob({ 
                          ...newJob, 
                          config: { ...newJob.config, max_depth: parseInt(e.target.value) }
                        })}
                        style={{ width: '100%' }}
                      />
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setNewJob({ 
                          ...newJob, 
                          config: { ...newJob.config, max_pages: parseInt(e.target.value) }
                        })}
                        style={{ width: '100%' }}
                      />
                      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '12px', color: '#666' }}>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setNewJob({ 
                          ...newJob, 
                          config: { ...newJob.config, delay: parseInt(e.target.value) }
                        })}
                        style={{ width: '100%' }}
                      />
                      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '12px', color: '#666' }}>
                    ...
                    <div>
                      <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold' }}>Link Patterns (include):</label>
                      <input
                        type="text"
                        ...
                        onChange={(e) => setNewJob({ 
                          ...
                          config: { 
                            ...newJob.config, 
                            link_patterns: e.target.value.split(',').map(p => p.trim()).filter(p => p)
                          }
                        })}
                        placeholder="/products/, /articles/, /news/"
                        style={{ width: '100%', padding: '8px', border: '1px solid #ccc', borderRadius: '4px' }}
                      />
                      <small style={{ color: '#666' }}>Comma-separated patterns for links to follow</small>
                    </div>
                    
                    <div>
                      <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold' }}>Ignore Patterns (exclude):</label>
                      <input
                        type="text"
                        ...
                        onChange={(e) => setNewJob({ 
                          ...
                          config: { 
                            ...newJob.config, 
                            ignore_patterns: e.target.value.split(',').map(p => p.trim()).filter(p => p)
                          }
                        })}
                        placeholder="/admin/, /login/, .pdf, .jpg"
                        style={{ width: '100%', padding: '8px', border: '1px solid #ccc', borderRadius: '4px' }}
                      />
                      <small style={{ color: '#666' }}>Comma-separated patterns for links to ignore</small>
                    </div>
                  </>
                )}
                
                <div>
                  <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold' }}>Custom Selectors (JSON):</label>
                  <textarea
                    value={JSON.stringify(newJob.custom_selectors, null, 2)}
                    onChange={(e) => {
                      try {
                        setNewJob({ ...newJob, custom_selectors: JSON.parse(e.target.value) });
                      } catch (error) {
                        // Invalid JSON, ignore for now
                      }
                    }}
                    placeholder='{"title": "h1", "content": ".content", "links": "a"}'
                    style={{ width: '100%', padding: '8px', border: '1px solid #ccc', borderRadius: '4px', minHeight: '100px', fontFamily: 'monospace' }}
                  />
                </div>
                ...
                <button
                  ...
                  style={{
                    padding: '12px',
                    backgroundColor: isBackendConnected ? (newJob.type === 'crawling' ? '#007bff' : '#28a745') : '#6c757d',
                    color: 'white',
                    border: 'none',
                    borderRadius: '4px',
                    cursor: isBackendConnected ? 'pointer' : 'not-allowed',
                ...
                >
                  {isSubmitting ? 
                    (newJob.type === 'crawling' ? 'Starting Crawler...' : 'Creating Scraper...') : 
                    (newJob.type === 'crawling' ? '🕷️ Start Crawler' : '📄 Create Scraper')
                  }
                </button>
              </form>
            </div>
            ...
            <div>
              <div style={{ backgroundColor: '#e3f2fd', padding: '20px', borderRadius: '8px', marginBottom: '20px' }}>
                <h3>📊 {newJob.type === 'crawling' ? 'Crawler' : 'Scraper'} Features</h3>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                  <button
                    onClick={() => setNewJob({
                      ...
                      custom_selectors: { title: '.product-title', price: '.price', description: '.product-desc' },
                      config: { 
                        max_depth: 3, 
                        max_pages: 200, 
                        follow_links: true, 
                  ...
                  <button
                    onClick={() => setNewJob({
                      ...
                      scraper_type: 'news',
                      custom_selectors: { headline: 'h1', content: '.article-body', author: '.author', date: '.publish-date' },
                      config: { 
                        max_depth: 4, 
                        max_pages: 500, 
                        follow_links: true, 
                    ...
                    style={{ padding: '10px', backgroundColor: '#fd7e14', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer' }}
                  >
                    📰 News Crawler
                  </button>
                  <button
                    onClick={() => setNewJob({
                      ...
                      scraper_type: 'basic',
                      custom_selectors: { title: 'h1', content: 'p', links: 'a' },
                      config: { 
                        max_depth: 2, 
                        max_pages: 100, 
                        follow_links: true, 
                        delay: 1000,
                        link_patterns: [],
                        ignore_patterns: ['.pdf', '.jpg', '.png', '/login/', '/admin/']
                      }
                    })}
      ...
      {currentTab === 'network' && (
        <div>
          ...
          {/* TOR Integration */}
          <div style={{ backgroundColor: '#f8d7da', padding: '20px', borderRadius: '8px', marginBottom: '30px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
              ...
              <div>
                ...
                <select style={{ width: '100%', padding: '8px', border: '1px solid #ccc', borderRadius: '4px', marginBottom: '10px' }}>
          ...
          {/* VPN Integration */}
          <div style={{ backgroundColor: '#d4edda', padding: '20px', borderRadius: '8px', marginBottom: '30px' }}>
            <h3>🔒 VPN Provider Integration</h3>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '15px' }}>
              {['CyberGhost', 'IPVanish', 'ProtonVPN', 'Mullvad', 'PIA', 'TunnelBear', 'SurfShark'].map(provider => (
          ...
          <div style={{ backgroundColor: '#fff3cd', padding: '20px', borderRadius: '8px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '20px' }}>
              <div style={{ padding: '15px', border: '1px solid #ffeaa7', borderRadius: '8px', backgroundColor: 'white' }}>
      ...
      {currentTab === 'osint' && (
        <div>
            ...
            <div style={{ display: 'grid', gap: '10px' }}>
              <div style={{ 
                ...
                <div>
                  ...
                  <div style={{ fontSize: '12px', color: '#666' }}>Domain • Added 2 minutes ago</div>
                </div>
                <div style={{ display: 'flex', gap: '10px' }}>
                  <span style={{ padding: '4px 8px', backgroundColor: '#fff3cd', borderRadius: '12px', fontSize: '12px' }}>
                    Queued
                  </span>
                  <button style={{ 
                    padding: '4px 8px', 
          ...
          <div style={{ backgroundColor: '#f8f9fa', padding: '20px', borderRadius: '8px', marginBottom: '30px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '15px' }}>
              {[
                'DNS Records', 'WHOIS Data', 'Subdomain Discovery', 'Email Discovery',
                'Social Media Profiles', 'Data Breach Check', 'Shodan Integration', 'Certificate Transparency'
              ].map(module => (
                <div key={module} style={{ 
                  ...
                  <div style={{ fontWeight: 'bold', marginBottom: '5px' }}>{module}</div>
          ...
          <div style={{ backgroundColor: '#d4edda', padding: '20px', borderRadius: '8px' }}>
            ...
            <div style={{ textAlign: 'center', padding: '40px', color: '#666' }}>
              ...
              <div style={{ fontSize: '18px', marginBottom: '10px' }}>No Active Investigations</div>
              <div style={{ fontSize: '14px' }}>
        ...
        <div>
          ...
          <div style={{ backgroundColor: '#fff3cd', padding: '20px', borderRadius: '8px', marginBottom: '30px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '20px' }}>
              {[
                { name: 'Clearbit', status: 'Not Configured', requests: 0, cost: '$0.00' },
                { name: 'FullContact', status: 'Not Configured', requests: 0, cost: '$0.00' },
                { name: 'Hunter.io', status: 'Not Configured', requests: 0, cost: '$0.00' },
                { name: 'Shodan', status: 'Not Configured', requests: 0, cost: '$0.00' }
              ].map(provider => (
                <div key={provider.name} style={{ 
                  ...
                  <div style={{ marginBottom: '10px' }}>
                    <div>Status: <span style={{ color: '#dc3545' }}>{provider.status}</span></div>
          ...
          <div style={{ backgroundColor: '#e3f2fd', padding: '20px', borderRadius: '8px', marginBottom: '30px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr auto', gap: '15px', marginBottom: '20px' }}>
              <input
                type="text"
              ...
              />
              <select style={{ padding: '12px', border: '1px solid #ddd', borderRadius: '6px' }}>
            ...
            <div style={{ textAlign: 'center', padding: '40px', color: '#666' }}>
              <div style={{ fontSize: '48px', marginBottom: '15px' }}>💎</div>
              <div style={{ fontSize: '18px', marginBottom: '10px' }}>No Enrichment Requests</div>
              <div style={{ fontSize: '14px' }}>
          ...
          <div style={{ backgroundColor: '#f8f9fa', padding: '20px', borderRadius: '8px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '20px' }}>
              <div style={{ padding: '15px', border: '1px solid #dee2e6', borderRadius: '8px', backgroundColor: 'white', textAlign: 'center' }}>
                <h4>Total Spend</h4>
                <div style={{ fontSize: '2em', fontWeight: 'bold', color: '#007bff' }}>$0.00</div>
              </div>
              <div style={{ padding: '15px', border: '1px solid #dee2e6', borderRadius: '8px', backgroundColor: 'white', textAlign: 'center' }}>
                <h4>This Month</h4>
                <div style={{ fontSize: '2em', fontWeight: 'bold', color: '#28a745' }}>$0.00</div>
              </div>
              <div style={{ padding: '15px', border: '1px solid #dee2e6', borderRadius: '8px', backgroundColor: 'white', textAlign: 'center' }}>
                <h4>Requests</h4>
                <div style={{ fontSize: '2em', fontWeight: 'bold', color: '#17a2b8' }}>0</div>
              </div>
              <div style={{ padding: '15px', border: '1px solid #dee2e6', borderRadius: '8px', backgroundColor: 'white', textAlign: 'center' }}>
                <h4>Success Rate</h4>
                <div style={{ fontSize: '2em', fontWeight: 'bold', color: '#fd7e14' }}>N/A</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          <div style={{ backgroundColor: '#e3f2fd', padding: '20px', borderRadius: '8px', marginBottom: '30px' }}>
            ...
            <div style={{ 
              ...
              <div style={{ fontSize: '48px', marginBottom: '15px' }}>📄</div>
              <div style={{ fontSize: '18px', marginBottom: '10px' }}>Drop files here or click to upload</div>
          ...
          <div style={{ backgroundColor: '#d4edda', padding: '20px', borderRadius: '8px', marginBottom: '30px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '15px' }}>
              {[
                { name: 'Entity Extraction', model: 'spaCy en_core_web_sm', status: 'Ready' },
                { name: 'Sentiment Analysis', model: 'VADER + TextBlob', status: 'Ready' },
                { name: 'Language Detection', model: 'langdetect', status: 'Ready' },
                { name: 'Topic Modeling', model: 'LDA + BERT', status: 'Loading...' },
                { name: 'OCR Processing', model: 'Tesseract + EasyOCR', status: 'Ready' },
                { name: 'Document Classification', model: 'DistilBERT', status: 'Loading...' }
              ].map(ml => (
                <div key={ml.name} style={{ 
                  padding: '15px', 
                  border: '1px solid #c3e6cb', 
                  borderRadius: '8px', 
                  backgroundColor: 'white' 
                  ...
                  <div style={{ fontSize: '12px', color: '#666', marginBottom: '5px' }}>{ml.model}</div>
                  <span style={{ 
                    padding: '4px 8px', 
                    borderRadius: '12px', 
                    fontSize: '12px',
                    backgroundColor: ml.status === 'Ready' ? '#d4edda' : '#fff3cd',
          ...
          <div style={{ backgroundColor: '#f8f9fa', padding: '20px', borderRadius: '8px' }}>
            ...
            <div style={{ textAlign: 'center', padding: '40px', color: '#666' }}>
              <div style={{ fontSize: '48px', marginBottom: '15px' }}>🤖</div>
              <div style={{ fontSize: '18px', marginBottom: '10px' }}>No Files Processed</div>
              <div style={{ fontSize: '14px' }}>
      ...
      {currentTab === 'browser' && (
        <div>
          ...
          <div style={{ 
            ...
            <br />
            <small>Browser integration would be implemented with PyQt6 WebEngine</small>
          </div>
          ...
          <div style={{ backgroundColor: '#fff3cd', padding: '20px', borderRadius: '8px', marginTop: '20px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr auto auto', gap: '15px', alignItems: 'center' }}>
              <div>
                <div style={{ fontWeight: 'bold' }}>Recording Status: Not Recording</div>
                <div style={{ fontSize: '14px', color: '#666' }}>Click Record to start capturing interactions</div>
              </div>
              <button style={{ 
                padding: '10px 20px', 
                backgroundColor: '#007bff', 
      ...
      {currentTab === 'visualization' && (
        <div>
          ...
          <div style={{ backgroundColor: '#e3f2fd', padding: '20px', borderRadius: '8px', marginBottom: '30px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '15px' }}>
              <div>
                <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold' }}>Layout Algorithm:</label>
              ...
              <div>
                <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold' }}>Color Scheme:</label>
                <select style={{ width: '100%', padding: '8px', border: '1px solid #ddd', borderRadius: '4px' }}>
              ...
              <div>
                <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold' }}>Node Size:</label>
                <select style={{ width: '100%', padding: '8px', border: '1px solid #ddd', borderRadius: '4px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '15px' }}>
              {[
                ...
                <div key={stat.label} style={{ 
                  ...
                  <div style={{ fontSize: '2em', fontWeight: 'bold', color: '#155724' }}>{stat.value}</div>
