...
class TestEntityRelationshipMapper:
    ...
    def test_extract_officer_relationships(self, relationship_mapper, sample_entities):
        ...
        relationships = relationship_mapper._extract_officer_relationships(sample_entities)
        
        # Should find officer relationships
        assert len(relationships) > 0
        
        # Check for John Smith as CEO of TechCorp
        ceo_relationships = [r for r in relationships if 
                           r.source_entity == 'person_1' and 
                           r.target_entity == 'company_1' and
                           'ceo' in r.metadata.get('role', '').lower()]
        assert len(ceo_relationships) > 0
        
        # Check for Jane Doe as CFO of TechCorp
        cfo_relationships = [r for r in relationships if
                           r.source_entity == 'person_2' and
                           r.target_entity == 'company_1' and
                           'cfo' in r.metadata.get('role', '').lower()]
        assert len(cfo_relationships) > 0

    def test_extract_ownership_relationships(self, relationship_mapper, sample_entities):
        """Test extraction of ownership relationships"""
        relationships = relationship_mapper._extract_ownership_relationships(sample_entities)
        
        # Should find ownership relationships
        assert len(relationships) > 0
        
        # Check for TechCorp Labs owned by TechCorp Inc
        ownership_rels = [r for r in relationships if
                         r.source_entity == 'company_1' and
                         r.target_entity == 'company_2' and
                         r.relationship_type == RelationshipType.OWNERSHIP]
    ...
    def test_extract_shared_address_relationships(self, relationship_mapper, sample_entities):
        """Test extraction of shared address relationships"""
        relationships = relationship_mapper._extract_shared_address_relationships(sample_entities)
        
        # Should find shared address relationships
        assert len(relationships) > 0
        
        # Check for TechCorp and TechCorp Labs sharing address
        shared_addr_rels = [r for r in relationships if
                          ((r.source_entity == 'company_1' and r.target_entity == 'company_2') or
                           (r.source_entity == 'company_2' and r.target_entity == 'company_1')) and
                          r.relationship_type == RelationshipType.SHARED_ADDRESS]
        assert len(shared_addr_rels) > 0

    def test_extract_shared_contact_relationships(self, relationship_mapper, sample_entities):
        """Test extraction of shared contact relationships"""
        relationships = relationship_mapper._extract_shared_contact_relationships(sample_entities)
        
        # Should find shared contact relationships (email domains, etc.)
        shared_contact_rels = [r for r in relationships if
                             r.relationship_type == RelationshipType.SHARED_CONTACT]
    ...
    def test_extract_shared_domain_relationships(self, relationship_mapper, sample_entities):
        """Test extraction of shared domain relationships"""
        relationships = relationship_mapper._extract_shared_domain_relationships(sample_entities)
        
        # Should find shared domain relationships
        assert len(relationships) > 0
        
        # Check for TechCorp and TechCorp Labs sharing domain
        shared_domain_rels = [r for r in relationships if
                            ((r.source_entity == 'company_1' and r.target_entity == 'company_2') or
                             (r.source_entity == 'company_2' and r.target_entity == 'company_1')) and
                            r.relationship_type == RelationshipType.SHARED_DOMAIN]
    ...
    def test_calculate_relationship_strength(self, relationship_mapper, sample_entities):
        ...
        officer_strength = relationship_mapper._calculate_relationship_strength(officer_rel, sample_entities)
        assert officer_strength > 0.8
        
        # Ownership relationships should have very high strength
        ownership_strength = relationship_mapper._calculate_relationship_strength(ownership_rel, sample_entities)
        assert ownership_strength > 0.9

    def test_extract_relationships_comprehensive(self, relationship_mapper, sample_entities):
        """Test comprehensive relationship extraction"""
        relationship_types = ['officer', 'ownership', 'address', 'contact', 'domain']
        relationships = relationship_mapper.extract_relationships(sample_entities, relationship_types)
        ...
        for rel in relationships:
            ...
            assert rel.confidence_score >= relationship_mapper.config['confidence_threshold']

    def test_build_network_graph(self, relationship_mapper, sample_entities):
        """Test network graph construction and analysis"""
        relationships = relationship_mapper.extract_relationships(sample_entities, ['officer', 'ownership'])
    ...
    def test_confidence_filtering(self, relationship_mapper, sample_entities):
        ...
        high_conf_relationships = relationship_mapper.extract_relationships(sample_entities, ['officer', 'ownership'])
        
        # Set low confidence threshold
        relationship_mapper.config['confidence_threshold'] = 0.1
        
        low_conf_relationships = relationship_mapper.extract_relationships(sample_entities, ['officer', 'ownership'])
    ...
    def test_edge_cases(self, relationship_mapper):
        ...
        relationships = relationship_mapper.extract_relationships(single_entity, ['officer'])
        ...
        relationships = relationship_mapper.extract_relationships(incomplete_entities, ['officer', 'ownership'])
    ...
    def test_performance_with_large_dataset(self, relationship_mapper):
        ...
        relationships = relationship_mapper.extract_relationships(large_entities, ['officer', 'ownership'])
        end_time = time.time()
    ...
    @pytest.mark.asyncio
    async def test_async_relationship_extraction(self, relationship_mapper, sample_entities):
    ...
    def test_custom_relationship_types(self, relationship_mapper, sample_entities):
        ...
        officer_only = relationship_mapper.extract_relationships(sample_entities, ['officer'])
        ownership_only = relationship_mapper.extract_relationships(sample_entities, ['ownership'])
        combined = relationship_mapper.extract_relationships(sample_entities, ['officer', 'ownership'])
