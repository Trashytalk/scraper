...
class TestAdvancedEntityResolver:
    ...
    def test_normalize_address(self, entity_resolver):
        ...
        test_cases = [
            ...
            ('1 Apple Park Way, Cupertino, CA 95014', '1 apple park way cupertino ca 95014'),
    ...
    def test_calculate_name_similarity(self, entity_resolver):
        ...
        similarity = entity_resolver._calculate_name_similarity('Apple Inc.', 'Apple Incorporated')
        assert similarity > 0.8
        
        # Medium similarity - similar but different companies
        similarity = entity_resolver._calculate_name_similarity('Apple Inc.', 'Apple Computer')
        assert 0.6 < similarity < 0.9
        
        # Low similarity - different companies
        similarity = entity_resolver._calculate_name_similarity('Apple Inc.', 'Microsoft Corporation')
    ...
    def test_apply_blocking_strategy(self, entity_resolver, sample_entities):
        ...
        if len(apple_entities) > 1:
            ...
            for block_entities in blocks.values():
                if any(e['entity_id'] in [ae['entity_id'] for ae in apple_entities] for e in block_entities):
                    apple_blocks.append(block_entities)
            
            # At least one block should contain multiple Apple entities
            assert any(len([e for e in block if 'apple' in e['name'].lower()]) > 1 for block in apple_blocks)
            ...
            if len(entities) > 1:
                ...
                for i in range(len(entities)):
                    for j in range(i + 1, len(entities)):
                        similarity = entity_resolver._calculate_multi_field_similarity(entities[i], entities[j])
                        assert similarity.overall_score >= entity_resolver.config['similarity_threshold']
    ...
    def test_phonetic_matching(self, entity_resolver):
        ...
        similarity2 = entity_resolver._calculate_phonetic_similarity('Johnson', 'Jonson')
    ...
    def test_performance_with_large_dataset(self, entity_resolver):
        ...
        for i in range(10):
            ...
            duplicate['name'] = duplicate['name'].replace('Company', 'Corp')  # Slight variation
    ...
    async def test_async_resolution(self, entity_resolver, sample_entities):
        ...
        clusters = await asyncio.to_thread(entity_resolver.resolve_entities, sample_entities)
