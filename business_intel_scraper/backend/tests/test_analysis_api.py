...
from business_intel_scraper.backend.analysis.orchestrator import AnalysisRequest, AnalysisResult
...
def mock_analysis_result():
    ...
    return AnalysisResult(
        ...
        entity_resolutions=[{
            ...
            'entities': [{'entity_id': 'test_entity_1', 'name': 'Test Company Inc.'}],
            'canonical_entity': {'entity_id': 'test_entity_1', 'name': 'Test Company Inc.'},
...
class TestAnalysisAPI:
    ...
    def test_comprehensive_analysis_success(self, mock_get_user, mock_get_orchestrator, 
                                          sample_analysis_request, mock_analysis_result):
        ...
        mock_orchestrator.run_comprehensive_analysis = AsyncMock(return_value=mock_analysis_result)
        mock_get_orchestrator.return_value = mock_orchestrator
        
        # Make request
        response = client.post("/api/v1/analysis/comprehensive", json=sample_analysis_request)
    ...
    @patch('business_intel_scraper.backend.api.analysis.get_analysis_orchestrator')
    @patch('business_intel_scraper.backend.api.analysis.get_current_user')
    def test_comprehensive_analysis_validation_error(self, mock_get_user, mock_get_orchestrator):
    ...
    @patch('business_intel_scraper.backend.api.analysis.get_analysis_orchestrator')
    @patch('business_intel_scraper.backend.api.analysis.get_current_user')
    def test_entity_resolution_endpoint(self, mock_get_user, mock_get_orchestrator, sample_entity_input):
    ...
    @patch('business_intel_scraper.backend.api.analysis.get_analysis_orchestrator')
    @patch('business_intel_scraper.backend.api.analysis.get_current_user')
    def test_relationship_mapping_endpoint(self, mock_get_user, mock_get_orchestrator, sample_entity_input):
        ...
        # Mock orchestrator
        mock_orchestrator = Mock()
        mock_orchestrator.relationship_mapper.extract_relationships.return_value = [mock_relationship]
        mock_get_orchestrator.return_value = mock_orchestrator
        ...
        response = client.post("/api/v1/analysis/relationship-mapping", json=request_data)
    ...
    @patch('business_intel_scraper.backend.api.analysis.get_analysis_orchestrator')
    @patch('business_intel_scraper.backend.api.analysis.get_current_user')
    def test_enrichment_endpoint(self, mock_get_user, mock_get_orchestrator, sample_entity_input):
        ...
        # Mock orchestrator
        mock_orchestrator = Mock()
        mock_orchestrator.enrichment_engine.enrich_entities = AsyncMock(return_value=[mock_enrichment])
        mock_get_orchestrator.return_value = mock_orchestrator
    ...
    @patch('business_intel_scraper.backend.api.analysis.get_analysis_orchestrator')
    @patch('business_intel_scraper.backend.api.analysis.get_current_user')
    def test_enrichment_cost_limit_exceeded(self, mock_get_user, mock_get_orchestrator, sample_entity_input):
    ...
    def test_event_detection_endpoint(self, mock_get_user, mock_get_orchestrator):
        ...
        # Mock orchestrator
        mock_orchestrator = Mock()
        mock_orchestrator.event_detector.detect_events = AsyncMock(return_value=[mock_event])
        mock_get_orchestrator.return_value = mock_orchestrator
    ...
    def test_analysis_status_endpoint(self, mock_get_user, mock_get_orchestrator):
        ...
        mock_orchestrator = Mock()
        mock_orchestrator.get_analysis_status = AsyncMock(return_value={
            'status': 'completed',
    ...
    def test_analysis_status_not_found(self, mock_get_user, mock_get_orchestrator):
        ...
        mock_orchestrator = Mock()
        mock_orchestrator.get_analysis_status = AsyncMock(return_value={'status': 'not_found'})
        mock_get_orchestrator.return_value = mock_orchestrator
    ...
    def test_export_results_json(self, mock_get_user, mock_get_orchestrator):
        ...
        mock_orchestrator = Mock()
        mock_orchestrator.export_analysis_results = AsyncMock(return_value='{"test": "data"}')
        mock_get_orchestrator.return_value = mock_orchestrator
    ...
    def test_export_results_summary(self, mock_get_user, mock_get_orchestrator):
        ...
        mock_orchestrator = Mock()
        mock_orchestrator.export_analysis_results = AsyncMock(return_value='Analysis Summary Report')
        mock_get_orchestrator.return_value = mock_orchestrator
    ...
    def test_get_metrics_endpoint(self, mock_get_user, mock_get_orchestrator):
        ...
        mock_orchestrator = Mock()
        mock_orchestrator.get_orchestrator_metrics.return_value = {'total_requests': 100}
        mock_orchestrator.entity_resolver.get_resolution_metrics.return_value = {'total_entities_processed': 500}
        mock_orchestrator.enrichment_engine.get_enrichment_metrics.return_value = {'total_requests': 200}
        mock_orchestrator.event_detector.get_detection_metrics.return_value = {'events_detected': 50}
        mock_get_orchestrator.return_value = mock_orchestrator
    ...
    @patch('business_intel_scraper.backend.api.analysis.get_analysis_orchestrator')
    @patch('business_intel_scraper.backend.api.analysis.get_current_user')
    def test_error_handling(self, mock_get_user, mock_get_orchestrator, sample_analysis_request):
        """Test error handling in API endpoints"""
        mock_get_user.return_value = {'user_id': 'test_user'}
        
        # Mock orchestrator to raise exception
        mock_orchestrator = Mock()
        mock_orchestrator.run_comprehensive_analysis = AsyncMock(side_effect=Exception("Analysis failed"))
        mock_get_orchestrator.return_value = mock_orchestrator
    ...
    @pytest.mark.asyncio
    async def test_async_endpoint_behavior(self, sample_analysis_request, mock_analysis_result):
        """Test asynchronous behavior of endpoints"""
        with patch('business_intel_scraper.backend.api.analysis.get_current_user') as mock_get_user:
            with patch('business_intel_scraper.backend.api.analysis.get_analysis_orchestrator') as mock_get_orchestrator:
                mock_get_user.return_value = {'user_id': 'test_user'}
