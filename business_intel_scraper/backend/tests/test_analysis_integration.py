...
def integration_config() -> Dict[str, Any]:
    ...
    return {
        ...
        'event_detection': {
            ...
            'enabled_patterns': ['ownership_acquisition', 'regulatory_filing', 'executive_change']
        }
    }
...
class TestAnalysisIntegration:
    ...
    def test_full_analysis_pipeline(self, comprehensive_test_data, integration_config):
        ...
        request = AnalysisRequest(
            ...
            analysis_types=['entity_resolution', 'relationship_mapping', 'enrichment', 'event_detection'],
        ...
        techcorp_clusters = [r for r in result.entity_resolutions 
        ...
        officer_rels = [r for r in result.relationships if r['relationship_type'] == 'officer']
        assert len(officer_rels) > 0
        
        # Should find ownership relationships
        ownership_rels = [r for r in result.relationships if r['relationship_type'] == 'ownership']
        assert len(ownership_rels) > 0
        
        # Check summary
        assert result.summary['entities_analyzed'] == len(comprehensive_test_data['entities'])
        assert result.summary['total_findings'] > 0
        assert 'risk_assessment' in result.summary
        
        # Check metrics
        assert result.metrics['duration_seconds'] > 0
        assert result.metrics['entities_processed'] == len(comprehensive_test_data['entities'])

    @patch('business_intel_scraper.backend.analysis.enrichment_engine.aiohttp.ClientSession')
    def test_enrichment_integration(self, mock_session, comprehensive_test_data, integration_config):
        ...
        # Run enrichment
        enrichments = asyncio.run(enrichment_engine.enrich_entities(
    ...
    def test_event_detection_integration(self, comprehensive_test_data, integration_config):
        """Test event detection integration"""
        event_detector = BusinessEventDetector(integration_config['event_detection'])
        
        # Run event detection
        events = asyncio.run(event_detector.detect_events(
        ...
        filing_events = [e for e in events if 'filing' in e.title.lower() or 'sec' in e.title.lower()]
    ...
    def test_component_integration(self, comprehensive_test_data, integration_config):
        ...
        entity_resolver = AdvancedEntityResolver(integration_config['entity_resolution'])
        relationship_mapper = EntityRelationshipMapper(integration_config['relationship_mapping'])
        
        # Step 1: Resolve entities
        entity_clusters = entity_resolver.resolve_entities(comprehensive_test_data['entities'])
    ...
    def test_performance_with_large_dataset(self, integration_config):
        ...
        # Quality assertions
        assert len(result.entity_resolutions) > 0
        assert len(result.entity_resolutions) < len(large_entities)  # Should detect duplicates
    ...
    def test_caching_integration(self, comprehensive_test_data, integration_config):
        ...
        enrichments1 = asyncio.run(enrichment_engine.enrich_entities(entities_subset, ['sanctions']))
        ...
        enrichments2 = asyncio.run(enrichment_engine.enrich_entities(entities_subset, ['sanctions']))
    ...
    def test_concurrent_analysis_requests(self, comprehensive_test_data, integration_config):
        ...
        for i in range(3):
            request = AnalysisRequest(
                ...
                entities=comprehensive_test_data['entities'][:3],  # Smaller subset for speed
    ...
    def test_export_functionality(self, comprehensive_test_data, integration_config):
        ...
        # Test JSON export
        json_export = asyncio.run(orchestrator.export_analysis_results('export_test_001', 'json'))
        ...
        # Test summary export
        summary_export = asyncio.run(orchestrator.export_analysis_results('export_test_001', 'summary'))
    ...
    def test_cli_integration(self, comprehensive_test_data):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Import CLI module
            from business_intel_scraper.backend.cli.analysis import load_entities_from_file
            
            # Test file loading
            loaded_entities = load_entities_from_file(str(input_file))
            assert len(loaded_entities) == len(comprehensive_test_data['entities'])
            assert loaded_entities[0]['entity_id'] == comprehensive_test_data['entities'][0]['entity_id']
    ...
    def test_metrics_aggregation(self, comprehensive_test_data, integration_config):
        ...
        if entity_resolver_metrics['total_entities_processed'] > 0:
            assert entity_resolver_metrics['total_entities_processed'] == len(comprehensive_test_data['entities'])
    ...
    def test_configuration_flexibility(self, comprehensive_test_data):
        ...
        high_result = asyncio.run(high_precision_orchestrator.run_comprehensive_analysis(high_request))
        ...
        low_result = asyncio.run(low_precision_orchestrator.run_comprehensive_analysis(low_request))
