...
def run_tests(test_type="all", verbose=False, coverage=False):
    ...
    if coverage:
        cmd.extend(["--cov=business_intel_scraper", "--cov-report=html", "--cov-report=term"])
...
def main():
    """Main test runner."""
    parser = argparse.ArgumentParser(description="Run Business Intelligence Scraper tests")
    parser.add_argument(
        "test_type", 
        choices=["all", "unit", "integration", "performance", "phase1", "phase2", "load"],
        default="all",
        nargs="?",
