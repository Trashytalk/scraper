...
class TestPhase1Performance:
    ...
    async def test_discovery_speed_benchmark(self):
        ...
        with patch("aiohttp.ClientSession.get") as mock_get:
            ...
            assert (
                len([r for r in results if not isinstance(r, Exception)]) >= 40
...
class TestPhase2Performance:
    ...
    async def test_dom_change_detection_speed(self, large_html_document):
        ...
        print(
            f"Snapshot time: {snapshot_time:.2f}s, Detection time: {detection_time:.2f}s"
        )
    ...
    async def test_concurrent_change_detection(self):
        ...
        # Modified content
        modified_html = (
            "<html><body><div class='new-content'>Modified content</div></body></html>"
        )
    ...
    def test_memory_usage_with_many_snapshots(self):
        ...
        for i in range(500):  # 500 snapshots = ~5MB of HTML
            asyncio.run(
                dom_detector.store_dom_snapshot(f"https://example{i}.com", large_html)
            )
...
class TestSystemLoadTesting:
    ...
    async def test_high_load_discovery_and_monitoring(self):
        ...
        with patch("aiohttp.ClientSession.get") as mock_get:
            ...
            success_rate = len(
                [r for r in discovery_results if not isinstance(r, Exception)]
            ) / len(discovery_results)
            assert success_rate > 0.8  # At least 80% success rate under load

            print(
                f"High load test completed in {elapsed:.2f}s with {success_rate:.2%} success rate"
            )
