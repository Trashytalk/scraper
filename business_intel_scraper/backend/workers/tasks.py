...
def validate_discovered_sources(self) -> str:
    ...
    try:
        ...
        candidate_sources = manager.get_discovered_sources(
            status="candidate", min_confidence=0.5
        )
...
def check_dom_changes(self) -> str:
    ...
    try:
        ...
        async def check_sources_for_changes():
            ...
            sources = discovery_manager.get_discovered_sources(
                status="validated", min_confidence=0.7
            )
                    ...
                    try:
                        async with session.get(source.url, timeout=30) as response:
                            if response.status == 200:
                                ...
                                changes = await dom_detector.check_for_changes(
                                    source.url, html_content
                                )
                                changes_detected.extend(changes)
                            else:
                                logger.warning(
                                    f"Failed to fetch {source.url}: HTTP {response.status}"
                                )
                    except Exception as e:
        ...
        result_msg = (
            f"DOM change detection completed. Found {len(changes)} total changes"
        )
...
def update_spider_logic(self) -> str:
    ...
    try:
        ...
        if update_results["status"] == "completed":
            ...
            return (
                f"Spider update completed: {results['spiders_updated']} spiders updated, "
    ...
    try:
        ...
        report_data = {
            ...
            "summary": {
                ...
                "critical_changes": len(
                    [c for c in dom_detector.changes if c.severity == "critical"]
                ),
                "high_changes": len(
                    [c for c in dom_detector.changes if c.severity == "high"]
                ),
                "medium_changes": len(
                    [c for c in dom_detector.changes if c.severity == "medium"]
                ),
                "low_changes": len(
                    [c for c in dom_detector.changes if c.severity == "low"]
                ),
            },
            "changes_by_type": {},
            "changes_by_url": {},
        ...
        for change in recent_changes:
            ...
            report_data["changes_by_type"][change_type] = (
                report_data["changes_by_type"].get(change_type, 0) + 1
            )

            # Count by URL
            url = change.url
            report_data["changes_by_url"][url] = (
                report_data["changes_by_url"].get(url, 0) + 1
            )
        ...
        report_filename = (
            f"dom_changes_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.json"
        )
        ...
        return (
            f"DOM change report generated: {report_file.name}. "
...
def analyze_content_with_ml(
    self, url: str, html_content: str, context: Dict[str, Any] = None
) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            TASK_DURATION.labels(task="ml_content_analysis").observe(
...
def assess_data_quality(
    self, data: Union[List[Dict[str, Any]], str], source_url: str = ""
) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            log_job_finish(task_id)
            TASK_DURATION.labels(task="data_quality_assessment").observe(
                time.perf_counter() - start
            )
...
def learn_from_spider_execution(self, session_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            log_job_finish(task_id)
            TASK_DURATION.labels(task="pattern_learning").observe(
                time.perf_counter() - start
            )
...
@celery_app.task(bind=True)  # type: ignore[misc]
def discover_predictive_sources(
    self, seed_url: str, max_sources: int = 10
) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            content_features, discovered_sources = loop.run_until_complete(
                _discover_async()
            )
            ...
            log_job_finish(task_id)
            TASK_DURATION.labels(task="predictive_discovery").observe(
                time.perf_counter() - start
            )
...
def optimize_extraction_strategy(
    self, url: str, selectors: List[str], context: Dict[str, Any] = None
) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            content_features, strategy_recommendation, optimized_selectors = (
                loop.run_until_complete(_optimize_async())
            )
            ...
            result = {
                ...
                "content_analysis": (
                    content_features.to_dict() if content_features else None
                ),
                "processing_time": time.perf_counter() - start,
            }

            log_job_finish(task_id)
            TASK_DURATION.labels(task="extraction_optimization").observe(
                time.perf_counter() - start
            )
    ...
    try:
        ...
        try:
            ...
            result = {
                ...
                "patterns_change": stats_after["total_patterns"]
                - stats_before["total_patterns"],
                "processing_time": time.perf_counter() - start,
            }

            log_job_finish(task_id)
            TASK_DURATION.labels(task="adaptive_learning").observe(
                time.perf_counter() - start
            )
...
def generate_ml_insights_report(self, days: int = 30) -> Dict[str, Any]:
    ...
    try:
        ...
        if content_analyzer:
            ...
            if avg_quality > 0.8:
                insights["content_analysis"]["recommendations"].append(
                    "Content quality is excellent"
                )
            elif avg_quality > 0.6:
                insights["content_analysis"]["recommendations"].append(
                    "Content quality is good but has room for improvement"
                )
            else:
                insights["content_analysis"]["recommendations"].append(
                    "Content quality needs significant improvement"
                )
        ...
        if "error" not in quality_trends:
            overall_trend = quality_trends.get("overall_quality", {}).get(
                "trend", "stable"
            )
            if overall_trend == "improving":
                insights["quality_assessment"]["recommendations"].append(
                    "Data quality is improving over time"
                )
            elif overall_trend == "declining":
                insights["quality_assessment"]["recommendations"].append(
                    "Data quality is declining - investigate causes"
                )
            else:
                insights["quality_assessment"]["recommendations"].append(
        ...
        log_job_finish(task_id)
        TASK_DURATION.labels(task="ml_insights_report").observe(
            time.perf_counter() - start
        )
...
def queue_data_quality_assessment(
    ...
    *,
    ...
    result = assess_data_quality.apply_async(args=[data, source_url], **options)
    return str(result.id)
...
def queue_pattern_learning(
    ...
    *,
    ...
    result = learn_from_spider_execution.apply_async(args=[session_data], **options)
    return str(result.id)
