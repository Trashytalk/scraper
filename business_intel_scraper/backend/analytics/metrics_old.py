...
class MetricsCollector:
    ...
    async def record_scraping_job(
        ...
        self.job_statistics[job_id].update(
            {"status": status, "timestamp": datetime.now(), "metadata": metadata or {}}
        )

        # Count active jobs
        active_count = sum(
            1 for job in self.job_statistics.values() if job.get("status") == "running"
        )
    ...
    async def record_data_quality(
        ...
        await analytics_engine.record_metric(
            "data.quality_score", score, metadata=metadata or {}
        )
    ...
    def get_realtime_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "active_jobs": len(
                [
                    job
                    for job in self.job_statistics.values()
    ...
    def get_endpoint_stats(self) -> Dict[str, Dict[str, Any]]:
        ...
        for endpoint in set(
            ...
            error_rate = (
                self.error_counts[endpoint] / total_requests
                if total_requests > 0
                else 0
            )
            success_rate = (
                self.success_counts[endpoint] / total_requests
                if total_requests > 0
                else 0
            )
    ...
    async def get_historical_trends(
        ...
        for metric_name in metrics_to_fetch:
            historical_data = await analytics_engine.get_historical_data(
                metric_name, hours
            )
