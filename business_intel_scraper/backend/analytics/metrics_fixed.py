...
class MetricsCollector:
    ...
    def start_collection(self):
        ...
        if not self._running:
            ...
            try:
                ...
                logger.warning("No event loop available for background metrics collection")
    ...
    async def record_request(self, method: str, endpoint: str, 
        ...
        try:
            await analytics_engine.record_metric(f"request.{endpoint}.duration", duration)
            await analytics_engine.record_metric(f"request.{endpoint}.count", 1)
        except Exception:
            pass  # Ignore analytics engine errors
    
    async def record_job_metrics(self, job_type: str, job_id: str, 
        ...
        # Count active jobs
        active_count = sum(1 for stats in self.job_statistics.values() 
        ...
        try:
            ...
            if duration:
                await analytics_engine.record_metric(f"job.{job_type}.duration", duration)
            if items_processed:
                await analytics_engine.record_metric(f"job.{job_type}.items", items_processed)
        except Exception:
            pass  # Ignore analytics engine errors
    ...
    def get_metrics_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            'jobs': {
                ...
                'active': sum(1 for stats in self.job_statistics.values() if stats['status'] == 'running'),
                'completed': sum(1 for stats in self.job_statistics.values() if stats['status'] == 'completed'),
                'failed': sum(1 for stats in self.job_statistics.values() if stats['status'] == 'failed')
            },
            'system': {}
        }
