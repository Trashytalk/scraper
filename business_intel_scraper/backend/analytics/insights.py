...
class InsightsGenerator:
    ...
    async def generate_quality_insights(
        ...
        # Duplicate analysis
        duplicate_rate = (
            current_metrics.duplicate_records / current_metrics.total_records
            if current_metrics.total_records > 0
            else 0
        )
    ...
    async def detect_anomalies(self, hours: int = 24) -> List[Dict[str, Any]]:
        ...
        # Get historical data
        response_time_data = await analytics_engine.get_historical_data(
            "api.request", hours
        )
        ...
        if stdev_time > 0:
            for i, time_point in enumerate(
                response_time_data[-10:]
    ...
    def _calculate_health_score(
        self, performance: PerformanceMetrics, analytics_summary: Dict[str, Any]
    ) -> float:
    ...
    def _prioritize_recommendations(
        ...
        sorted_insights = sorted(
            ...
            key=lambda x: (
                ...
                ),  # Longer recommendations assumed more important
            ),
            reverse=True,
        )
