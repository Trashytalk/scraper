...
class DashboardAnalytics:
    ...
    def __init__(self) -> None:
        ...
        self.use_performance_cache = (
            PERFORMANCE_OPTIMIZATION_AVAILABLE and analytics_performance
        )
    ...
    def _is_cache_valid(self, key: str) -> bool:
        ...
        return (
            datetime.now() - self._cache_timestamps[key]
    ...
    async def _get_from_optimized_cache(self, key: str) -> Optional[Any]:
        ...
        if (
            self.use_performance_cache
            and analytics_performance
            and hasattr(analytics_performance, "optimizer")
        ):
    ...
    async def _set_optimized_cache(self, key: str, value: Any, ttl: int = 30) -> None:
        ...
        if (
            ...
            await analytics_performance.optimizer.cache.set(
                f"dashboard:{key}", value, ttl
            )
    ...
    async def get_dashboard_overview(self) -> Dict[str, Any]:
        ...
        overview = {
            ...
            "key_metrics": {
                "requests_per_minute": (
                    performance.throughput * 60 if performance.throughput > 0 else 0
                ),
                "avg_response_time": performance.avg_response_time,
                "success_rate": performance.success_rate,
                "error_rate": performance.error_rate,
                "active_jobs": realtime_metrics.get("active_jobs", 0),
                "data_quality_score": analytics_summary.get("data_quality", {}).get(
                    "quality_score", 0.0
                ),
            },
            "system_health": {
                "cpu_percent": realtime_metrics.get("system", {}).get("cpu_percent", 0),
                "memory_percent": realtime_metrics.get("system", {}).get(
                    "memory_percent", 0
                ),
                "disk_percent": realtime_metrics.get("system", {}).get(
                    "disk_percent", 0
                ),
            },
    ...
    async def get_performance_charts(
        ...
        try:
            response_times = await analytics_engine.get_historical_data(
                "api.request", hours
            )
            cpu_data = await analytics_engine.get_historical_data(
                "system.cpu_percent", hours
            )
            memory_data = await analytics_engine.get_historical_data(
                "system.memory_percent", hours
            )
        ...
        charts = {
            "response_times": self._prepare_time_series(
                trends.get("api.request", []), "Response Time (seconds)"
            ),
            "system_cpu": self._prepare_time_series(
                trends.get("system.cpu_percent", []), "CPU Usage (%)"
            ),
            "system_memory": self._prepare_time_series(
                trends.get("system.memory_percent", []), "Memory Usage (%)"
            ),
            "data_quality": self._prepare_time_series(
                trends.get("data.quality_score", []), "Data Quality Score"
            ),
        }
    ...
    def _prepare_time_series(
        self, data: List[Dict[str, Any]], label: str
    ) -> List[Dict[str, Any]]:
    ...
    def _categorize_alerts_by_type(
    ...
    async def get_data_quality_dashboard(self) -> Dict[str, Any]:
        ...
        quality_trends = await analytics_engine.get_historical_data(
            "data.quality_score", 24
        )
    ...
    def _calculate_quality_trend(self, quality_trends: List[Dict[str, Any]]) -> str:
        ...
        earlier_scores = (
            [point["value"] for point in quality_trends[-10:-5]]
            if len(quality_trends) >= 10
            else []
        )
    ...
    def _determine_system_status(
        self, performance: Any, analytics_summary: Dict[str, Any]
    ) -> str:
