        ...
        if HAS_SPACY and self.language.spacy_model:
            try:
                ...
                try:
                    ...
                    logger.warning(
                        f"Could not initialize spaCy model for {self.language.name}"
                    )
        ...
        if HAS_MECAB:
            try:
                self.mecab = MeCab.Tagger(
                    "-Owakati"
                )  # Wakati-gaki (word boundary) mode
    ...
    def _initialize_stanza(self):
        ...
        if HAS_STANZA and self.language.stanza_model:
            try:
                ...
                logger.info(
                    f"Arabic tokenizer (Stanza) initialized for {self.language.name}"
                )
            except Exception as e:
                logger.warning(
                    f"Failed to initialize Stanza for {self.language.name}: {e}"
                )
    ...
    def _initialize_stanza(self):
        ...
        if HAS_STANZA and self.language.stanza_model:
            try:
                ...
                logger.info(
                    f"Universal tokenizer (Stanza) initialized for {self.language.name}"
                )
            except Exception as e:
                logger.warning(
                    f"Failed to initialize Stanza for {self.language.name}: {e}"
    ...
    def get_tokenizer(self, language: LanguageInfo) -> BaseTokenizer:
        ...
        if cache_key not in self.tokenizer_cache:
            tokenizer_class = self.tokenizer_classes.get(
                language.script, UniversalTokenizer
            )
