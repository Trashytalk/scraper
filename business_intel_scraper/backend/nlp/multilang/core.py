...
class DetectedText:
    ...
    script: ScriptType
    confidence: float
    segments: List[Tuple[str, LanguageInfo]] = field(
...
class MultiLanguageDetector:
        ...
        if HAS_FASTTEXT:
            try:
                ...
                if model_path.exists():
                    self.language_models["fasttext"] = fasttext.load_model(
                        str(model_path)
                    )
    ...
    def detect_script(self, text: str) -> Tuple[ScriptType, float]:
        ...
        dominant_script = next(
            script for script, count in script_counts.items() if count == max_count
        )
    ...
    def detect_language(self, text: str) -> List[LanguageInfo]:
        ...
        if HAS_FASTTEXT and "fasttext" in self.language_models:
            try:
                predictions = self.language_models["fasttext"].predict(
                    text.replace("\n", " "), k=3
                )
                for i, (label, confidence) in enumerate(
                    ...
                    if lang_code in self.language_data and confidence > 0.1:
                        ...
                        existing = next(
                            (l for l in detected_languages if l.code == lang_code), None
                        )
                        if existing:
            ...
            try:
                ...
                for language in detector.languages:
                    ...
                    if lang_code in self.language_data:
                        ...
                        if existing:
                            existing.confidence = max(
                                existing.confidence, language.confidence
                            )
                        else:
    ...
    def detect_mixed_language_segments(
        self, text: str
    ) -> List[Tuple[str, LanguageInfo]]:
    ...
    def create_detected_text(self, text: str) -> DetectedText:
        ...
        primary_language = (
            detected_languages[0] if detected_languages else self.language_data["en"]
        )

        # Detect mixed segments if confidence is low or script is mixed
        segments = []
        if script == ScriptType.MIXED or (
            detected_languages and detected_languages[0].confidence < 0.7
        ):
