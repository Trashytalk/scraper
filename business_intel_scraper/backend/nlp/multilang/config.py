...
MULTILANG_CONFIG = {
    ...
    'language_detection': {
        'confidence_threshold': float(os.getenv('MULTILANG_CONFIDENCE_THRESHOLD', '0.7')),
        'fallback_language': os.getenv('MULTILANG_FALLBACK_LANGUAGE', 'en'),
        'enable_fasttext': os.getenv('MULTILANG_ENABLE_FASTTEXT', 'false').lower() == 'true',
        'enable_polyglot': os.getenv('MULTILANG_ENABLE_POLYGLOT', 'false').lower() == 'true'
    },
    
    # Script Detection Settings  
    'script_detection': {
        'min_script_chars': int(os.getenv('MULTILANG_MIN_SCRIPT_CHARS', '3')),
        'mixed_script_threshold': float(os.getenv('MULTILANG_MIXED_SCRIPT_THRESHOLD', '0.3'))
    },
    
    # Tokenization Settings
    'tokenization': {
        'max_token_length': int(os.getenv('MULTILANG_MAX_TOKEN_LENGTH', '100')),
        'preserve_whitespace': os.getenv('MULTILANG_PRESERVE_WHITESPACE', 'false').lower() == 'true',
        'chinese_mode': os.getenv('MULTILANG_CHINESE_MODE', 'accurate'),  # 'accurate' or 'fast'
        'japanese_mode': os.getenv('MULTILANG_JAPANESE_MODE', 'wakati')   # 'wakati' or 'detail'
    },
    
    # NER Settings
    'ner': {
        'confidence_threshold': float(os.getenv('MULTILANG_NER_CONFIDENCE', '0.6')),
        'enable_business_patterns': os.getenv('MULTILANG_ENABLE_BUSINESS_PATTERNS', 'true').lower() == 'true',
        ...
        'transformers_model': os.getenv('MULTILANG_TRANSFORMERS_MODEL', 'bert-base-multilingual-cased')
    },
    
    # Translation Settings
    'translation': {
        'target_language': os.getenv('MULTILANG_TARGET_LANGUAGE', 'en'),
        'prefer_offline': os.getenv('MULTILANG_PREFER_OFFLINE', 'false').lower() == 'true',
    ...
    'transliteration': {
        ...
        'fallback_to_unidecode': os.getenv('MULTILANG_FALLBACK_UNIDECODE', 'true').lower() == 'true'
    },
    ...
    'normalization': {
        ...
        'financial_default_currency': os.getenv('MULTILANG_FINANCIAL_DEFAULT_CURRENCY', 'USD'),
        'date_ambiguous_format': os.getenv('MULTILANG_DATE_AMBIGUOUS_FORMAT', 'mdy')  # 'mdy' or 'dmy'
    },
