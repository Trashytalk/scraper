...
from .core import LanguageInfo, ScriptType, language_detector
...
class ScriptTransliterator:
    ...
    def _initialize_icu(self):
        ...
        try:
            ...
            for trans_id in transliterator_ids:
                try:
                    self.icu_transliterators[trans_id] = Transliterator.createInstance(
                        trans_id
                    )
    ...
    def transliterate_cyrillic(self, text: str) -> TransliterationResult:
        ...
        if not text:
            return TransliterationResult(
                "", "", ScriptType.CYRILLIC, ScriptType.LATIN, "none", 0.0
            )

        # Method 1: ICU transliterator
        if "Cyrillic-Latin" in self.icu_transliterators:
            try:
                transliterated = self.icu_transliterators[
                    "Cyrillic-Latin"
                ].transliterate(text)
                return TransliterationResult(
                    text,
    ...
    def transliterate_arabic(self, text: str) -> TransliterationResult:
        ...
        if not text:
            return TransliterationResult(
                "", "", ScriptType.ARABIC, ScriptType.LATIN, "none", 0.0
            )

        # Method 1: ICU transliterator
        if "Arabic-Latin" in self.icu_transliterators:
            try:
                transliterated = self.icu_transliterators["Arabic-Latin"].transliterate(
                    text
                )
                return TransliterationResult(
                    text,
    ...
    def transliterate_cjk(self, text: str) -> TransliterationResult:
        ...
        if not text:
            return TransliterationResult(
                "", "", ScriptType.CJK, ScriptType.LATIN, "none", 0.0
            )

        # Method 1: ICU transliterator for Chinese
        if "Han-Latin" in self.icu_transliterators:
            try:
                transliterated = self.icu_transliterators["Han-Latin"].transliterate(
                    text
                )
                return TransliterationResult(
                    text,
        ...
        if HAS_UNIDECODE:
            ...
            return TransliterationResult(
                text, transliterated, ScriptType.CJK, ScriptType.LATIN, "unidecode", 0.6
            )
    ...
    def transliterate_universal(
        self, text: str, script: ScriptType
    ) -> TransliterationResult:
        """Universal transliteration using unidecode"""
        if not text or not HAS_UNIDECODE:
            return TransliterationResult(
                text, text, script, ScriptType.LATIN, "none", 0.0
            )

        try:
            transliterated = unidecode(text)
            return TransliterationResult(
                text, transliterated, script, ScriptType.LATIN, "unidecode", 0.5
            )
        except Exception as e:
            logger.error(f"Unidecode transliteration failed: {e}")
            return TransliterationResult(
                text, text, script, ScriptType.LATIN, "failed", 0.0
            )

    def transliterate(
        self, text: str, script: Optional[ScriptType] = None
    ) -> TransliterationResult:
        """Main transliteration method with automatic script detection"""
        if not text:
            return TransliterationResult(
                "", "", ScriptType.UNKNOWN, ScriptType.LATIN, "none", 0.0
            )
        ...
        if script == ScriptType.CYRILLIC:
            ...
            return TransliterationResult(
                text, text, script, ScriptType.LATIN, "none", 1.0
            )
...
class MultiLanguageTranslator:
    ...
    def translate_google(
        ...
        try:
            ...
            return TranslationResult(
                text,
                translated,
                language_detector.language_data.get(
                    source_lang, language_detector.language_data["en"]
                ),
                language_detector.language_data.get(
                    target_lang, language_detector.language_data["en"]
                ),
                "google",
                0.9,
            )
    ...
    def translate_marian(
        ...
        if model_key not in self.marian_models:
            raise ValueError(
                f"Marian model not available for {source_lang} -> {target_lang}"
            )
    ...
    def translate(
        ...
        logger.error(
            f"All translation methods failed for {source_lang} -> {target_lang}: {errors}"
        )
        return TranslationResult(
            text,
            text,
...
class EntityNormalizer:
    ...
    def __init__(self):
        ...
        self.phonetic_algorithms = (
            ["soundex", "metaphone", "nysiis"] if HAS_PHONETICS else []
        )

    def normalize_company_name(
        self, name: str, language: LanguageInfo
    ) -> Dict[str, str]:
        ...
        if language.script != ScriptType.LATIN:
            transliteration_result = self.transliterator.transliterate(
            ...
            normalized["transliterated_cleaned"] = self._clean_company_name(
                transliteration_result.transliterated,
                language_detector.language_data["en"],
            )
    ...
    def calculate_similarity(
        self, normalized1: Dict[str, str], normalized2: Dict[str, str]
    ) -> float:
        ...
        for algorithm in self.phonetic_algorithms:
            ...
            if key in normalized1 and key in normalized2:
                if normalized1[key] == normalized2[key]:
                    max_similarity = max(
                        max_similarity, 0.8
                    )  # High similarity for phonetic match
