...
from .tokenization import multilang_tokenizer
...
class SpacyNERExtractor(BaseNERExtractor):
    ...
    def __init__(self, language: LanguageInfo):
        ...
        self.nlp_model = None
        self.business_labels = {'ORG', 'PERSON', 'GPE', 'MONEY', 'PERCENT', 'DATE', 'FAC', 'PRODUCT'}
    ...
    def _initialize_model(self):
        ...
        if self.language.spacy_model:
            try:
                ...
                logger.warning(f"Failed to load spaCy model {self.language.spacy_model}: {e}")
                try:
...
class TransformersNERExtractor(BaseNERExtractor):
    ...
    def _initialize_pipeline(self):
        ...
        try:
            ...
            logger.info(f"Initialized Transformers NER for {self.language.name} using {self.model_name}")
        except Exception as e:
            logger.warning(f"Failed to initialize Transformers NER for {self.language.name}: {e}")
    ...
    def extract_entities(self, text: str) -> List[MultiLangEntity]:
        ...
        try:
            ...
            logger.error(f"Transformers NER extraction failed for {self.language.name}: {e}")
...
class BusinessEntityExtractor:
    ...
    def _initialize_business_patterns(self) -> Dict[str, Dict[str, Any]]:
        ...
        patterns['universal'] = {
            'company_suffix': {
                'en': re.compile(r'\b(?:Inc|Corp|LLC|Ltd|Limited|Company|Co|Group|Holdings|International|Intl|Industries|Solutions|Services|Technologies|Systems|Enterprises|Associates)\b\.?', re.IGNORECASE),
                'es': re.compile(r'\b(?:S\.A\.|S\.L\.|S\.R\.L\.|Sociedad|Empresa|Compañía|Corporación|Grupo)\b\.?', re.IGNORECASE),
                'fr': re.compile(r'\b(?:S\.A\.|S\.A\.R\.L\.|S\.A\.S\.|Société|Entreprise|Compagnie|Corporation|Groupe)\b\.?', re.IGNORECASE),
                'de': re.compile(r'\b(?:GmbH|AG|KG|OHG|Gesellschaft|Unternehmen|Firma|Konzern|Gruppe)\b\.?', re.IGNORECASE),
                'zh': re.compile(r'(?:公司|企业|集团|有限公司|股份有限公司|责任有限公司)', re.IGNORECASE),
                'ja': re.compile(r'(?:株式会社|有限会社|合同会社|合名会社|合資会社|企業|会社)', re.IGNORECASE),
                'ar': re.compile(r'(?:شركة|مؤسسة|مجموعة|القابضة|محدودة|مساهمة)', re.IGNORECASE),
                'ru': re.compile(r'\b(?:ООО|ЗАО|ОАО|ИП|Общество|Компания|Корпорация|Группа|Холдинг)\b\.?', re.IGNORECASE),
            }
        }
        
        # Registration number patterns
        patterns['registration'] = {
            'en': re.compile(r'\b(?:Tax ID|EIN|SSN|Registration|Reg|ID)[\s:]*([A-Z0-9-]{5,20})\b', re.IGNORECASE),
            'zh': re.compile(r'(?:统一社会信用代码|营业执照号码|组织机构代码)[\s:：]*([A-Z0-9]{10,20})'),
        ...
        # Address patterns
        patterns['address'] = {
            'en': re.compile(r'\d+\s+[A-Za-z\s]+(?:Street|St|Avenue|Ave|Road|Rd|Boulevard|Blvd|Drive|Dr|Lane|Ln|Way|Place|Pl)(?:\s+[A-Za-z\s]+)?', re.IGNORECASE),
            'zh': re.compile(r'(?:[北上广深杭]?[市区县])?[^市区县]*?[市区县][^路街道]*?[路街道][^号]*?号?'),
...
class MultiLanguageNER:
    ...
    def extract_entities(self, text: str, language: Optional[LanguageInfo] = None) -> List[MultiLangEntity]:
        ...
        for extractor in extractors:
            try:
                ...
                logger.error(f"Entity extraction failed with {type(extractor).__name__}: {e}")
    ...
    def extract_entities_mixed_language(self, detected_text: DetectedText) -> Dict[str, List[MultiLangEntity]]:
    ...
    def _filter_and_deduplicate(self, entities: List[MultiLangEntity]) -> List[MultiLangEntity]:
    ...
    def extract_structured_entities(self, text: str, language: Optional[LanguageInfo] = None) -> Dict[str, List[Dict[str, Any]]]:
