...
from .core import LanguageInfo, language_detector
    ...
    def normalize_phone(
        ...
        if not phone:
            return NormalizedField(
                "", "", "phone", 0.0, language_detector.language_data["en"]
            )
...
class AddressNormalizer:
    ...
    def parse_address(
        self, address: str, language: LanguageInfo, country: Optional[str] = None
    ) -> AddressComponents:
    ...
    def normalize_address(
        ...
        confidence = 0.7
        metadata = {"components": components.__dict__, "country": country}
...
class CompanyIdentifierNormalizer:
    ...
    def _load_id_patterns(self) -> Dict[str, Dict[str, str]]:
        ...
        return {
            "US": {"EIN": r"\d{2}-\d{7}", "DUNS": r"\d{9}", "CIK": r"\d{10}"},
            "CN": {
                "USCC": r"[0-9A-Z]{18}",  # Unified Social Credit Code
                "ORG_CODE": r"[0-9A-Z]{8}-[0-9A-Z]",
            },
            "RU": {"INN": r"\d{10,12}", "OGRN": r"\d{13,15}", "KPP": r"\d{9}"},
            "IN": {
                "CIN": r"[UL]\d{5}[A-Z]{2}\d{4}[A-Z]{3}\d{6}",
                "PAN": r"[A-Z]{5}\d{4}[A-Z]",
            },
            "JP": {"CORPORATE_NUMBER": r"\d{13}"},
            "DE": {"HRB": r"HRB\s*\d+", "UST_ID": r"DE\d{9}"},
            "GB": {"COMPANY_NUMBER": r"\d{8}", "VAT": r"GB\d{9}"},
        }
    ...
    def normalize_company_id(
        ...
        if not identifier:
            return NormalizedField(
                "", "", "company_id", 0.0, language_detector.language_data["en"]
            )
...
class FinancialDataNormalizer:
    ...
    def normalize_amount(
        self, amount_str: str, default_currency: str = "USD"
    ) -> NormalizedField:
        """Normalize financial amount"""
        if not amount_str:
            return NormalizedField(
                "", "", "amount", 0.0, language_detector.language_data["en"]
            )
        ...
        if not currency:
            for code in self.currency_codes:
                if re.search(rf"\b{code}\b", amount_str, re.IGNORECASE):
                    currency = code
                    amount_str = re.sub(
                        rf"\b{code}\b", "", amount_str, flags=re.IGNORECASE
                    ).strip()
                    confidence = 0.7
                    break
        ...
        if not amount_str:
            return NormalizedField(
                original, original, "amount", 0.0, language_detector.language_data["en"]
            )
        ...
        metadata = {
            "currency": currency,
            "numeric_value": (
                str(numeric_value) if "numeric_value" in locals() else None
            ),
