...
from .ner import EntityResult, EntityType, MultiLanguageNER, multilang_ner
...
class MultiLanguageProcessor:
    ...
    def process_text(
        ...
        if not text:
            return ProcessingResult(
                "",
                DetectedText(
                    "", self.detector.language_data["en"], ScriptType.UNKNOWN, 0.0
                ),
            )
        ...
        logger.debug(
            f"Detected language: {detected_language.language.name} ({detected_language.confidence:.2f})"
        )
        ...
        if include_normalization and result.entities:
            try:
                ...
                logger.debug(
                    f"Normalized {len(result.normalized_entities)} entity types"
                )
            except Exception as e:
    ...
    def _normalize_entities(
        self, entities: List[EntityResult], language: LanguageInfo
    ) -> Dict[str, List[NormalizedField]]:
        ...
        for entity in entities:
            ...
            try:
                ...
                if entity_type in ["phone", "telephone", "mobile"]:
                    ...
                    norm_field = self.address_normalizer.normalize_address(
                        entity_text, language
                    )

                elif entity_type in ["company_id", "registration_number", "tax_id"]:
                    norm_field = self.company_id_normalizer.normalize_company_id(
                        entity_text
                    )
                    ...
                    norm_field = self.date_normalizer.normalize_date(
                        entity_text, language
                    )
                ...
                logger.error(
                    f"Failed to normalize {entity_type} entity '{entity_text}': {e}"
                )
    ...
    def create_business_entity(
        ...
        if entity_type in ["company", "organization"]:
            normalized_dict = self.entity_normalizer.normalize_company_name(
                entity_text, language
            )
        elif entity_type in ["person", "name"]:
            normalized_dict = self.entity_normalizer.normalize_person_name(
                entity_text, language
            )
        else:
            normalized_dict = {
    ...
    def batch_process(self, texts: List[str], **kwargs) -> List[ProcessingResult]:
        ...
        for text in texts:
            try:
                ...
                logger.error(
                    f"Batch processing failed for text: {text[:100]}... Error: {e}"
                )
                # Create minimal result for failed processing
                detected = DetectedText(
                    text, self.detector.language_data["en"], ScriptType.UNKNOWN, 0.0
                )
    ...
    def extract_business_intelligence(self, text: str) -> Dict[str, Any]:
        ...
        if result.transliteration:
            intelligence["cross_language_matching"][
                "transliterated"
            ] = result.transliteration.transliterated
        ...
        if result.translation:
            ...
            ] = result.translation.language_to.code
    ...
    def get_capabilities(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "ner_support": {
                "spacy": (
                    self.ner.spacy_extractor.get_supported_languages()
                    if self.ner.spacy_extractor
                    else []
                ),
                "stanza": (
                    self.ner.stanza_extractor.get_supported_languages()
                    if self.ner.stanza_extractor
                    else []
                ),
