...
class AlertEvaluator:
    ...
    async def evaluate_entity_alerts(
        ...
        latest_assessment = (
            await session.execute(assessment_query)
        ).scalar_one_or_none()
        ...
        for rule in self.default_rules:
            ...
            if (
                rule.entity_type_filter
                and entity.entity_type != rule.entity_type_filter
            ):
                continue
    ...
    async def evaluate_source_alerts(
        ...
        result = await session.execute(
            metrics_query, {"url_pattern": f"{source.url_pattern}%", "since": since}
        )
        ...
        for rule in self.default_rules:
            if rule.source_filter and not source.url_pattern.startswith(
                rule.source_filter
            ):
                continue
    ...
    async def _evaluate_rule(
        ...
        if rule.condition == AlertCondition.THRESHOLD_BELOW:
            ...
            if historical_avg:
                change_percent = (
                    (metric_value - historical_avg) / historical_avg
                ) * 100
                triggered = (
                    change_percent <= rule.threshold_value
                )  # Negative threshold for drops
                message = f"{rule.metric_name} changed by {change_percent:.1f}% from historical average"

        if triggered:
            # Check cooldown
            if await self._is_in_cooldown(
                rule.rule_name, entity.entity_id, rule.cooldown_minutes, session
            ):
                return None
    ...
    async def _evaluate_source_rule(
        self,
        rule: AlertRule,
        source: DataSource,
        metrics: Dict[str, float],
    ...
    async def _get_historical_average(
        self, entity: EntityRecord, metric_name: str, session: AsyncSession
    ) -> Optional[float]:
        ...
        query = select(func.avg(field_map[metric_name])).where(
            and_(
    ...
    async def _is_in_cooldown(
        ...
        query = select(func.count(QualityAlert.alert_id)).where(
            and_(
...
class NotificationManager:
    ...
    async def send_alert_notification(
        self, alert: QualityAlert, channels: List[str] = None
    ) -> Dict[str, bool]:
    ...
    async def _send_email_notification(self, alert: QualityAlert) -> bool:
        ...
        try:
            ...
            msg["Subject"] = (
                f"Data Quality Alert: {alert.severity.upper()} - {alert.alert_type}"
            )
...
class AlertManager:
    ...
    async def process_entity_alerts(
        self, entity: EntityRecord, session: AsyncSession
    ) -> List[QualityAlert]:
    ...
    async def process_source_alerts(
        self, source: DataSource, session: AsyncSession
    ) -> List[QualityAlert]:
        ...
        for trigger in triggers:
            ...
            logger.info(
                f"Created source alert {alert.alert_id} for source {source.source_id}"
            )
    ...
    async def process_batch_alerts(
        ...
        try:
            ...
            for entity in entities:
                try:
                    ...
                    for alert in alerts:
                        severity = alert.severity
                        results["alerts_by_severity"][severity] = (
                            results["alerts_by_severity"].get(severity, 0) + 1
                        )
                    ...
                    logger.error(
                        f"Error processing alerts for entity {entity.entity_id}: {e}"
                    )
            ...
            for source in sources:
                try:
                    ...
                    logger.error(
                        f"Error processing alerts for source {source.source_id}: {e}"
                    )
        ...
        logger.info(
            f"Batch alert processing complete: {results['total_alerts']} alerts generated"
        )
        return results
