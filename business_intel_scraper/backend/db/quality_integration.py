...
class QualityIntegrationPipeline:
    ...
    async def process_scraped_entity(self, entity_data: Dict[str, Any],
                                   raw_content: str, source_info: Dict[str, Any],
                                   session: AsyncSession) -> ProcessingResult:
        ...
        try:
            ...
            self.processing_stats['quality_assessments'] += 1 if quality_assessment else 0
            self.processing_stats['provenance_records'] += 1
            self.processing_stats['alerts_generated'] += alerts_generated
            
            logger.info(f"Successfully processed entity {entity_id} (quality: {quality_assessment.overall_score:.3f})")
    ...
    async def _ensure_data_source(self, source_info: Dict[str, Any],
                                session: AsyncSession) -> DataSource:
    ...
    async def _record_raw_data(self, raw_content: str, source_info: Dict[str, Any],
                             entity_data: Dict[str, Any], session: AsyncSession) -> RawDataRecord:
    ...
    async def _create_or_update_entity(self, entity_data: Dict[str, Any],
                                     raw_record: RawDataRecord,
                                     session: AsyncSession) -> EntityRecord:
    ...
    async def _record_field_provenance(self, entity_record: EntityRecord,
        ...
        for field_name, field_value in entity_data.items():
            ...
            await provenance_tracker.record_field_provenance({
                ...
                'metadata': {
                    'scraper_type': source_info.get('scraper_type'),
                    'field_xpath': source_info.get('field_selectors', {}).get(field_name),
                    'extraction_timestamp': datetime.now(timezone.utc).isoformat()
                }
            }, entity_record, session)
    
    async def _generate_auto_corrections(self, entity_record: EntityRecord,
                                       session: AsyncSession):
        ...
        try:
            ...
            logger.error(f"Error generating auto-corrections for {entity_record.entity_id}: {e}")
    
    async def process_batch_entities(self, entities_data: List[Dict[str, Any]],
        ...
        for entity_data in entities_data:
            ...
            clean_entity_data = {k: v for k, v in entity_data.items() 
        ...
        logger.info(f"Batch processing complete: {results['successful']}/{results['total_entities']} successful")
        return results
    
    async def migrate_existing_entities(self, session: AsyncSession,
        ...
        logger.info(f"Migration complete: {migration_results['entities_migrated']} entities migrated")
...
class QualityMiddleware:
    ...
    async def process_scraped_data(self, data: Dict[str, Any],
                                 session: AsyncSession) -> Dict[str, Any]:
