...
from .quality_models import (
    EntityRecord, DataCorrection, DataChangeLog, QualityAlert,
    ProvenanceRecord, ChangeType
)
...
class CorrectionValidator:
    ...
    def __init__(self):
        ...
        self.company_suffixes = [
            'inc', 'corp', 'llc', 'ltd', 'limited', 'company', 'co',
            'corporation', 'incorporated', 'group', 'holdings',
            'gmbh', 'ag', 'sa', 'sarl', 'bv', 'oy', 'ab'
        ]
    
    async def validate_correction(self, correction: Dict[str, Any], 
                                entity: EntityRecord, session: AsyncSession) -> ValidationResult:
        ...
        if field_type in self.field_validators:
            try:
                is_valid, field_confidence, field_issues = await self.field_validators[field_type](
        ...
        if conflicting_corrections:
            warnings.append(f"Found {len(conflicting_corrections)} conflicting corrections")
            evidence.append(f"Conflicting values: {[c.suggested_value for c in conflicting_corrections]}")
        ...
        if supporting_sources:
            ...
            evidence.append(f"Supported by {len(supporting_sources)} additional sources")
    ...
    def _infer_field_type(self, field_name: str, value: Any) -> str:
        ...
        if 'email' in field_name_lower:
            ...
            return 'address'
        elif 'date' in field_name_lower or isinstance(value, str) and re.match(r'\d{4}-\d{2}-\d{2}', value):
            return 'date'
        else:
            return 'generic'
    
    async def _validate_email(self, value: str, field_name: str, 
                            entity: EntityRecord, session: AsyncSession) -> Tuple[bool, float, List[str]]:
        ...
        similar_domain = difflib.get_close_matches(domain, common_domains, n=1, cutoff=0.8)
        
        if similar_domain and similar_domain[0] != domain:
            issues.append(f"Domain '{domain}' is similar to '{similar_domain[0]}' - possible typo")
            confidence = 0.6
        
        return len(issues) == 0, confidence, issues
    
    async def _validate_phone(self, value: str, field_name: str,
                            entity: EntityRecord, session: AsyncSession) -> Tuple[bool, float, List[str]]:
    ...
    async def _validate_url(self, value: str, field_name: str,
                          entity: EntityRecord, session: AsyncSession) -> Tuple[bool, float, List[str]]:
        """Validate URL"""
    ...
    async def _validate_registration_id(self, value: str, field_name: str,
                                      entity: EntityRecord, session: AsyncSession) -> Tuple[bool, float, List[str]]:
    ...
    async def _validate_address(self, value: str, field_name: str,
                              entity: EntityRecord, session: AsyncSession) -> Tuple[bool, float, List[str]]:
        """Validate address"""
        issues = []
        confidence = 0.7
        ...
        has_street_indicator = bool(re.search(r'\b(street|st|avenue|ave|road|rd|lane|ln|drive|dr|boulevard|blvd)\b', value.lower()))
    ...
    async def _validate_date(self, value: str, field_name: str,
                           entity: EntityRecord, session: AsyncSession) -> Tuple[bool, float, List[str]]:
        """Validate date"""
        issues = []
        confidence = 0.8
    ...
    async def _check_conflicting_corrections(self, entity_id: str, field_name: str,
                                           suggested_value: str, session: AsyncSession) -> List[DataCorrection]:
        ...
        query = (
            ...
            .where(
                and_(
                    ...
                    DataCorrection.status.in_([CorrectionStatus.PENDING, CorrectionStatus.APPROVED])
                )
            )
        )
    ...
    async def _find_supporting_evidence(self, entity: EntityRecord, field_name: str,
                                       suggested_value: str, session: AsyncSession) -> List[str]:
    ...
    async def _validate_business_logic(self, entity: EntityRecord, field_name: str,
                                     suggested_value: str, session: AsyncSession) -> Dict[str, Any]:
        ...
        if field_name.lower() in ['name', 'company_name'] and entity.entity_type == 'company':
            ...
            has_suffix = any(word.strip('.,') in self.company_suffixes for word in words)
            
            if not has_suffix:
                result['warnings'].append("Company name doesn't include common business suffix")
                result['confidence_multiplier'] *= 0.9
        
        # Person name validation
        elif field_name.lower() in ['name', 'person_name'] and entity.entity_type == 'person':
...
class AutoCorrectionEngine:
    ...
    async def generate_suggestions(self, entity: EntityRecord, 
                                 session: AsyncSession) -> List[CorrectionSuggestion]:
        ...
        try:
            ...
            suggestions.extend(await self._suggest_missing_field_fixes(entity, provenance_records, session))
            suggestions.extend(await self._suggest_format_fixes(entity, session))
            suggestions.extend(await self._suggest_consistency_fixes(entity, provenance_records, session))
            suggestions.extend(await self._suggest_duplicate_merges(entity, session))
            
        except Exception as e:
            logger.error(f"Error generating suggestions for entity {entity.entity_id}: {e}")
        
        return suggestions
    
    async def _suggest_missing_field_fixes(self, entity: EntityRecord, 
    ...
    async def _suggest_format_fixes(self, entity: EntityRecord, 
                                   session: AsyncSession) -> List[CorrectionSuggestion]:
            ...
            if 'phone' in field_name.lower():
                ...
                if cleaned_phone != value and len(cleaned_phone) >= 10:
                    ...
                    if not cleaned_phone.startswith('+'):
                        formatted = f"+1{cleaned_phone}" if len(cleaned_phone) == 10 else f"+{cleaned_phone}"
                    else:
    ...
    async def _suggest_consistency_fixes(self, entity: EntityRecord,
                                       provenance_records: List[ProvenanceRecord],
                                       session: AsyncSession) -> List[CorrectionSuggestion]:
            ...
            if len(unique_values) > 1:
                ...
                if best_value and best_value != current_value:
                    suggestion = CorrectionSuggestion(
                        ...
                        evidence=[inst['source_url'] for inst in unique_values[best_value]],
    ...
    async def _suggest_duplicate_merges(self, entity: EntityRecord,
                                      session: AsyncSession) -> List[CorrectionSuggestion]:
...
class CorrectionManager:
    ...
    async def submit_correction(self, correction_data: Dict[str, Any],
                               session: AsyncSession) -> DataCorrection:
        ...
        # Validate correction
        validation = await self.validator.validate_correction(correction_data, entity, session)
        ...
        logger.info(f"Submitted correction {correction.correction_id} for entity {entity.entity_id}")
        
        # Auto-apply if high confidence and no issues
        if (validation.is_valid and 
            validation.confidence >= 0.95 and 
            correction_data.get('auto_apply', False)):
            await self.apply_correction(correction.correction_id, 'system', session)
        
        return correction
    
    async def review_correction(self, correction_id: str, reviewer: str,
                               decision: str, notes: str, session: AsyncSession) -> DataCorrection:
        """Review and approve/reject a correction"""
        correction_query = select(DataCorrection).where(DataCorrection.correction_id == correction_id)
    ...
    async def apply_correction(self, correction_id: str, applied_by: str,
                              session: AsyncSession) -> Optional[DataChangeLog]:
        ...
        try:
            ...
            if correction.correction_type == CorrectionType.MERGE_ENTITIES:
                ...
                change_log = await self._merge_entities(entity, new_value, applied_by, session)
            else:
                # Simple field update
            ...
            logger.info(f"Applied correction {correction_id} to entity {entity.entity_id}")
            return change_log
    ...
    async def _merge_entities(self, primary_entity: EntityRecord, target_entity_id: str,
                             merged_by: str, session: AsyncSession) -> DataChangeLog:
        """Merge two entities"""
        # Get target entity
        target_query = select(EntityRecord).where(EntityRecord.entity_id == target_entity_id)
        ...
        }, primary_entity, session)
        
        return change_log
    
    async def get_pending_corrections(self, session: AsyncSession,
    ...
    async def generate_auto_corrections(self, session: AsyncSession,
        ...
        for entity in entities:
            try:
                suggestions = await self.auto_engine.generate_suggestions(entity, session)
                ...
                for suggestion in suggestions:
                    ...
                    correction_data = {
                        ...
                        'auto_apply': suggestion.confidence >= suggestion.auto_apply_threshold
                    }
        ...
        logger.info(f"Auto-correction complete: {results['total_suggestions']} suggestions generated")
        return results
