...
from sqlalchemy import (
    String, ForeignKey, DateTime, Float, Integer, Boolean, Text, JSON,
...
class RawDataRecord(Base):
    ...
    source_timestamp: Mapped[datetime] = mapped_column(DateTime, nullable=True)  # When source claims data was created
    created_at: Mapped[datetime] = mapped_column(DateTime, default=lambda: datetime.now(timezone.utc))
    ...
    provenance_records: Mapped[List["ProvenanceRecord"]] = relationship(back_populates="raw_record")
...
class EntityRecord(Base):
    ...
    entity_type: Mapped[str] = mapped_column(String(50), nullable=False)  # company, person, address, etc.
    ...
    # Status
    quality_status: Mapped[QualityStatus] = mapped_column(String(20), default=QualityStatus.FAIR)
    confidence_level: Mapped[ConfidenceLevel] = mapped_column(String(20), default=ConfidenceLevel.MEDIUM)
    ...
    duplicate_of: Mapped[str] = mapped_column(String(100), nullable=True)  # entity_id of canonical record
    
    # Relationships
    provenance_records: Mapped[List["ProvenanceRecord"]] = relationship(back_populates="entity")
    quality_assessments: Mapped[List["QualityAssessment"]] = relationship(back_populates="entity")
...
class ProvenanceRecord(Base):
    ...
    # Entity and field
    entity_id: Mapped[int] = mapped_column(ForeignKey("entity_records.id"), nullable=False)
    field_name: Mapped[str] = mapped_column(String(100), nullable=False)
    field_value: Mapped[str] = mapped_column(Text, nullable=True)
    
    # Source tracking
    raw_record_id: Mapped[int] = mapped_column(ForeignKey("raw_data_records.id"), nullable=False)
    source_url: Mapped[str] = mapped_column(String(1000), nullable=False)
    ...
    signature: Mapped[str] = mapped_column(String(512), nullable=True)  # For auditability
    
    # Relationships
    entity: Mapped["EntityRecord"] = relationship(back_populates="provenance_records")
    raw_record: Mapped["RawDataRecord"] = relationship(back_populates="provenance_records")
...
class QualityAssessment(Base):
    ...
    # Assessment details
    assessment_type: Mapped[str] = mapped_column(String(50), nullable=False)  # completeness, consistency, etc.
    assessed_at: Mapped[datetime] = mapped_column(DateTime, default=lambda: datetime.now(timezone.utc))
    assessor: Mapped[str] = mapped_column(String(100), nullable=True)  # system, user_id, etc.
...
class DataChangeLog(Base):
    ...
    changed_by: Mapped[str] = mapped_column(String(100), nullable=True)  # user_id, system, etc.
...
class QualityRule(Base):
    ...
    # Rule definition
    entity_type: Mapped[str] = mapped_column(String(50), nullable=True)  # Apply to specific entity types
    field_name: Mapped[str] = mapped_column(String(100), nullable=True)  # Apply to specific fields
    rule_type: Mapped[str] = mapped_column(String(50), nullable=False)  # completeness, format, range, etc.
...
class DataCorrection(Base):
    ...
    correction_type: Mapped[str] = mapped_column(String(50), nullable=False)  # fix, merge, split, flag
    ...
    submission_source: Mapped[str] = mapped_column(String(100), nullable=True)  # gui, api, etc.
...
class QualityAlert(Base):
    ...
    # Alert details
    alert_type: Mapped[str] = mapped_column(String(50), nullable=False)  # quality_drop, staleness, anomaly
...
Index('idx_entity_type_quality', EntityRecord.entity_type, EntityRecord.overall_quality_score)
Index('idx_entity_updated_at', EntityRecord.updated_at)
Index('idx_entity_quality_status', EntityRecord.quality_status)

Index('idx_provenance_entity_field', ProvenanceRecord.entity_id, ProvenanceRecord.field_name)
...
Index('idx_quality_assessment_entity', QualityAssessment.entity_id, QualityAssessment.assessment_type)
