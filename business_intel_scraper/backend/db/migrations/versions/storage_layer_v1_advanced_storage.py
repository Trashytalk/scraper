...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('fetched_at', sa.DateTime(), nullable=False, index=True, server_default=sa.func.now()),
    ...
    op.create_index('idx_raw_data_domain_date', 'raw_data', ['source_domain', 'fetched_at'])
    op.create_index('idx_raw_data_processing', 'raw_data', ['processing_status', 'extraction_attempted'])
    op.create_index('idx_raw_data_quality', 'raw_data', ['content_quality_score', 'is_duplicate'])
    ...
    op.create_table(
        ...
        sa.Column('extracted_at', sa.DateTime(), nullable=False, index=True, server_default=sa.func.now()),
    ...
    op.create_index('idx_entity_type_name', 'structured_entities', ['entity_type', 'canonical_name'])
    op.create_index('idx_entity_confidence', 'structured_entities', ['entity_type', 'confidence_score'])
    op.create_index('idx_entity_category', 'structured_entities', ['category', 'subcategory'])
    op.create_index('idx_entity_status', 'structured_entities', ['is_active', 'verification_status'])
    op.create_index('idx_entity_quality', 'structured_entities', ['data_quality_score', 'completeness_score'])
    op.create_index('idx_entity_temporal', 'structured_entities', ['first_seen_at', 'last_verified_at'])
    
    # Raw to Structured Mapping table
    op.create_table(
        'raw_to_structured_mapping',
        sa.Column('mapping_id', sa.String(), primary_key=True),
        sa.Column('raw_id', sa.String(), sa.ForeignKey('raw_data.raw_id'), nullable=False, index=True),
        sa.Column('entity_id', sa.String(), sa.ForeignKey('structured_entities.entity_id'), nullable=False, index=True),
    ...
    op.create_index('idx_mapping_raw_entity', 'raw_to_structured_mapping', ['raw_id', 'entity_id'])
    op.create_index('idx_mapping_entity_confidence', 'raw_to_structured_mapping', ['entity_id', 'extraction_confidence'])
    op.create_index('idx_mapping_method', 'raw_to_structured_mapping', ['extraction_method', 'extractor_name'])
    ...
    op.create_table(
        ...
        sa.Column('source_entity_id', sa.String(), sa.ForeignKey('structured_entities.entity_id'), nullable=False, index=True),
        sa.Column('target_entity_id', sa.String(), sa.ForeignKey('structured_entities.entity_id'), nullable=False, index=True),
    ...
    op.create_index('idx_rel_source_target', 'entity_relationships', ['source_entity_id', 'target_entity_id'])
    op.create_index('idx_rel_type', 'entity_relationships', ['relationship_type', 'relationship_subtype'])
    op.create_index('idx_rel_strength', 'entity_relationships', ['strength', 'confidence'])
    op.create_index('idx_rel_temporal', 'entity_relationships', ['relationship_start_date', 'relationship_end_date'])
    
    # Add check constraints for relationships
    op.create_check_constraint('ck_strength_range', 'entity_relationships', 'strength >= 0 AND strength <= 1')
    op.create_check_constraint('ck_confidence_range', 'entity_relationships', 'confidence >= 0 AND confidence <= 1')
    ...
    op.create_table(
        ...
        sa.Column('entity_id', sa.String(), sa.ForeignKey('structured_entities.entity_id'), index=True),
        ...
        sa.Column('measurement_date', sa.DateTime(), nullable=False, server_default=sa.func.now()),
    ...
    op.create_index('idx_quality_scope', 'data_quality_metrics', ['scope_type', 'scope_id'])
    op.create_index('idx_quality_metric', 'data_quality_metrics', ['metric_type', 'metric_name'])
    op.create_index('idx_quality_score', 'data_quality_metrics', ['score', 'grade'])
    op.create_index('idx_quality_temporal', 'data_quality_metrics', ['measurement_date', 'scope_type'])
    op.create_index('idx_quality_trend', 'data_quality_metrics', ['trend', 'passes_threshold'])
    ...
    op.create_index('idx_lineage_transformation', 'data_lineage', ['transformation_type', 'transformation_name'])
    op.create_index('idx_lineage_path', 'data_lineage', ['path_length', 'dependency_count'])
    op.create_index('idx_lineage_quality', 'data_lineage', ['transformation_confidence', 'data_quality_impact'])
    ...
    op.create_table(
        ...
        sa.Column('measured_at', sa.DateTime(), nullable=False, server_default=sa.func.now(), index=True),
    ...
    op.create_index('idx_storage_metrics_category', 'storage_metrics', ['metric_category', 'metric_name'])
    op.create_index('idx_storage_metrics_temporal', 'storage_metrics', ['measured_at', 'measurement_period'])
    op.create_index('idx_storage_metrics_scope', 'storage_metrics', ['scope', 'scope_id'])
    op.create_index('idx_storage_metrics_alerts', 'storage_metrics', ['alert_level', 'measured_at'])
