...
from .correction_system import CorrectionStatus
...
class QualityDashboard:
    ...
    async def get_overview_metrics(
        ...
        time_range: DashboardTimeRange = DashboardTimeRange.LAST_DAY,
    ) -> Dict[str, Any]:
        ...
        # Basic entity counts
        total_entities_query = select(func.count(EntityRecord.id)).where(
        ...
        if time_filter:
            quality_dist_query = quality_dist_query.where(
                EntityRecord.updated_at >= time_filter
            )

        quality_stats = (await session.execute(quality_dist_query)).first()

        # Issue counts
        entities_with_issues_query = select(func.count(EntityRecord.id)).where(
            and_(EntityRecord.is_active == True, EntityRecord.has_issues == True)
        )

        if time_filter:
            entities_with_issues_query = entities_with_issues_query.where(
                EntityRecord.updated_at >= time_filter
            )

        entities_with_issues = (
            await session.execute(entities_with_issues_query)
        ).scalar()

        # Duplicate counts
        duplicate_entities_query = select(func.count(EntityRecord.id)).where(
            EntityRecord.is_duplicate == True
        )
        duplicates = (await session.execute(duplicate_entities_query)).scalar()

        # Recent corrections
        corrections_query = select(func.count(DataCorrection.correction_id)).where(
            DataCorrection.status == CorrectionStatus.PENDING
        )
        pending_corrections = (await session.execute(corrections_query)).scalar()

        # Recent alerts
        alerts_query = select(func.count(QualityAlert.alert_id)).where(
        ...
        freshness_query = select(
            func.avg(func.extract("epoch", func.now() - EntityRecord.last_seen) / 86400)
        ).where(EntityRecord.is_active == True)
        ...
        return {
            ...
            "issue_rate": round(
                (entities_with_issues / max(total_entities, 1)) * 100, 1
            ),
    ...
    async def get_source_quality_reports(
        self, session: AsyncSession, limit: int = 20
    ) -> List[SourceQualityReport]:
        ...
        for source in sources:
            ...
            metrics_result = await session.execute(
                metrics_query, {"url_pattern": f"{source.url_pattern}%"}
            )
    ...
    async def get_entity_quality_reports(
        self,
        session: AsyncSession,
        entity_type: Optional[str] = None,
        quality_threshold: float = 0.5,
        limit: int = 50,
    ) -> List[EntityQualityReport]:
        """Get quality reports for individual entities"""
        logger.info(
            f"Generating entity quality reports (type: {entity_type}, limit: {limit})"
        )
        ...
        for entity in entities:
            ...
            if entity.quality_assessments:
                latest_assessment = max(
                    entity.quality_assessments, key=lambda x: x.assessed_at
                )
            ...
            sources_query = select(
                func.count(func.distinct(ProvenanceRecord.source_url))
    ...
    async def get_alerts_summary(
        ...
        severity_query = select(
            QualityAlert.severity, func.count(QualityAlert.alert_id).label("count")
        ).group_by(QualityAlert.severity)

        if time_filter:
            severity_query = severity_query.where(
                QualityAlert.triggered_at >= time_filter
            )
        ...
        # Alerts by type
        type_query = select(
            QualityAlert.alert_type, func.count(QualityAlert.alert_id).label("count")
        ...
        return {
            ...
            "resolution_rate": round(
                ((total_count - active_count) / max(total_count, 1)) * 100, 1
            ),
    ...
    async def get_correction_statistics(
        ...
        if time_filter:
            status_query = status_query.where(
                DataCorrection.submitted_at >= time_filter
            )
        ...
        # Average confidence
        confidence_query = select(func.avg(DataCorrection.confidence))
        ...
        if time_filter:
            contributors_query = contributors_query.where(
                DataCorrection.submitted_at >= time_filter
            )
    ...
    async def _calculate_source_trend(
        self, source: DataSource, session: AsyncSession
    ) -> str:
        ...
        result = await session.execute(
            trend_query, {"url_pattern": f"{source.url_pattern}%", "since": since}
        )
...
class DashboardAPI:
    ...
    async def get_dashboard_data(
        self, session: AsyncSession, dashboard_type: str = "overview", **kwargs
    ) -> Dict[str, Any]:
        ...
        try:
            if dashboard_type == "overview":
                ...
                return {
                    "trends": [
                        asdict(trend)
                        for trend in await self.dashboard.get_quality_trends(
                            session, **kwargs
                        )
                    ]
                }
                ...
                return {
                    "sources": [
                        asdict(report)
                        for report in await self.dashboard.get_source_quality_reports(
                            session, **kwargs
                        )
                    ]
                }
                ...
                return {
                    "entities": [
                        asdict(report)
                        for report in await self.dashboard.get_entity_quality_reports(
                            session, **kwargs
                        )
                    ]
                }
    ...
    async def export_quality_report(
        ...
        if report_type in ["comprehensive", "sources"]:
            report["sources"] = [
                asdict(source)
                for source in await self.dashboard.get_source_quality_reports(
                    session, limit=100
                )
            ]
        ...
        if report_type in ["comprehensive", "entities"]:
            report["problem_entities"] = [
                asdict(entity)
                for entity in await self.dashboard.get_entity_quality_reports(
                    session, limit=100
                )
            ]
        ...
        if report_type in ["comprehensive", "corrections"]:
            report["corrections"] = await self.dashboard.get_correction_statistics(
                session
            )
