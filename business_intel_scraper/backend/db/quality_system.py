...
from .quality_models import (
    Base, DataSource, EntityRecord, QualityAssessment, 
    ProvenanceRecord, DataChangeLog, QualityRule, 
    DataCorrection, QualityAlert, RawDataRecord
)
...
class DataQualitySystem:
    ...
    async def initialize(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Created {len(init_results['tables_created'])} quality system tables")
            ...
            init_results['components_initialized'] = [
                'quality_engine', 'provenance_tracker', 'correction_manager',
                'alert_manager', 'quality_dashboard', 'integration_pipeline'
            ]
            
            self.is_initialized = True
            
            logger.info("Data Quality & Provenance Intelligence System successfully initialized")
            return init_results
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            async with AsyncSession(self.engine) as session:
                ...
                activity_check = await session.execute(text("""
                    SELECT 
    ...
    async def run_maintenance_tasks(self) -> Dict[str, Any]:
        ...
        try:
            async with AsyncSession(self.engine) as session:
                ...
                correction_results = await correction_manager.generate_auto_corrections(session)
                maintenance_results['corrections_generated'] = correction_results['total_suggestions']
                maintenance_results['tasks_completed'].append('auto_correction_generation')
                
                # Cleanup old records (older than 6 months)
                cleanup_date = datetime.now(timezone.utc).replace(month=datetime.now().month - 6)
        ...
        logger.info(f"Maintenance tasks completed: {len(maintenance_results['tasks_completed'])}")
        ...
        try:
            async with AsyncSession(self.engine) as session:
                ...
                rules_result = await session.execute(
                    text("SELECT rule_name, rule_type, conditions, severity, is_active FROM quality_rules")
                )
                ...
                sources_result = await session.execute(
                    text("SELECT source_id, name, url_pattern, source_type, reliability_score FROM data_sources WHERE is_active = true")
                )
...
data_quality_system = DataQualitySystem("postgresql+asyncpg://localhost/business_intel_scraper")
