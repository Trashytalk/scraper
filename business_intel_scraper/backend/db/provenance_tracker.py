...
class ProvenanceTracker:
    ...
    def _initialize_signing(self):
        """Initialize cryptographic signing if available."""
        if not CRYPTO_AVAILABLE:
            return
            
        import os
        key_path = os.getenv("PROVENANCE_PRIVATE_KEY_PATH")
        if key_path and os.path.exists(key_path):
            try:
                ...
                with open(key_path, "rb") as key_file:
                    self.signature_key = serialization.load_pem_private_key(
                        key_file.read(),
                        password=os.getenv("PROVENANCE_KEY_PASSWORD", "").encode() or None
                    )
                self.enable_signatures = True
            except Exception as e:
                logging.warning(f"Failed to load signing key: {e}")
        elif os.getenv("PROVENANCE_ENABLE_SIGNING", "").lower() == "true":
            ...
            try:
                ...
                self.enable_signatures = True
                logging.info("Generated new signing key for provenance tracking")
            except Exception as e:
                logging.warning(f"Failed to generate signing key: {e}")

    async def record_data_source(
        self, source_config: Dict[str, Any], session: AsyncSession
    ) -> DataSource:
        ...
        if existing_source:
            ...
            existing_source.description = source_config.get(
    ...
    async def record_raw_extraction(
        self, extraction_data: Dict[str, Any], session: AsyncSession
    ) -> RawDataRecord:
        ...
        if duplicate:
            logger.info(
                f"Duplicate content detected for {source_url}, using existing record"
            )
            return duplicate
        ...
        raw_record = RawDataRecord(
            ...
            extracted_at=extraction_data.get(
    ...
    async def record_field_provenance(
        ...
        logger.debug(
            f"Recorded provenance for field {field_name} of entity {entity.entity_id}"
        )
        return provenance

    async def record_entity_change(
        self, change_data: Dict[str, Any], entity: EntityRecord, session: AsyncSession
    ) -> DataChangeLog:
        ...
        logger.info(
            f"Recorded change {change_log.change_type.value} for entity {entity.entity_id}"
        )
        return change_log

    async def get_field_lineage(
        self, entity_id: str, field_name: str, session: AsyncSession
        ...
        prov_query = (
            ...
            .options(
                selectinload(ProvenanceRecord.raw_record).selectinload(
        ...
        return ProvenanceInfo(
            ...
            transformations=(
                provenance.transformation_applied.split(",")
                if provenance.transformation_applied
                else []
            ),
    ...
    async def get_entity_lineage_tree(
        self, entity_id: str, session: AsyncSession
        ...
        entity_query = (
            ...
            .options(
                selectinload(EntityRecord.provenance_records)
                .selectinload(ProvenanceRecord.raw_record)
                .selectinload(RawDataRecord.source)
            )
        )
        ...
        for prov in entity.provenance_records:
            ...
            if raw_record.raw_id not in raw_records:
                raw_records[raw_record.raw_id] = {"record": raw_record, "fields": []}
    ...
    async def trace_data_lineage(
        self, entity_id: str, session: AsyncSession, field_name: Optional[str] = None
    ) -> List[Dict[str, Any]]:
    ...
    async def verify_data_integrity(
        ...
        prov_query = select(ProvenanceRecord).where(
            ProvenanceRecord.entity_id == entity.id
        )
        ...
        for prov in provenance_records:
            ...
            if expected_hash == prov.provenance_hash:
                ...
                integrity_report["issues"].append(
                    f"Provenance hash mismatch for field {prov.field_name}"
                )
                integrity_report["hash_mismatches"].append(
                    {
                        "type": "provenance",
    ...
    def _generate_signature(self, provenance: ProvenanceRecord) -> str:
        ...
        message_data = {
            ...
            "extracted_at": provenance.extracted_at.isoformat() if provenance.extracted_at else "",
            "source_url": provenance.source_url or "",
            "transformation_applied": provenance.transformation_applied or ""
        }
        message = json.dumps(message_data, sort_keys=True).encode()
        
        # Check if signature key is available and is RSA key
        if (self.signature_key and self.enable_signatures and CRYPTO_AVAILABLE and 
            hasattr(self.signature_key, 'sign')):
            try:
                from cryptography.hazmat.primitives import hashes, serialization
                from cryptography.hazmat.primitives.asymmetric import rsa, padding
                
                # Ensure we have an RSA key
                if isinstance(self.signature_key, rsa.RSAPrivateKey):
                ...
                # Fall back to hash if signing fails
                import logging
                logging.warning(f"Signature generation failed, using hash: {e}")
        
        # Fall back to strong hash-based signature
