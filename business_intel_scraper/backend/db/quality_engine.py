...
from .quality_models import (
    EntityRecord, QualityAssessment, QualityRule, QualityAlert,
    DataSource, ProvenanceRecord, QualityStatus, ConfidenceLevel
)
...
class QualityAssessor(ABC):
    ...
    @abstractmethod
    async def assess(self, entity: EntityRecord, session: AsyncSession) -> QualityMetric:
...
class CompletenessAssessor(QualityAssessor):
    ...
    def __init__(self):
        ...
        self.optional_fields = {
            'company': ['phone', 'email', 'website', 'industry', 'employees', 'revenue'],
    ...
    async def assess(self, entity: EntityRecord, session: AsyncSession) -> QualityMetric:
        ...
        present_required = sum(1 for field in required if self._is_field_present(data, field))
        required_score = present_required / len(required) if required else 1.0
        
        # Count present optional fields
        present_optional = sum(1 for field in optional if self._is_field_present(data, field))
        ...
        missing_required = [field for field in required if not self._is_field_present(data, field)]
        missing_optional = [field for field in optional if not self._is_field_present(data, field)]
        ...
        if missing_required:
            ...
            recommendations.append(f"Collect missing required data: {', '.join(missing_required)}")
        
        if len(missing_optional) > len(optional) * 0.5:  # More than 50% optional fields missing
            recommendations.append(f"Consider collecting optional data for better insights: {', '.join(missing_optional[:3])}...")
    ...
    def _is_field_present(self, data: Dict[str, Any], field: str) -> bool:
        ...
        placeholders = {"n/a", "na", "null", "unknown", "not available", "tbd", "pending"}
...
class ConsistencyAssessor(QualityAssessor):
    ...
    async def assess(self, entity: EntityRecord, session: AsyncSession) -> QualityMetric:
        ...
        provenance_query = select(ProvenanceRecord).where(
        ...
        for field, sources in field_sources.items():
            ...
            if len(unique_values) == 1:
                ...
                issues.append(f"Minor value conflicts in field '{field}': {list(unique_values)[:3]}")
            else:
                # Major conflicts
                consistency_scores.append(0.3)
                issues.append(f"Major value conflicts in field '{field}': {list(unique_values)[:3]}")
                recommendations.append(f"Review conflicting values for field '{field}' and determine canonical value")
        ...
        overall_score = statistics.mean(consistency_scores) if consistency_scores else 0.0
    ...
    async def _assess_temporal_consistency(self, entity: EntityRecord, session: AsyncSession) -> float:
...
class FreshnessAssessor(QualityAssessor):
    ...
    async def assess(self, entity: EntityRecord, session: AsyncSession) -> QualityMetric:
        ...
        if age_days > expected_freshness:
            if age_days > expected_freshness * 4:
                issues.append(f"Data is very stale: {age_days:.1f} days old (expected < {expected_freshness} days)")
                recommendations.append("Urgent: Update this entity's data from original sources")
            elif age_days > expected_freshness * 2:
                issues.append(f"Data is stale: {age_days:.1f} days old (expected < {expected_freshness} days)")
    ...
    async def _assess_source_freshness(self, entity: EntityRecord, session: AsyncSession) -> float:
        """Assess freshness of underlying sources"""
        # Get all data sources for this entity
        sources_query = select(DataSource).join(
            ProvenanceRecord, DataSource.id == ProvenanceRecord.raw_record.has(
                ProvenanceRecord.entity_id == entity.id
            )
        ).distinct()
        ...
        try:
            ...
            for source in sources:
                if source.last_successful_access:
                    age = (datetime.now(timezone.utc) - source.last_successful_access).total_seconds() / 86400
...
class ConfidenceAssessor(QualityAssessor):
    ...
    async def assess(self, entity: EntityRecord, session: AsyncSession) -> QualityMetric:
        ...
        provenance_query = select(ProvenanceRecord).where(
            ...
            selectinload(ProvenanceRecord.raw_record).selectinload(DataSource)
        )
        ...
        avg_extraction_score = statistics.mean(extraction_scores) if extraction_scores else 0.5
        overall_confidence = statistics.mean(confidence_scores) if confidence_scores else 0.1
        
        # Source diversity bonus
        unique_sources = len(set(prov.raw_record.source.source_id for prov in provenance_records if prov.raw_record))
        diversity_bonus = min(0.1, unique_sources * 0.02)  # Up to 10% bonus for multiple sources
        ...
        if unique_sources == 1:
            recommendations.append("Cross-verify with additional sources to increase confidence")
...
class DataQualityEngine:
    ...
    async def assess_entity(self, entity: EntityRecord, session: AsyncSession) -> QualityAssessmentResult:
        ...
        completeness_score = next((m.score for m in metrics if m.name == "completeness"), 0.0)
        consistency_score = next((m.score for m in metrics if m.name == "consistency"), 0.0)
        freshness_score = next((m.score for m in metrics if m.name == "freshness"), 0.0)
        confidence_score = next((m.score for m in metrics if m.name == "confidence"), 0.0)
    ...
    async def store_assessment(self, assessment: QualityAssessmentResult, 
                             entity: EntityRecord, session: AsyncSession) -> None:
        ...
        logger.info(f"Stored assessment for entity {entity.entity_id} (score: {assessment.overall_score:.3f})")
    
    async def batch_assess_entities(self, entity_ids: List[str], session: AsyncSession) -> List[QualityAssessmentResult]:
    ...
    async def assess_all_entities(self, session: AsyncSession, batch_size: int = 100) -> Dict[str, Any]:
        """Assess all entities in the database"""
        logger.info("Starting full database quality assessment")
        
        # Count total entities
        count_query = select(func.count(EntityRecord.id)).where(EntityRecord.is_active == True)
        ...
        logger.info(f"Completed quality assessment: {processed}/{total_count} entities processed")
        return results
    ...
    async def get_quality_summary(self, session: AsyncSession) -> Dict[str, Any]:
        ...
        issues_query = (
            ...
            .where(
                and_(
                    ...
                    EntityRecord.updated_at >= datetime.now(timezone.utc) - timedelta(days=7)
                )
            )
        ...
        return {
            ...
            'entities_with_issues': sum(1 for status in status_distribution if status in [QualityStatus.POOR, QualityStatus.CRITICAL]),
