...
class VisualizationDataProcessor:
    ...
    async def get_entity_network_data(
        ...
        try:
                ...
                try:
                    ...
                    if entity_type:
                        node_query = node_query.filter(
                            StructuredEntityModel.entity_type == entity_type
                        )
                    ...
                    if filters:
                        for key, value in filters.items():
                            if hasattr(StructuredEntityModel, key):
                                node_query = node_query.filter(
                                    getattr(StructuredEntityModel, key) == value
                                )
                    ...
                    if entity_ids:
                        ...
                        relationships = (
                            self.db.query(EntityRelationshipModel)
                            .filter(
                                and_(
                                    EntityRelationshipModel.source_entity_id.in_(
                                        entity_ids
                                    ),
                                    EntityRelationshipModel.target_entity_id.in_(
                                        entity_ids
                                    ),
                                )
                            )
            ...
            return {
                ...
                "edges": [],
                "metadata": {"total_nodes": 1, "total_edges": 0, "error": str(e)},
            }
    ...
    async def get_timeline_data(
        ...
        try:
            if self.db:
                try:
                    from ..storage.models import StructuredEntityModel
                    ...
                    if entity_id:
                        entity_query = entity_query.filter(
                            StructuredEntityModel.entity_id == entity_id
                        )

                    if start_date:
                        entity_query = entity_query.filter(
                            StructuredEntityModel.first_seen_at >= start_date
                        )

                    entities = (
                        entity_query.order_by(
                            StructuredEntityModel.first_seen_at.desc()
                        )
                        .limit(50)
                        .all()
                    )
                    ...
                    for entity in entities:
                        ...
                        if (
                            ...
                            and entity.last_verified_at != entity.first_seen_at
                        ):
                            events.append(
                                {
            ...
            return {"events": [], "groups": groups, "metadata": {"error": str(e)}}
    ...
    async def get_geospatial_data(
        ...
        try:
            ...
            return {"points": [], "bounds": bounds, "metadata": {"error": str(e)}}
            ...
            if self.db:
                try:
                    ...
                    rel_counts = (
                        ...
                        .group_by(EntityRelationshipModel.relationship_type)
                        .all()
                    )

                    total_relationships = 0
                    for rel_type, count in rel_counts:
                        metrics["relationship_counts"]["by_type"][
                            rel_type or "unknown"
                        ] = count
