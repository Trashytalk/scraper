...
class AIProcessor:
        ...
        default_config = {
            "ai": {
                ...
                "entity_types": [
                    ...
                    "PRODUCT", "EVENT", "LAW", "EMAIL", "URL"
                ],
                "classification_categories": [
                    "business", "technology", "finance", "news", 
                    "social", "entertainment", "sports", "politics"
                ],
        ...
        if HAS_TRANSFORMERS:
            try:
                ...
                self.models["sentiment"] = pipeline(
                    "sentiment-analysis",
                    model=self.config["ai"]["models"]["sentiment"]
                )
                
                # Summarization model
                self.models["summarizer"] = pipeline(
                    "summarization",
                    model=self.config["ai"]["models"]["summarization"]
                )
                
                logger.info("Loaded transformer models for classification, sentiment, and summarization")
            except Exception as e:
            ...
            for ent in doc.ents:
                if ent.label_ in self.config["ai"]["entity_types"]:
                    entity = ExtractedEntity(
                        ...
                        metadata={"spacy_kb_id": ent.kb_id_ if hasattr(ent, 'kb_id_') else None}
                    )
    ...
    def classify_text(self, text: str) -> ClassificationResult:
        ...
        try:
            ...
            if isinstance(result, list) and len(result) > 0:
                prediction = result[0]
                category = category_mapping.get(prediction["label"], prediction["label"].lower())
    ...
    def detect_duplicates(self, texts: List[str], threshold: float = None) -> List[List[int]]:
    ...
    def calculate_quality_score(self, data: Dict[str, Any]) -> float:
        ...
        try:
            ...
            present_structured = sum(1 for field in structured_fields if data.get(field))
    ...
    async def enhance_scraped_data(self, scraped_items: List[Dict[str, Any]]) -> List[ProcessedData]:
        ...
        if len(processed_data) > 1:
            texts = [self._extract_text_content(item.original_data) for item in processed_data]
            ...
            for group in duplicate_groups:
                for idx in group[1:]:  # Skip first item in each group
                    if idx < len(processed_data):
                        processed_data[idx].duplicates = [processed_data[group[0]].original_data.get("url", "")]
...
async def process_scraped_data(data: List[Dict[str, Any]], 
                              config_path: str = None) -> List[ProcessedData]:
