            ...
            "text_classifier": ModelConfig(
                ...
                custom_params={
                    "candidate_labels": [
                        ...
                        "job posting", "product", "service", "research"
                    ]
                }
            ),
            ...
            if model.enabled:
                if model.type == ModelType.OPENAI and not model.api_key:
                    ...
                    if not os.getenv('OPENAI_API_KEY'):
                        issues.append(f"OpenAI model {name} enabled but no API key provided")
                
                if model.batch_size <= 0:
                    issues.append(f"Model {name} has invalid batch_size: {model.batch_size}")
        ...
        for name, model in self.config.models.items():
            if model.enabled:
                if model.type == ModelType.OPENAI:
                    ...
                    env_vars['TRANSFORMERS_CACHE'] = os.path.join(self.config.cache_dir, 'transformers')
                elif model.type == ModelType.SENTENCE_TRANSFORMERS:
                    env_vars['SENTENCE_TRANSFORMERS_HOME'] = os.path.join(self.config.cache_dir, 'sentence_transformers')
...
def setup_ai_config():
    ...
    ai_enabled = input(f"Enable AI features? [Y/n]: ").lower() not in ['n', 'no']
    ...
    for name, model in config.models.items():
        ...
        enable = input(f"Enable {name} ({model.type.value})? [{current_status}] [Y/n]: ")
        model.enabled = enable.lower() not in ['n', 'no']
        
        if model.enabled and model.type == ModelType.OPENAI:
            api_key = input(f"OpenAI API key (current: {'set' if model.api_key else 'not set'}): ")
            if api_key:
