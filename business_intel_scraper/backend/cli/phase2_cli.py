...
def setup_dom_change_parser(subparsers):
    ...
    dom_subparsers = parser.add_subparsers(dest='dom_command', help='DOM change commands')
    ...
    list_parser.add_argument('--days', type=int, default=7, help='Show changes from last N days')
    list_parser.add_argument('--output-format', choices=['table', 'json'], default='table',
                           help='Output format')
    
    # Analyze page command
    analyze_parser = dom_subparsers.add_parser('analyze', help='Analyze page DOM structure')
...
def setup_spider_update_parser(subparsers):
    ...
    update_subparsers = parser.add_subparsers(dest='update_command', help='Spider update commands')
    
    # Check and update command
    check_parser = update_subparsers.add_parser('check', help='Check and update spiders')
    check_parser.add_argument('--url', help='Check spiders for specific URL')
    check_parser.add_argument('--severity', choices=['low', 'medium', 'high', 'critical'],
    ...
    check_parser.add_argument('--backup', action='store_true', default=True,
                             help='Create backup before updating (default: True)')
    
    # Status command
    status_parser = update_subparsers.add_parser('status', help='Show spider update status')
    status_parser.add_argument('--days', type=int, default=30, help='Status period in days')
...
def list_dom_changes_command(args):
    ...
    if args.output_format == 'json':
        ...
        print("-" * 120)
        print(f"{'URL':<40} {'Type':<12} {'Severity':<8} {'Date':<16} {'Description':<40}")
        print("-" * 120)
        
        for change in sorted(changes, key=lambda x: x.detected_at, reverse=True):
            url_display = change.url[:37] + "..." if len(change.url) > 40 else change.url
            desc_display = change.description[:37] + "..." if len(change.description) > 40 else change.description
            date_display = change.detected_at.strftime('%Y-%m-%d %H:%M')
            
            print(f"{url_display:<40} {change.change_type:<12} {change.severity:<8} "
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(args.url, timeout=30) as response:
                if response.status == 200:
                    ...
                    top_elements = sorted(fingerprint.element_counts.items(), 
...
def generate_dom_report_command(args):
    ...
    report['top_affected_urls'] = dict(
        sorted(report['top_affected_urls'].items(), key=lambda x: x[1], reverse=True)[:10]
    )
...
async def check_spider_updates_command(args):
    ...
    if args.auto_fix:
        ...
        print(f"📋 Changes that would trigger updates:")
        for change in changes[:10]:  # Show first 10
            print(f"   • {change.url}")
            print(f"     {change.change_type} [{change.severity}]: {change.description}")
            if change.auto_fixable:
                print(f"     ✅ Auto-fixable: {change.suggested_fixes[0] if change.suggested_fixes else 'Generic fix'}")
            else:
    ...
    for session in reversed(history):  # Most recent first
        ...
        if session.get('updates'):
            for update in session['updates'][:3]:  # Show first 3 updates
                print(f"   • {update['spider_name']}: {update['changes_processed']} changes")
        print()
