...
from ..discovery.automated_discovery import (
...
def setup_discovery_parser(subparsers):
    ...
    parser = subparsers.add_parser('discovery', help='Manage automated source discovery')
    
    discovery_subparsers = parser.add_subparsers(dest='discovery_command', help='Discovery commands')
    ...
    list_parser = discovery_subparsers.add_parser('list', help='List discovered sources')
    list_parser.add_argument('--status', choices=['candidate', 'validated', 'failed'], 
    ...
    validate_parser = discovery_subparsers.add_parser('validate', help='Validate discovered sources')
    validate_parser.add_argument('--urls', nargs='*', help='Specific URLs to validate')
    validate_parser.add_argument('--all', action='store_true', help='Validate all candidate sources')
    
    # Generate spiders command
    generate_parser = discovery_subparsers.add_parser('generate', help='Generate spiders from discovered sources')
    generate_parser.add_argument('--min-confidence', type=float, default=0.7,
                               help='Minimum confidence score for spider generation')
    generate_parser.add_argument('--template', choices=['scrapy', 'playwright', 'requests'],
                               default='scrapy', help='Spider template to use')
...
async def run_discovery_command(args):
    ...
    seed_urls = args.urls if args.urls else [
    ...
    try:
        ...
        print(f"   📉 Lower confidence (<0.4): {len(sources) - len(high_confidence) - len(medium_confidence)} sources")
        ...
        if high_confidence:
            ...
            for source in sorted(high_confidence, key=lambda x: x.confidence_score, reverse=True)[:5]:
...
def list_sources_command(args):
    ...
    if args.output_format == 'json':
        ...
        print("-" * 120)
        print(f"{'URL':<50} {'Domain':<25} {'Type':<15} {'Confidence':<10} {'Status':<10}")
        print("-" * 120)
        
        for source in sorted(sources, key=lambda x: x.confidence_score, reverse=True):
            url_display = source.url[:47] + "..." if len(source.url) > 50 else source.url
            domain_display = source.domain[:22] + "..." if len(source.domain) > 25 else source.domain
            
            print(f"{url_display:<50} {domain_display:<25} {source.source_type:<15} "
...
def generate_spiders_command(args):
    ...
    print(f"🕷️  Generating spiders from high-confidence sources...")
...
"""
    ...
    for i, source in enumerate(sources[:5]):  # Limit to 5 spiders for demo
        ...
        if 'api_endpoints' in source.metadata:
            extraction_logic += "# API endpoints discovered: " + ', '.join(source.metadata['api_endpoints'][:3])
