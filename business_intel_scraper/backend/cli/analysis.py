...
def comprehensive(
    input_file, output_file, analysis_types, confidence_threshold, format, verbose
):
    ...
    try:
        ...
        with click.progressbar(length=100, label="Running analysis") as bar:
            result = asyncio.run(
                orchestrator.run_comprehensive_analysis(analysis_request)
            )
            bar.update(100)
        ...
        click.echo(
            f"  - Duration: {result.metrics.get('duration_seconds', 0):.2f} seconds"
        )
        click.echo(
            f"  - Entities processed: {result.metrics.get('entities_processed', 0)}"
        )
        click.echo(
            f"  - Risk level: {result.summary.get('risk_assessment', 'UNKNOWN')}"
        )
...
@click.option(
    "--output-file", "-o", type=click.Path(), help="Output file for entity clusters"
)
@click.option(
...
@click.option(
    "--clustering-method",
    "-m",
    type=click.Choice(["dbscan", "hierarchical"]),
    default="dbscan",
...
@click.option(
    "--output-file", "-o", type=click.Path(), help="Output file for relationships"
)
@click.option(
    "--relationship-types",
    "-r",
...
def map_relationships(
    ...
    try:
        ...
        with click.progressbar(length=100, label="Mapping relationships") as bar:
            relationships = mapper.extract_relationships(
                entities, list(relationship_types)
            )
            bar.update(100)
...
@click.option(
    "--output-file", "-o", type=click.Path(), help="Output file for enrichment results"
)
@click.option(
    "--sources",
    "-s",
...
@click.option(
    "--max-cost", default=10.0, type=float, help="Maximum cost for enrichment requests"
)
@click.option(
    "--format",
    "-f",
    ...
    try:
        ...
        with click.progressbar(length=100, label="Enriching entities") as bar:
            enrichments = asyncio.run(
                enrichment_engine.enrich_entities(entities, list(sources))
            )
            bar.update(100)
        ...
        if total_cost > max_cost:
            click.echo(
                f"Warning: Total cost ${total_cost:.2f} exceeds maximum ${max_cost:.2f}"
            )
        ...
        high_risk_enrichments = [
            e for e in enrichments if "sanctioned" in str(e.data).lower()
        ]
...
@click.option(
    "--output-file", "-o", type=click.Path(), help="Output file for detected events"
)
@click.option(
    "--entity-names", "-e", multiple=True, help="Entity names to focus detection on"
)
@click.option(
    "--severity-filter",
        ...
        with click.progressbar(length=100, label="Detecting events") as bar:
            events = asyncio.run(
                detector.detect_events(
                    data_sources, list(entity_names) if entity_names else None
                )
            )
            bar.update(100)
    ...
    if file_path.suffix.lower() == ".json":
        with open(file_path, "r") as f:
            ...
            if isinstance(data, list):
                ...
                raise ValueError(
                    "Invalid JSON format: expected list or object with 'entities' key"
                )
        ...
        if verbose:
            ...
            if result.entity_resolutions:
                ...
                for resolution in result.entity_resolutions[:10]:  # Limit to 10
                    click.echo(
                        f"  - Cluster {resolution['cluster_id']}: {len(resolution['entities'])} entities"
                    )
            ...
            if result.enrichments:
                ...
                for enrich in result.enrichments[:10]:  # Limit to 10
                    click.echo(
                        f"  - {enrich['entity_id']}: {enrich['source_name']} ({enrich['enrichment_type']})"
                    )
