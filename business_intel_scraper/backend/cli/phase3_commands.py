...
@click.option(
    ...
    help="HTML file to analyze instead of fetching URL",
)
@click.option(
...
def analyze_content(
    url: str, html_file: Optional[str], output: Optional[str], detailed: bool
) -> None:
    ...
    with Progress(
        ...
        try:
            ...
            async def analyze():
                ...
                quality_predictions = await content_analyzer.predict_content_quality(
                    features
                )
    ...
    table = Table(
        title="Content Features", show_header=True, header_style="bold magenta"
    )
    ...
    if quality_predictions:
        quality_table = Table(
            title="Quality Predictions", show_header=True, header_style="bold green"
        )
        ...
        for metric, data in quality_predictions.items():
            if isinstance(data, dict):
                ...
                quality_table.add_row(
                    metric.replace("_", " ").title(), str(data), "N/A"
                )
    ...
    if detailed:
        ...
        if features.entities:
            entity_table = Table(
                title="Named Entities", show_header=True, header_style="bold blue"
            )
        ...
        if features.topics:
            topic_table = Table(
                title="Topic Analysis", show_header=True, header_style="bold purple"
            )
...
@click.option(
    "--format",
    "data_format",
    type=click.Choice(["json", "csv", "raw"]),
    default="json",
    help="Data format",
)
@click.option(
    "--output", "-o", type=click.Path(), help="Output file for quality report"
)
...
def assess_quality(
    ...
    score_color = (
        "green" if overall_score >= 0.8 else "yellow" if overall_score >= 0.6 else "red"
    )
    ...
    quality_table = Table(
        title="Quality Metrics", show_header=True, header_style="bold magenta"
    )
    quality_table.add_column("Metric", style="cyan")
    quality_table.add_column("Score", style="yellow")
    ...
    quality_table.add_row(
        "Completeness",
        f"{metrics.completeness_score:.3f}",
        f"{metrics.completeness_details}",
    )
    quality_table.add_row(
        "Consistency",
        f"{metrics.consistency_score:.3f}",
        f"{metrics.consistency_details}",
    )
    quality_table.add_row(
        "Validity", f"{metrics.validity_score:.3f}", f"{metrics.validity_details}"
    )
    quality_table.add_row(
        "Accuracy", f"{metrics.accuracy_score:.3f}", f"{metrics.accuracy_details}"
    )
    quality_table.add_row(
        "Uniqueness", f"{metrics.uniqueness_score:.3f}", f"{metrics.uniqueness_details}"
    )
    quality_table.add_row(
        "Timeliness", f"{metrics.timeliness_score:.3f}", f"{metrics.timeliness_details}"
    )
    ...
    if metrics.anomalies_detected:
        anomaly_table = Table(
            title="Anomalies Detected", show_header=True, header_style="bold red"
        )
    ...
    if detailed:
        ...
        if hasattr(metrics, "statistical_overview"):
            stats_table = Table(
                title="Statistical Overview", show_header=True, header_style="bold blue"
            )
...
@click.option(
    "--output", "-o", type=click.Path(), help="Output file for learned patterns"
)
def learn_patterns(session_file: str, output: Optional[str]) -> None:
    ...
    with Progress(
        ...
        try:
            ...
            # Import Phase 3 components
            from ..discovery.intelligent_pattern_recognition import (
                pattern_recognizer,
                LearningSession,
            )
            ...
            session = LearningSession(
                ...
                response_characteristics=session_data.get(
    ...
    if learned_patterns:
        pattern_table = Table(
            title="Learned Patterns", show_header=True, header_style="bold purple"
        )
        ...
        stats_table = Table(
            title="Pattern Statistics", show_header=True, header_style="bold blue"
        )
        stats_table.add_column("Statistic", style="cyan")
        stats_table.add_column("Value", style="yellow")
...
@click.option(
    "--max-sources", type=int, default=10, help="Maximum number of sources to discover"
)
@click.option(
    "--output", "-o", type=click.Path(), help="Output file for discovered sources"
)
def discover_sources(seed_url: str, max_sources: int, output: Optional[str]) -> None:
    ...
    with Progress(
        ...
        try:
            # Import Phase 3 components
            from ..discovery.ml_content_analysis import (
                content_analyzer,
            ...
            async def discover():
                    ...
                    async with session.get(seed_url) as response:
                        if response.status == 200:
                            ...
                            content_features = await content_analyzer.analyze_content(
                                seed_url, html_content
                            )
                        else:
                            raise Exception(
                                f"Failed to fetch seed URL: HTTP {response.status}"
                            )
                ...
                discovered_sources = (
                    await predictive_discovery.discover_related_sources(
    ...
    if discovered_sources:
        source_table = Table(
            title="Discovered Sources", show_header=True, header_style="bold green"
        )
...
@click.option(
    "--output", "-o", type=click.Path(), help="Output file for optimization results"
)
def optimize_selectors(url: str, selectors: tuple, output: Optional[str]) -> None:
        ...
        try:
            ...
            async def optimize():
                ...
                strategy_recommendation = (
                    await pattern_recognizer.recommend_extraction_strategy(
            ...
            content_features, strategy_recommendation, optimized_selectors = (
    ...
    comparison_table = Table(
        title="Selector Optimization", show_header=True, header_style="bold blue"
    )
...
def run_learning_cycle() -> None:
    ...
    stats_table = Table(
        title="Learning Cycle Results", show_header=True, header_style="bold purple"
    )
    stats_table.add_column("Statistic", style="cyan")
    ...
    # Learning summary
    patterns_change = stats_after.get("total_patterns", 0) - stats_before.get(
        "total_patterns", 0
    )
    if patterns_change > 0:
        ...
        console.print(
            f"[yellow]🔄 {abs(patterns_change)} patterns refined/removed[/yellow]"
        )
    else:
        console.print(
            "[blue]ℹ️ No new patterns learned - existing patterns optimized[/blue]"
        )
...
@click.option(
    "--output", "-o", type=click.Path(), help="Output file for insights report"
)
def generate_insights(days: int, output: Optional[str]) -> None:
    ...
    with Progress(
        ...
        try:
            ...
            analysis_stats = (
                content_analyzer.get_analysis_statistics() if content_analyzer else {}
            )
            ...
            insights = {
                ...
                "content_analysis": {
                    ...
                    "status": (
                        "Active"
                        if analysis_stats.get("total_analyzed", 0) > 0
                        else "Inactive"
                    ),
                },
                "quality_assessment": {
    ...
    overview_table = Table(
        title="Phase 3 System Overview", show_header=True, header_style="bold cyan"
    )
    ...
    if pattern_stats:
        pattern_table = Table(
            title="Pattern Recognition Details",
            show_header=True,
...
def status() -> None:
    ...
    try:
        ...
        if pattern_recognizer is not None:
            ...
            health_items.append(
                f"  • Learning Sessions: {stats.get('learning_sessions', 0)}"
            )
        else:
        ...
        try:
            ...
            health_items.append(
                "[yellow]⚠️ scikit-learn[/yellow] - Not available (fallback mode)"
            )
