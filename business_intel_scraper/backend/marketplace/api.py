...
from . import SpiderMarketplace, SpiderInfo
...
async def get_spider_info(spider_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get spider info: {str(e)}")
...
async def get_featured_spiders(limit: int = Query(10, le=20)) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        featured = sorted(
            all_spiders,
            key=lambda x: (x.get('verified', False), x.get('rating', 0), x.get('downloads', 0)),
            reverse=True
        )
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get featured spiders: {str(e)}")
...
async def get_my_spiders() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get user spiders: {str(e)}")
