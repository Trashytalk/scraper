...
class Phase3Demo:
    ...
    async def demo_ml_content_analysis(self) -> None:
        ...
        with Progress(
            ...
            try:
                ...
                if content_analyzer is None:
                    ...
                    console.print(
                        "[yellow]💡 Install scikit-learn, pandas, numpy for full functionality[/yellow]"
                    )
                    return
                ...
                features = await content_analyzer.analyze_content(
                    self.sample_urls[0], self.sample_html
                )
        ...
        features_table = Table(
            title="Extracted Content Features",
            show_header=True,
        ...
        if quality_predictions:
            ...
            for metric, prediction in quality_predictions.items():
                if isinstance(prediction, dict):
                    ...
                    quality_table.add_row(
                        metric.replace("_", " ").title(), str(prediction)
                    )
        ...
        self.results["ml_content_analysis"] = {
            ...
            "quality_predictions": (
                len(quality_predictions) if quality_predictions else 0
            ),
            "success": True,
        }
    ...
    async def demo_data_quality_assessment(self) -> None:
        ...
        with Progress(
            ...
            try:
                ...
                metrics = await quality_assessor.assess_data_quality(
                    self.sample_data, "demo://sample-business-data"
                )
        ...
        score_color = (
            ...
            else "yellow" if overall_score >= 0.6 else "red"
        )
        console.print(
        ...
        quality_table = Table(
            title="Quality Assessment Metrics",
            show_header=True,
            header_style="bold magenta",
        )
        ...
        quality_table.add_row(
            ...
            "✅ Good" if metrics.completeness_score >= 0.7 else "⚠️ Needs attention",
        )
        quality_table.add_row(
            "Consistency",
            f"{metrics.consistency_score:.3f}",
            "✅ Good" if metrics.consistency_score >= 0.7 else "⚠️ Needs attention",
        )
        quality_table.add_row(
            "Validity",
            f"{metrics.validity_score:.3f}",
            "✅ Good" if metrics.validity_score >= 0.7 else "⚠️ Needs attention",
        )
        quality_table.add_row(
            "Accuracy",
            f"{metrics.accuracy_score:.3f}",
            "✅ Good" if metrics.accuracy_score >= 0.7 else "⚠️ Needs attention",
        )
        quality_table.add_row(
            "Uniqueness",
            f"{metrics.uniqueness_score:.3f}",
            "✅ Good" if metrics.uniqueness_score >= 0.7 else "⚠️ Needs attention",
        )
        quality_table.add_row(
            "Timeliness",
            f"{metrics.timeliness_score:.3f}",
            "✅ Good" if metrics.timeliness_score >= 0.7 else "⚠️ Needs attention",
        )
        ...
        if metrics.anomalies_detected:
            anomaly_table = Table(
                title="Detected Anomalies", show_header=True, header_style="bold red"
            )
    ...
    async def demo_intelligent_pattern_recognition(self) -> None:
        ...
        with Progress(
            ...
            try:
                ...
                session = LearningSession(
                    ...
                    failed_selectors=[".nonexistent-selector", "div.missing-class"],
                    adaptations_made=[
        ...
        console.print(
            f"[bold]Learned {len(learned_patterns)} patterns from demo session[/bold]"
        )

        # Pattern statistics table
        stats_table = Table(
            title="Pattern Recognition Statistics",
            show_header=True,
        ...
        if learned_patterns:
            ...
            for pattern in learned_patterns[:5]:  # Show first 5 patterns
                pattern_table.add_row(
                    (
                        pattern.pattern_id[:15] + "..."
                        if len(pattern.pattern_id) > 15
    ...
    async def demo_predictive_discovery(self) -> None:
            ...
            try:
                ...
                discovered_sources = (
                    await predictive_discovery.discover_related_sources(
                        self.sample_urls[0], content_features, max_sources=8
                    )
                )
        ...
        console.print(
            f"[bold]Discovered {len(discovered_sources)} related sources[/bold]"
        )

        # Display discovered sources
        if discovered_sources:
            source_table = Table(
                title="Discovered Related Sources",
                show_header=True,
                header_style="bold green",
            )
            ...
            console.print(
                "[yellow]⚠️ No related sources discovered in demo mode[/yellow]"
            )
    ...
    async def demo_system_integration(self) -> None:
        ...
        task_table = Table(
            title="Available Phase 3 Tasks", show_header=True, header_style="bold blue"
        )
        ...
        cli_table = Table(
            title="Phase 3 CLI Commands", show_header=True, header_style="bold purple"
        )
    ...
    def generate_demo_summary(self) -> None:
        ...
        overview_table = Table(
            title="Demo Overview", show_header=True, header_style="bold magenta"
        )
        ...
        overview_table.add_row(
            ...
            (
                "100%"
                if all(r.get("success", False) for r in self.results.values())
                else "Partial"
            ),
        )
        overview_table.add_row(
            "ML Components Active", "Yes" if self.components_tested else "No"
        )
        ...
        console.print(
            "  3. Start using ML-powered content analysis and quality assessment"
        )
        console.print(
            "  4. Enable adaptive pattern learning for your extraction workflows"
        )
        console.print(
            "  5. Explore predictive source discovery for automated expansion"
        )
