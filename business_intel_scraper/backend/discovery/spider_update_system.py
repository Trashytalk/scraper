...
class SpiderTemplate:
    ...
    def save_updated_spider(self, backup: bool = True) -> bool:
        ...
        try:
            if backup:
                # Create backup
                backup_path = self.filepath.with_suffix(f'.backup_{datetime.now().strftime("%Y%m%d_%H%M%S")}.py')
...
class SpiderUpdater:
    ...
    def __init__(self, spiders_dir: Optional[Path] = None):
        self.spiders_dir = spiders_dir or Path('business_intel_scraper/backend/modules/spiders')
        self.generated_spiders_dir = Path('business_intel_scraper/backend/discovery/generated')
        ...
        self.selector_mappings = {
            ...
            '.listing': ['.business-item', '.company-listing', '.directory-entry', '.result-item'],
            ...
            'input[name="query"]': ['input[name="search"]', 'input[name="q"]', 'input[type="search"]'],
            'input[name="location"]': ['input[name="loc"]', 'input[name="address"]', 'input[name="city"]']
        }
        ...
        for search_dir in search_dirs:
            ...
            for spider_file in search_dir.glob('*.py'):
                try:
                    ...
                    if (url in content or 
                        domain in content or 
                        any(domain_part in content for domain_part in domain.split('.'))):
    ...
    async def update_spiders_for_changes(self, changes: List[DOMChange]) -> Dict[str, Any]:
    ...
    async def update_spider_for_url_changes(self, spider: SpiderTemplate, url: str, changes: List[DOMChange]) -> Dict[str, Any]:
            ...
            for change in changes:
                if change.auto_fixable and change.suggested_fixes:
                    ...
                    if fix_applied:
                        ...
                        update_result['manual_actions_needed'].append(change.suggested_fixes[0])
                else:
                    # Manual review needed
                    update_result['manual_actions_needed'].extend(change.suggested_fixes)
                    update_result['warnings'].append(
    ...
    async def apply_automatic_fix(self, spider: SpiderTemplate, change: DOMChange) -> bool:
        """Apply automatic fix for a DOM change"""
        
        try:
            if change.change_type == 'structure' and 'selector' in change.description.lower():
                ...
                logger.info(f"No automatic fix available for change type: {change.change_type}")
                return False
    ...
    async def fix_missing_selectors(self, spider: SpiderTemplate, change: DOMChange) -> bool:
        ...
        spider.current_code = spider.current_code.replace(old_xpath_pattern, new_xpath_pattern)
    ...
    def find_replacement_selectors(self, missing_selector: str, new_fingerprint: DOMFingerprint) -> List[str]:
        ...
        for available_selector in available_selectors:
            similarity = self.calculate_selector_similarity(missing_selector, available_selector)
            if similarity > 0.5:
    ...
    def calculate_selector_similarity(self, sel1: str, sel2: str) -> float:
        ...
        terms1 = set(sel1.lower().replace('.', ' ').replace('#', ' ').replace('[', ' ').replace(']', ' ').split())
        terms2 = set(sel2.lower().replace('.', ' ').replace('#', ' ').replace('[', ' ').replace(']', ' ').split())
    ...
    async def fix_form_changes(self, spider: SpiderTemplate, change: DOMChange) -> bool:
        ...
        if 'action changed' in change.description:
            ...
            if len(parts) == 2:
                ...
                spider.current_code = spider.current_code.replace(
                    f"FormRequest(url='{old_action}'",
                    f"FormRequest(url='{new_action}'"
                )
            ...
            missing_fields_match = re.search(r'fields removed: ([^"]+)', change.description)
    ...
    async def fix_content_changes(self, spider: SpiderTemplate, change: DOMChange) -> bool:
        """Fix content-related changes"""
        
        if 'count changed significantly' in change.description:
            # Add robustness to element counting
            element_match = re.search(r'(\w+) count changed significantly', change.description)
            if element_match:
    ...
    async def fix_api_changes(self, spider: SpiderTemplate, change: DOMChange) -> bool:
        ...
        if 'endpoints removed' in change.description:
            ...
            endpoints_match = re.search(r'endpoints removed: ([^"]+)', change.description)
            if endpoints_match:
    ...
    def record_update_session(self, update_results: Dict[str, Any]) -> None:
        ...
        session_record = {
            ...
            'success_rate': (update_results['spiders_updated'] / max(update_results['total_changes'], 1)) * 100,
        ...
        logger.info(f"Recorded update session: {update_results['spiders_updated']} spiders updated")
    ...
    def get_update_statistics(self, days: int = 30) -> Dict[str, Any]:
        ...
        recent_sessions = [
            session for session in self.update_history
        ...
        return {
            ...
            'most_common_change_types': dict(sorted(change_types.items(), key=lambda x: x[1], reverse=True))
        }
...
class SpiderUpdateScheduler:
    ...
    async def check_and_update_spiders(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if not all_changes:
                return {'status': 'no_changes', 'message': 'No critical or high-severity changes found'}
            
            logger.info(f"Processing {len(all_changes)} high-priority DOM changes")
            
            # Update spiders
            update_results = await self.spider_updater.update_spiders_for_changes(all_changes)
    ...
    async def scheduled_update_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if result.get('status') == 'no_changes':
                medium_changes = self.dom_detector.get_changes_by_severity('medium', days=3)
                
                if medium_changes:
                    logger.info(f"Processing {len(medium_changes)} medium-priority DOM changes")
                    update_results = await self.spider_updater.update_spiders_for_changes(medium_changes)
...
'''
    ...
    print(f"📊 Spider Analysis:")
    print(f"   Selectors found: {len(spider_template.selectors)}")
    print(f"   Field mappings: {len(spider_template.extraction_patterns['field_mappings'])}")
    ...
    new_fp = DOMFingerprint(
        ...
        key_selectors={'.business-item': 'ABC Company|XYZ Corp|New Business', '.company-name': 'ABC Company'},
    )
    ...
    if updated_content != demo_spider_content:
        ...
        print(f"   Original selectors: .listing")
        print(f"   Updated selectors: .business-item")
    else:
