...
class AdaptiveLinkClassifier:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.feedback_links: List[Tuple[LinkInfo, bool, float]] = []  # link, success, value
        ...
        logger.info(f"AdaptiveLinkClassifier initialized with ML support: {SKLEARN_AVAILABLE}")
    ...
    def _initialize_ml_components(self) -> None:
        ...
        model_path = Path(self.config.get('model_path', 'data/models/link_classifier.pkl'))
    ...
    def classify_link(self, url: str, anchor_text: str, parent_url: str = "", context: str = "") -> LinkInfo:
    ...
    def _extract_link_features(self, link_info: LinkInfo, context: str = "") -> Dict[str, Any]:
        ...
        try:
            ...
            features.update({
                ...
                'is_html': path_lower.endswith(('.html', '.htm')) or not any(path_lower.endswith(ext) for ext in ['.pdf', '.doc', '.docx', '.xls', '.xlsx']),
            })
            
            # Text-based features
            combined_text = f"{link_info.anchor_text} {parsed_url.path} {context}".lower()
            ...
            for category, patterns in self.business_patterns.items():
                ...
                features[f'{category}_score'] = score / len(patterns) if patterns else 0.0
    ...
    def _ml_classify(self, features: Dict[str, Any], anchor_text: str, url: str) -> Tuple[LinkCategory, float]:
    ...
    def _rule_based_classify(self, features: Dict[str, Any], anchor_text: str, url: str) -> Tuple[LinkCategory, float]:
    ...
    def _assign_priority(self, category: LinkCategory, confidence: float) -> LinkPriority:
    ...
    def add_training_data(self, link_info: LinkInfo, actual_category: LinkCategory) -> None:
    ...
    def add_feedback(self, link_info: LinkInfo, was_successful: bool, extracted_value: float) -> None:
    ...
    def _prepare_feature_vector(self, features: Dict[str, Any]) -> Optional[List[float]]:
        ...
        try:
            ...
            expected_features = [
                'url_length', 'path_segments', 'has_query', 'has_fragment', 'is_https',
                'subdomain_count', 'is_pdf', 'is_doc', 'is_excel', 'is_image', 'is_html',
                'business_profile_score', 'financial_data_score', 'company_news_score',
                'regulatory_filing_score', 'contact_info_score', 'product_service_score',
                'legal_document_score', 'social_media_score', 'anchor_length', 'anchor_words',
                'anchor_has_numbers', 'anchor_all_caps', 'is_gov_domain', 'is_edu_domain',
                'is_org_domain', 'is_com_domain', 'domain_length'
            ]
    ...
    def _load_initial_training_data(self) -> None:
        ...
        initial_samples = [
            ...
            ("https://company.com/investors", "Investor Relations", LinkCategory.FINANCIAL_DATA),
            ("https://company.com/news/press-release", "Latest News", LinkCategory.COMPANY_NEWS),
            ("https://company.com/contact", "Contact Us", LinkCategory.CONTACT_INFO),
            ("https://company.com/privacy-policy", "Privacy Policy", LinkCategory.LEGAL_DOCUMENT),
        ]
    ...
    def _retrain_model(self) -> None:
        ...
        if not SKLEARN_AVAILABLE or len(self.training_links) < self.min_training_samples:
            return
    ...
    def _incorporate_feedback(self) -> None:
        ...
        for link_info, was_successful, extracted_value in self.feedback_links:
            if was_successful and extracted_value > 0.5:
                ...
                if link_info.category in [LinkCategory.UNKNOWN, LinkCategory.IRRELEVANT]:
    ...
    def get_classifier_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'category_distribution': {
                cat.value: sum(1 for link in self.training_links if link.category == cat)
                for cat in LinkCategory
            } if self.training_links else {}
        }
