...
from .ml_content_analysis import ContentFeatures
...
class LearningSession:
    ...
    # Learning data
    successful_selectors: Dict[str, float] = field(
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "content_features": (
                self.content_features.to_dict() if self.content_features else None
            ),
...
class IntelligentPatternRecognizer:
    ...
    async def learn_from_session(
        ...
        try:
            ...
            self.logger.info(
                f"Learned {len(discovered_patterns)} new patterns from session {session.session_id}"
            )
    ...
    async def _discover_structural_patterns(
        ...
        try:
            ...
            selector_groups = self._group_similar_selectors(
                session.successful_selectors
            )
            ...
            for group_selectors in selector_groups:
                if len(group_selectors) >= 2:  # Need multiple similar selectors
                    ...
                    success_rates = [
                        session.successful_selectors[sel] for sel in group_selectors
                    ]
                    ...
                    if avg_success_rate > 0.7:  # Only patterns with good success rate
                        ...
                        if session.content_features:
                            pattern.content_types.add(
                                session.content_features.content_type
                            )
                            pattern.business_categories.add(
                                session.content_features.business_category
                            )
                        ...
                        pattern.error_rate = session.errors_encountered / max(
                            session.records_extracted, 1
                        )
    ...
    async def _discover_semantic_patterns(
        ...
        try:
            ...
            for pattern in self.patterns.values():
                if (
                    pattern.pattern_type == "semantic"
                    and features.content_type in pattern.content_types
                    and features.business_category in pattern.business_categories
                ):
            ...
            if existing_pattern:
                ...
                existing_pattern.avg_extraction_time = (
                    1 - alpha
                ) * existing_pattern.avg_extraction_time + alpha * session.duration
                ...
                pattern = ExtractionPattern(
                    ...
                    success_rate=sum(session.successful_selectors.values())
        ...
        try:
            ...
            if session.adaptations_made:
                ...
                pattern = ExtractionPattern(
                    ...
                    selectors=[],
                    success_rate=1
                    - (session.errors_encountered / max(session.records_extracted, 1)),
                    usage_count=1,
                )
    ...
    async def _update_existing_patterns(self, session: LearningSession):
        ...
        try:
            for pattern in self.patterns.values():
                ...
                if session.content_features:
                    if (
                        session.content_features.content_type in pattern.content_types
                        or session.content_features.business_category
                        in pattern.business_categories
                    ):
                        applicable = True
                ...
                if applicable:
                    ...
                    pattern.avg_extraction_time = (
                        1 - alpha
                    ) * pattern.avg_extraction_time + alpha * session.duration
                    pattern.avg_data_quality = (
                        (1 - alpha) * pattern.avg_data_quality
                        + alpha * session.data_quality_score
                    )

                    session_error_rate = session.errors_encountered / max(
                        session.records_extracted, 1
                    )
                    pattern.error_rate = (
                        1 - alpha
                    ) * pattern.error_rate + alpha * session_error_rate

                    # Update success rate based on session performance
                    session_success_rate = 1 - session_error_rate
                    pattern.success_rate = (
                        1 - alpha
                    ) * pattern.success_rate + alpha * session_success_rate

                    # Update confidence based on success rate and usage
                    usage_factor = min(
                        1.0, pattern.usage_count / 10
    ...
    async def _train_pattern_models(self):
        ...
        try:
                ...
                if session.content_features:
                    ...
                    success_score = (
                        ...
                        + (
                            1
                            - session.errors_encountered
                            / max(session.records_extracted, 1)
                        )
                        * 0.6
                    )
                    labels.append(
                        "successful" if success_score > 0.7 else "needs_improvement"
                    )
            ...
            if len(features) >= 10:
                ...
                self.logger.info(
                    f"Pattern classifier trained with accuracy: {accuracy:.3f}"
                )
    ...
    def _selectors_similar(self, sel1: str, sel2: str) -> bool:
        ...
        sel1_parts = set(
            sel1.replace(".", " ").replace("#", " ").replace(">", " ").split()
        )
        sel2_parts = set(
            sel2.replace(".", " ").replace("#", " ").replace(">", " ").split()
        )
    ...
    async def recommend_extraction_strategy(
        self, url: str, content_features: ContentFeatures
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if applicable_patterns:
                ...
                applicable_patterns.sort(
                    key=lambda p: (p.confidence * p.success_rate), reverse=True
                )

                # Take top patterns
                top_patterns = applicable_patterns[:3]
                recommendations["recommended_patterns"] = [
                    p.pattern_id for p in top_patterns
                ]
                ...
                recommendations["suggested_selectors"] = [
                    sel for sel, score in sorted_selectors[:10]
                ]

                # Overall confidence
                recommendations["confidence"] = sum(
                    p.confidence for p in top_patterns
                ) / len(top_patterns)

                # Determine extraction approach
                pattern_types = [p.pattern_type for p in top_patterns]
                if (
                    "structural" in pattern_types
                    and len([p for p in top_patterns if p.pattern_type == "structural"])
                    >= 2
                ):
                ...
                avg_quality = sum(p.avg_data_quality for p in top_patterns) / len(
                    top_patterns
                )
                avg_time = sum(p.avg_extraction_time for p in top_patterns) / len(
                    top_patterns
                )
                avg_error_rate = sum(p.error_rate for p in top_patterns) / len(
                    top_patterns
                )
    ...
    async def _get_ml_recommendation(
        ...
        try:
            ...
            probability = self.pattern_classifier.predict_proba(feature_vector_scaled)[
                0
            ]
            ...
            return {
                ...
                "recommendation": (
                    "Use high-confidence selectors"
                    if prediction == "successful"
                    else "Apply cautious extraction with validation"
                ),
            }
    ...
    async def optimize_selectors(
        self, selectors: List[str], url: str, content_features: ContentFeatures
        ...
        try:
            stats = {
                ...
                "pattern_types": Counter(
                    p.pattern_type for p in self.patterns.values()
                ),
                "avg_confidence": (
                ...
                "domains_covered": len(
                    set().union(*(p.applicable_domains for p in self.patterns.values()))
                ),
                "content_types_covered": len(
                    set().union(*(p.content_types for p in self.patterns.values()))
                ),
                "business_categories_covered": len(
                    set().union(
                        *(p.business_categories for p in self.patterns.values())
                    )
                ),
            }

            # Pattern performance distribution
            high_perf_patterns = sum(
                1 for p in self.patterns.values() if p.success_rate > 0.8
            )
            medium_perf_patterns = sum(
                1 for p in self.patterns.values() if 0.5 <= p.success_rate <= 0.8
            )
            low_perf_patterns = sum(
                1 for p in self.patterns.values() if p.success_rate < 0.5
            )
            ...
            stats["recent_activity"] = {
                ...
                "total_records_extracted": sum(
                    s.records_extracted for s in recent_sessions
                ),
                "avg_session_quality": (
                    sum(s.data_quality_score for s in recent_sessions)
                    / len(recent_sessions)
                    if recent_sessions
                    else 0
                ),
        ...
        try:
            ...
            for pattern_id in list(self.patterns.keys()):
                ...
                if (
                    ...
                    and pattern.usage_count < 3
                    and datetime.utcnow() - pattern.discovered_at > timedelta(days=30)
                ):
            ...
            self.logger.info(
                f"Adaptive learning cycle complete: removed {removed_count} patterns, consolidated {consolidated_count}"
            )
    ...
    async def _merge_patterns(self, patterns: List[ExtractionPattern]):
        ...
        try:
            ...
            best_pattern = max(
                patterns, key=lambda p: p.confidence * p.success_rate * p.usage_count
            )
            ...
            for pattern in patterns:
                ...
                best_pattern.avg_data_quality = (
                    ...
                    + pattern.avg_data_quality * pattern.usage_count
                ) / total_usage
