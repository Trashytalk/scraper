...
from business_intel_scraper.backend.discovery.marketplace_integration import MarketplaceIntegration
...
async def demo_discovery_system():
    ...
    try:
        domain_sources = await domain_bot.discover(seed_urls[:1])  # Just one URL for demo
        ...
        print(f"   [Demo] Would discover {len(domain_sources)} sources from domain scanning")
    ...
    analysis_result = heuristic_bot.analyze_content('https://example.gov/business', mock_html)
    print(f"   Content analysis confidence: {analysis_result:.2f}")
    print(f"   Detected: API endpoints, forms, contact information")
    ...
    try:
        ...
        discovered_sources = [
            ...
            DiscoveredSource(
                ...
                status='candidate',
                metadata={
                    'business_indicators': {'has_regulations': True, 'multi_language': True},
                    'mock_demo': True
                }
            ),
            DiscoveredSource(
    ...
    medium_confidence = [s for s in discovered_sources if 0.6 <= s.confidence_score <= 0.8]
    ...
    print(f"📋 Registry contains {len(all_sources)} unique sources (deduplication working)")
    ...
    print(f"🎯 Discovery Session Summary:")
    ...
    print(f"   Average confidence score: {sum(s.confidence_score for s in discovered_sources) / len(discovered_sources):.2f}")
    print(f"   Sources with API endpoints: {len([s for s in discovered_sources if s.metadata.get('api_endpoints')])}")
    print(f"   Sources with forms: {len([s for s in discovered_sources if s.metadata.get('forms')])}")
    print(f"   Government sources: {len([s for s in discovered_sources if 'gov' in s.domain])}")
    print(f"   Sources validated: {len([s for s in discovered_sources if s.status == 'validated'])}")
    ...
    print(f"\n🤖 Bot Performance:")
    for bot_name, stats in bot_performance.items():
        print(f"   {bot_name}: {stats['count']} sources, avg confidence {stats['avg_confidence']:.2f}")
    ...
    print("   python -m business_intel_scraper.backend.cli.main discovery run --urls https://example.com")
    print("   python -m business_intel_scraper.backend.cli.main discovery list --min-confidence 0.7")
    print("   python -m business_intel_scraper.backend.cli.main discovery validate --all")
    print("   python -m business_intel_scraper.backend.cli.main discovery generate --template scrapy")
    ...
    return {
        ...
        'validated_sources': len([s for s in discovered_sources if s.status == 'validated']),
...
def main():
    ...
    try:
        ...
        print(f"\n📊 Final Results:")
        print(f"   Discovery Success: {'✅ Yes' if results['discovery_complete'] else '❌ No'}")
