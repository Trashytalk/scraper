...
class CrawlGraphAnalyzer:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        logger.info(f"CrawlGraphAnalyzer initialized with NetworkX support: {NETWORKX_AVAILABLE}")
    ...
    def analyze_crawl_patterns(self, force_refresh: bool = False) -> Dict[str, Any]:
        ...
        if (not force_refresh and 
            ...
            time.time() - self.last_analysis < self.cache_ttl and
            self.analysis_cache):
    ...
    def _get_graph_statistics(self) -> Dict[str, Any]:
        ...
        # Success metrics
        successful_nodes = [n for n in nodes if self.graph.nodes[n].get('success', False)]
        stats.update({
        ...
        response_times = [self.graph.nodes[n].get('response_time', 0.0) for n in nodes if self.graph.nodes[n].get('response_time', 0.0) > 0]
        if response_times:
            stats.update({
    ...
    def _analyze_centrality(self) -> Dict[str, Any]:
        ...
        try:
            ...
            top_pagerank = sorted(centrality_scores.items(), 
                                key=lambda x: x[1]['pagerank'], 
                                reverse=True)[:10]
            
            top_composite = sorted(centrality_scores.items(),
                                 key=lambda x: x[1]['composite_score'],
                                 reverse=True)[:10]
            
            return {
                'top_pagerank_nodes': [(url, scores['pagerank']) for url, scores in top_pagerank],
                'top_composite_nodes': [(url, scores['composite_score']) for url, scores in top_composite],
    ...
    def _detect_communities(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for i, community in enumerate(communities):
                ...
                successful_nodes = sum(1 for n in community_nodes 
                                     if self.graph.nodes[n].get('success', False))
                total_value = sum(self.graph.nodes[n].get('data_value', 0.0) 
                                for n in community_nodes)
                
                community_analysis.append({
                    'community_id': i,
                    'size': len(community_nodes),
                    'success_rate': successful_nodes / len(community_nodes) if community_nodes else 0.0,
                    'total_value': total_value,
                    'avg_value': total_value / len(community_nodes) if community_nodes else 0.0,
    ...
    def _analyze_value_propagation(self) -> Dict[str, Any]:
        ...
        try:
            ...
            node_values = {n: self.graph.nodes[n].get('data_value', 0.0) 
            ...
            top_propagated = sorted(propagated_values.items(), 
                                  key=lambda x: x[1], 
                                  reverse=True)[:10]
            ...
            for node, prop_value in propagated_values.items():
                ...
                efficiency = prop_value / max(response_time, 0.1)  # avoid division by zero
                value_efficiency[node] = efficiency
            
            top_efficient = sorted(value_efficiency.items(),
                                 key=lambda x: x[1],
                                 reverse=True)[:10]
    ...
    def _analyze_crawl_efficiency(self) -> Dict[str, Any]:
        ...
        try:
            ...
            successful_nodes = [n for n in self.graph.nodes() 
                              if self.graph.nodes[n].get('success', False)]
            ...
            for node in successful_nodes:
                try:
                    ...
                    if root_nodes:
                        min_depth = min(nx.shortest_path_length(self.graph, root, node)
                                      for root in root_nodes if nx.has_path(self.graph, root, node))
            ...
            failed_nodes = [n for n in self.graph.nodes() 
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        try:
            ...
            if len(nodes) < self.min_nodes_for_analysis:
                return [f"Collect more crawl data (current: {len(nodes)}, needed: {self.min_nodes_for_analysis})"]
            
            # Success rate recommendations
            successful = sum(1 for n in nodes if self.graph.nodes[n].get('success', False))
            success_rate = successful / len(nodes) if nodes else 0.0
            
            if success_rate < 0.7:
                recommendations.append(f"Low success rate ({success_rate:.1%}) - review target selection and retry policies")
            ...
            if avg_value < 0.3:
                recommendations.append("Low average data value - focus on higher-value targets")
            
            # Graph density recommendations
            density = nx.density(self.graph)
            if density < 0.1:
                recommendations.append("Low graph connectivity - consider broader link discovery")
            elif density > 0.5:
                recommendations.append("High graph density - implement more selective crawling")
                ...
                try:
                    ...
                    if len(communities) > 1:
                        ...
                        for community in communities:
                            community_value = sum(self.graph.nodes[n].get('data_value', 0.0) 
                                                for n in community)
                        ...
                        if best_community:
                            sample_domains = set(self._extract_domain(url) for url in list(best_community)[:5])
                            recommendations.append(f"Focus on high-value community domains: {', '.join(sample_domains)}")
                
                except:
                    pass
            
            if not recommendations:
                recommendations.append("Crawl patterns look optimal - continue current strategy")
    ...
    def _get_centrality_distribution(self, pagerank: Dict[str, float]) -> Dict[str, int]:
    ...
    def get_high_value_targets(self, limit: int = 20) -> List[Tuple[str, float]]:
        ...
        try:
            ...
            for node in self.graph.nodes():
                if self.graph.nodes[node].get('success') is not False:  # not explicitly failed
                    score = 0.0
                    ...
                    if neighbors:
                        neighbor_success = sum(1 for n in neighbors 
                                             if self.graph.nodes[n].get('success', False))
            ...
            top_targets = sorted(target_scores.items(), key=lambda x: x[1], reverse=True)
