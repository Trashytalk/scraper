...
class AdvancedDataQualityAssessor:
    ...
    def _initialize_quality_components(self):
        ...
        if ML_AVAILABLE:
            ...
            self.logger.warning("ML libraries not available for advanced quality assessment")
        ...
        default_rules = {
            ...
            'phone': [
                QualityRule(
                    ...
                    field_name='phone',
                    rule_type='pattern',
                    rule_config={'pattern': r'^\+?1?-?\.?\s?\(?\d{3}\)?[\s\-\.]?\d{3}[\s\-\.]?\d{4}$'},
                    severity='medium'
                )
            ],
            'price': [
            ...
            'date': [
                QualityRule(
                    ...
                    field_name='date',
                    rule_type='pattern',
                    rule_config={'pattern': r'^\d{4}-\d{2}-\d{2}|\d{2}/\d{2}/\d{4}|\d{2}-\d{2}-\d{4}$'},
                    severity='medium'
                )
            ]
        }
    ...
    async def assess_data_quality(self, data: Union[List[Dict], pd.DataFrame], 
                                source_url: str = "", 
                                field_mapping: Optional[Dict[str, str]] = None) -> DataQualityMetrics:
        ...
        try:
            ...
            metrics = DataQualityMetrics(
                ...
                total_records=len(data) if isinstance(data, list) else len(df) if ML_AVAILABLE else 0,
                total_fields=len(data[0]) if data and isinstance(data, list) else len(df.columns) if ML_AVAILABLE else 0
            )
            
            # Assess different quality dimensions
            await self._assess_completeness(df if ML_AVAILABLE else data, metrics)
            await self._assess_accuracy(df if ML_AVAILABLE else data, metrics, field_mapping)
            ...
            await self._assess_validity(df if ML_AVAILABLE else data, metrics, field_mapping)
            ...
            self.logger.info(f"Quality assessment complete for {source_url}: {metrics.overall_quality_score:.2f}")
            return metrics
    ...
    async def _assess_completeness(self, data: Union[List[Dict], pd.DataFrame], 
                                 metrics: DataQualityMetrics):
        ...
        try:
            if ML_AVAILABLE and isinstance(data, pd.DataFrame):
                ...
                metrics.completeness_score = 1 - (missing_cells / total_cells) if total_cells > 0 else 0
                
                # Per-field completeness
                for column in data.columns:
                    missing_count = data[column].isnull().sum()
                    completeness = 1 - (missing_count / len(data)) if len(data) > 0 else 0
                ...
                for field in all_fields:
                    present_count = sum(1 for record in data if field in record and record[field] is not None and str(record[field]).strip() != '')
                    field_completeness[field] = present_count / len(data) if len(data) > 0 else 0
                
                metrics.field_completeness = field_completeness
                metrics.completeness_score = sum(field_completeness.values()) / len(field_completeness) if field_completeness else 0
                
        except Exception as e:
            self.logger.error(f"Error assessing completeness: {e}")
            metrics.completeness_score = 0.0
    
    async def _assess_accuracy(self, data: Union[List[Dict], pd.DataFrame], 
                             metrics: DataQualityMetrics,
        ...
        try:
            ...
            if ML_AVAILABLE and isinstance(data, pd.DataFrame):
                ...
                for column in data.columns:
                    field_type = field_mapping.get(column, self._infer_field_type(data[column]))
                                    ...
                                    if rule.enabled:
                                        ...
                                        if not is_valid:
                                            ...
                                            validation_errors.append({
                                                ...
                                                'error': error_msg or f"Failed {rule.name} validation"
                                            })
                                
                                if all_valid:
                                    valid_count += 1
                        
                        field_accuracy[column] = valid_count / total_count if total_count > 0 else 1.0
                    else:
                ...
                for field in all_fields:
                    field_type = field_mapping.get(field, self._infer_field_type_from_values(
                    ...
                    if rules:
                        ...
                        field_accuracy[field] = valid_count / total_count if total_count > 0 else 1.0
                    else:
                        field_accuracy[field] = 1.0
            
            metrics.field_accuracy = field_accuracy
            metrics.accuracy_score = sum(field_accuracy.values()) / len(field_accuracy) if field_accuracy else 1.0
            metrics.validation_errors = validation_errors[:100]  # Limit to first 100 errors
            
        except Exception as e:
            self.logger.error(f"Error assessing accuracy: {e}")
            metrics.accuracy_score = 0.5
    
    async def _assess_consistency(self, data: Union[List[Dict], pd.DataFrame], 
                                metrics: DataQualityMetrics):
        ...
        try:
            ...
            if ML_AVAILABLE and isinstance(data, pd.DataFrame):
                ...
                for field in all_fields:
                    values = [record.get(field) for record in data if field in record and record[field] is not None]
                    if values:
            ...
            metrics.consistency_score = sum(consistency_scores) / len(consistency_scores) if consistency_scores else 1.0
            
        except Exception as e:
            self.logger.error(f"Error assessing consistency: {e}")
            metrics.consistency_score = 0.5
    
    async def _assess_uniqueness(self, data: Union[List[Dict], pd.DataFrame], 
                               metrics: DataQualityMetrics):
        ...
        try:
            if ML_AVAILABLE and isinstance(data, pd.DataFrame):
                ...
                total_rows = len(data)
                metrics.uniqueness_score = unique_rows / total_rows if total_rows > 0 else 1.0
                ...
                metrics.uniqueness_score = len(record_hashes) / len(data) if len(data) > 0 else 1.0
                
        except Exception as e:
            self.logger.error(f"Error assessing uniqueness: {e}")
            metrics.uniqueness_score = 0.5
    
    async def _assess_validity(self, data: Union[List[Dict], pd.DataFrame], 
                             metrics: DataQualityMetrics,
                             field_mapping: Optional[Dict[str, str]] = None):
        ...
        try:
            ...
            business_rules = [
                ...
                {
                    'name': 'price_positive',
                    'condition': lambda record: float(record.get('price', 0)) >= 0 if record.get('price') else True,
                    'applicable_fields': ['price', 'cost', 'amount']
                },
                {
                    'name': 'future_dates',
                    'condition': lambda record: self._validate_future_date(record.get('event_date', '')),
            ...
            if isinstance(data, list) and data:
                for rule in business_rules:
                    ...
                    for record in data:
                        ...
                        has_applicable_field = any(field in record for field in rule['applicable_fields'])
            ...
            metrics.validity_score = sum(validity_scores) / len(validity_scores) if validity_scores else 1.0
    ...
    async def _detect_anomalies(self, df: pd.DataFrame, metrics: DataQualityMetrics):
        ...
        try:
            ...
            if len(numeric_data) < 10:  # Need minimum data for meaningful anomaly detection
                return
            ...
            for idx, label in enumerate(anomaly_labels):
                if label == -1:  # Anomaly detected
                    anomaly_data = {
                        'row_index': int(idx),
                        'anomaly_score': float(self.anomaly_detector.score_samples([scaled_data[idx]])[0]),
                        'values': {}
                    }
    ...
    async def _assess_advanced_metrics(self, df: pd.DataFrame, metrics: DataQualityMetrics):
        ...
        try:
            ...
            if timestamp_columns:
                ...
                try:
                    ...
                    if pd.notna(latest_date):
                        ...
                        metrics.freshness_score = max(0, np.exp(-days_old / 30))  # 30-day half-life
            ...
            field_diversity = len(df.columns) / max(len(df.columns), 10)  # Normalize to typical range
            ...
            for col in df.columns:
                ...
                precision_factors.append(min(1.0, unique_ratio * 2))  # Higher diversity = higher precision
            
            metrics.precision_score = sum(precision_factors) / len(precision_factors) if precision_factors else 0.5
    ...
    def generate_quality_report(self, metrics: DataQualityMetrics) -> Dict[str, Any]:
        ...
        try:
            report = {
                ...
                'field_analysis': {
                    'completeness_by_field': {k: round(v, 3) for k, v in metrics.field_completeness.items()},
                    'accuracy_by_field': {k: round(v, 3) for k, v in metrics.field_accuracy.items()}
                },
                'quality_issues': {
    ...
    def _generate_quality_recommendations(self, metrics: DataQualityMetrics) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if metrics.completeness_score < 0.8:
                incomplete_fields = [field for field, score in metrics.field_completeness.items() if score < 0.8]
                recommendations.append({
                    'type': 'completeness',
                    'priority': 'high' if metrics.completeness_score < 0.6 else 'medium',
            ...
            if metrics.accuracy_score < 0.8:
                error_types = Counter(error['rule'] for error in metrics.validation_errors)
                most_common_errors = error_types.most_common(3)
                
                recommendations.append({
                    'type': 'accuracy',
                    'priority': 'high' if metrics.accuracy_score < 0.6 else 'medium',
            ...
            if len(metrics.data_anomalies) > len(metrics.data_anomalies) * 0.1:  # More than 10% anomalies
                recommendations.append({
                    'type': 'anomalies',
                    'priority': 'medium',
    ...
    def add_custom_quality_rule(self, field_name: str, rule: QualityRule):
        ...
        self.logger.info(f"Added custom quality rule '{rule.name}' for field '{field_name}'")
    
    def get_quality_trends(self, source_url: str = "", days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            filtered_history = [
                ...
                if m.assessment_timestamp > cutoff_date and
                (not source_url or m.source_url == source_url)
            ]
            ...
            trends = {
                ...
                'accuracy': {
                    ...
                    'average': statistics.mean(accuracy_scores) if accuracy_scores else 0,
