...
class IntelligentCrawlScheduler:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.enable_ml_scoring = (
            self.config.get("enable_ml_scoring", True) and SKLEARN_AVAILABLE
        )
        self.model_update_interval = self.config.get(
        ...
        logger.info(
            f"IntelligentCrawlScheduler initialized with ML support: {self.enable_ml_scoring}"
        )
    ...
    def _initialize_ml_components(self) -> None:
        ...
        model_path = Path(
            self.config.get("model_path", "data/models/crawl_scheduler.pkl")
        )
    ...
    def add_crawl_request(self, request: CrawlRequest) -> bool:
        ...
        try:
            ...
            logger.debug(
                f"Added crawl request: {request.url} (priority: {request.priority.name})"
            )
            return True
    ...
    def complete_crawl_request(
        ...
        if (
            self.enable_ml_scoring
            and self.requests_processed - self.last_model_update
    ...
    def fail_crawl_request(self, url: str, error: str) -> None:
        ...
        if request.retry_count <= request.max_retries:
            ...
            self.priority_queues[request.priority].append(request)
            logger.info(
                f"Retrying crawl request: {url} (attempt {request.retry_count})"
            )
        else:
            # Mark as permanently failed
            request.metadata.update(
                {"completed_at": time.time(), "success": False, "error": error}
            )
            self.completed_crawls.append(request)
            logger.warning(
                f"Permanently failed crawl request: {url} after {request.retry_count} attempts"
            )
        ...
        try:
            ...
            if request.parent_url:
                self.crawl_graph.add_edge(
                    request.parent_url, request.url, depth=request.depth
                )
        except Exception as e:
    ...
    def _update_crawl_stats(
        ...
        domain_stats.avg_response_time = (
            domain_stats.avg_response_time * (domain_stats.total_requests - 1)
            + response_time
        ) / domain_stats.total_requests
        domain_stats.success_rate = (
            domain_stats.completed_requests / domain_stats.total_requests
        )
        domain_stats.data_quality_score = (
            domain_stats.data_quality_score * (domain_stats.total_requests - 1)
            + data_quality
        ) / domain_stats.total_requests
        ...
        try:
            ...
            for request in self.completed_crawls[-1000:]:  # use last 1000 requests
                ...
                if success:
                    ...
                    target_value += max(
                        0, 0.2 * (1.0 - min(response_time / 5.0, 1.0))
                    )  # speed bonus
            ...
            logger.info(
                f"Updated ML model - Train R²: {train_score:.3f}, Test R²: {test_score:.3f}"
            )
    ...
    def optimize_crawl_strategy(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(self.crawl_graph.nodes()) > 0:
                ...
                if density < 0.1:
                    recommendations.append(
                        "Low graph connectivity - consider broader discovery strategies"
                    )
                elif density > 0.8:
                    recommendations.append(
                        "High graph density - consider more selective crawling"
                    )

            # Analyze success patterns
            recent_requests = self.completed_crawls[-100:]
            success_rate = sum(
                1 for r in recent_requests if r.metadata.get("success", False)
            ) / len(recent_requests)

            if success_rate < 0.8:
                recommendations.append(
                    "Low success rate - review target selection and retry policies"
                )
            ...
            if self.domain_stats:
                ...
                recommendations.append(
                    f"Top performing domains: {[d[0] for d in best_domains]}"
                )
