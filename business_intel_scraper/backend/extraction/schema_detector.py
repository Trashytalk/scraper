...
class SchemaDetector:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        logger.info(f"SchemaDetector initialized with analysis support: {ANALYSIS_AVAILABLE}")
    ...
    def _simple_schema_detection(self, html_content: str, url: str) -> Optional[DetectedSchema]:
        ...
        try:
            ...
            # Email detection
            email_matches = re.findall(self.field_patterns[DataType.EMAIL][0], html_content)
            if email_matches:
                fields.append(SchemaField(
                    name="email",
    ...
    def _detect_business_field(self, text: str, context: str, element: Any) -> Optional[SchemaField]:
        """Detect business-specific fields"""
        text_lower = text.lower()
        context_lower = context.lower()
        
        # Company name detection
        if (any(keyword in context_lower for keyword in ['company', 'name', 'title', 'brand']) and
            len(text.split()) <= 5 and len(text) > 3):
            return SchemaField(
                name="company_name",
                data_type=DataType.TEXT,
                selector=self._generate_selector(element),
        ...
        # Address detection
        if (any(keyword in text_lower for keyword in ['street', 'ave', 'road', 'blvd', 'suite']) or
            any(keyword in context_lower for keyword in ['address', 'location'])):
            return SchemaField(
                name="address",
        ...
        if any(keyword in context_lower for keyword in ['industry', 'sector', 'business', 'category']):
            return SchemaField(
                name="industry",
                data_type=DataType.TEXT,
                selector=self._generate_selector(element),
        ...
        if (len(text) > 50 and 
            any(keyword in context_lower for keyword in ['description', 'about', 'overview', 'summary'])):
            return SchemaField(
                name="description",
                data_type=DataType.TEXT,
                selector=self._generate_selector(element),
    ...
    def _extract_meta_fields(self, doc: Any) -> List[SchemaField]:
        ...
        try:
            ...
            for meta in meta_tags:
                ...
                if content and any(keyword in name.lower() for keyword in [
                    'company', 'organization', 'business', 'title', 'description'
                ]):
    ...
    def _merge_similar_fields(self, fields: List[SchemaField]) -> List[SchemaField]:
        ...
        for (name, data_type), group in field_groups.items():
            if len(group) == 1:
                ...
                best_field.examples = list(set(sum([f.examples for f in group], [])))[:5]
    ...
    def _calculate_schema_confidence(self, schema: DetectedSchema, doc: Any) -> float:
        ...
        critical_fields = sum(1 for f in schema.fields if f.importance == FieldImportance.CRITICAL)
        ...
        final_confidence = base_confidence + critical_bonus + structured_bonus - field_count_penalty
    ...
    def _is_contact_page(self, doc: Any) -> bool:
        ...
        contact_indicators = ['contact', 'phone', 'email', 'address', 'location', 'reach us']
        return any(indicator in text or indicator in title_text for indicator in contact_indicators)
    ...
    def _is_about_page(self, doc: Any) -> bool:
        ...
        about_indicators = ['about', 'company', 'history', 'mission', 'vision', 'team', 'leadership']
    ...
    def _is_product_page(self, doc: Any) -> bool:
        ...
        text = doc.text_content().lower()
        return any(indicator in text for indicator in ['product', 'service', 'solution', 'offering'])
    ...
    def _has_structured_data(self, doc: Any) -> bool:
        ...
        return (len(doc.xpath('//script[@type="application/ld+json"]')) > 0 or
                len(doc.xpath('//*[@itemtype or @itemscope]')) > 0 or
                len(doc.xpath('//*[@typeof]')) > 0)
    ...
    def _determine_importance(self, data_type: DataType, context: str) -> FieldImportance:
        """Determine field importance based on type and context"""
        context_lower = context.lower()
        
        # Critical business fields
        if (data_type == DataType.TEXT and 
            any(keyword in context_lower for keyword in ['name', 'title', 'company'])):
    ...
    def save_schema(self, schema: DetectedSchema, filepath: Optional[str] = None) -> bool:
        """Save schema to file"""
        try:
            if not filepath:
    ...
    def get_detector_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'schemas_by_confidence': {
                'high': sum(1 for s in self.detected_schemas.values() if s.confidence >= 0.8),
                'medium': sum(1 for s in self.detected_schemas.values() if 0.6 <= s.confidence < 0.8),
                'low': sum(1 for s in self.detected_schemas.values() if s.confidence < 0.6)
            },
            'field_types': dict(Counter(
                field.data_type.value 
                for schema in self.detected_schemas.values() 
                for field in schema.fields
            )),
            'avg_fields_per_schema': np.mean([
                len(schema.fields) for schema in self.detected_schemas.values()
            ]) if self.detected_schemas else 0.0
        }
