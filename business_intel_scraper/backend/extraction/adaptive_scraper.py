...
from ..discovery.classifier import AdaptiveLinkClassifier, LinkInfo, LinkCategory
...
class AdaptiveBusinessScraper:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        logger.info(f"AdaptiveBusinessScraper initialized (Playwright: {PLAYWRIGHT_AVAILABLE})")
    ...
    def add_seed_urls(self, urls: List[str], priority: CrawlPriority = CrawlPriority.NORMAL) -> int:
    ...
    async def run_adaptive_crawl(self, max_pages: int = 100, time_limit: Optional[int] = None) -> Dict[str, Any]:
        """Run adaptive crawling session"""
        start_time = time.time()
        pages_processed = 0
        results = []
        
        logger.info(f"Starting adaptive crawl (max_pages: {max_pages}, time_limit: {time_limit}s)")
        ...
        try:
            while pages_processed < max_pages:
                ...
                if result.extracted_links:
                    await self._add_discovered_links(result.extracted_links, request.url, request.depth + 1)
    ...
    async def _process_crawl_request(self, request: CrawlRequest) -> ExtractionResult:
        ...
        try:
            ...
            if schema:
                ...
                extracted_data = await self._extract_data_with_schema(html_content, schema)
                result.data = extracted_data
                result.confidence = self._calculate_extraction_confidence(extracted_data, schema)
            ...
            logger.debug(f"Extraction result for {request.url}: success={result.success}, confidence={result.confidence:.2f}")
    ...
    async def _detect_or_match_schema(self, html_content: str, url: str) -> Optional[DetectedSchema]:
        """Detect schema or match against existing schemas"""
        try:
            # Try to match against existing schemas first
            domain = urlparse(url).netloc
            matching_schemas = [
                schema for schema in self.known_schemas.values()
    ...
    async def _extract_data_with_schema(self, html_content: str, schema: DetectedSchema) -> Dict[str, Any]:
        ...
        try:
            ...
            for field in schema.fields:
                if field.data_type.value == 'email':
                    ...
                    if matches:
                        extracted_data[field.name] = matches[0] if not field.multiple else matches
                    ...
                    if matches:
                        extracted_data[field.name] = matches[0] if not field.multiple else matches[:5]
    ...
    def _calculate_extraction_confidence(self, data: Dict[str, Any], schema: DetectedSchema) -> float:
        ...
        critical_fields = sum(1 for field in schema.fields 
    ...
    async def _add_discovered_links(self, links: List[str], parent_url: str, depth: int) -> None:
        ...
        for link_info in classified_links:
            if link_info.priority.value <= 3:  # High to Medium priority
                request = CrawlRequest(
                    ...
                    metadata={'link_category': link_info.category.value, 'link_confidence': link_info.confidence}
                )
    ...
    def _generate_crawl_summary(self, results: List[ExtractionResult], duration: float) -> Dict[str, Any]:
        ...
        for result in results:
            if result.schema_id:
                schema_usage[result.schema_id] = schema_usage.get(result.schema_id, 0) + 1
        
        # Data extraction stats
        total_data_points = sum(len(r.data) for r in results)
        avg_confidence = sum(r.confidence for r in results) / total_results if total_results > 0 else 0.0
        avg_response_time = sum(r.response_time for r in results) / total_results if total_results > 0 else 0.0
