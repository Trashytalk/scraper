...
class AdvancedFilters:
    ...
    def remove_filter(self, group_id: str, field: str):
        ...
        if group_id in self.active_filters:
            self.active_filters[group_id] = [
                f for f in self.active_filters[group_id] if f.field != field
            ]
    ...
    def _build_condition(self, criteria: FilterCriteria, model_class):
        ...
        try:
            ...
            if not hasattr(model_class, criteria.field):
                logger.warning(
                    f"Field {criteria.field} not found in model {model_class}"
                )
                return None
    ...
    def save_filter_set(self, name: str, description: str = ""):
        ...
        self.saved_filters[name] = {
            "filters": {
                k: [asdict(f) for f in v] for k, v in self.active_filters.items()
            },
    ...
    def load_filter_set(self, name: str) -> bool:
        ...
        if name in self.saved_filters:
            ...
            for group_id, filters in saved["filters"].items():
                self.active_filters[group_id] = [FilterCriteria(**f) for f in filters]
            return True
        return False
    ...
    def get_filter_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "total_filters": sum(
                len(filters) for filters in self.active_filters.values()
            ),
            "groups": {},
        }
...
class CollaborationManager:
    ...
    async def add_participant(
        self, websocket: WebSocket, user_id: str, room_id: str = "default"
    ):
        ...
        try:
            ...
            if event_type == EventType.CURSOR_MOVED:
                ...
                if room_id in self.shared_annotations and annotation_id:
                    self.shared_annotations[room_id] = [
                        a
                        for a in self.shared_annotations[room_id]
                        if a["id"] != annotation_id
                    ]

            # Broadcast event to room participants
            await self.broadcast_to_room(
                session_info["room_id"], event, exclude_session=session_id
            )
    ...
    async def broadcast_to_room(
        self, room_id: str, event: CollaborationEvent, exclude_session: str = None
    ):
                ...
                "participants": [
                    {
                        ...
                        "joined_at": self.active_sessions[session_id][
                            "joined_at"
                        ].isoformat(),
                    }
...
class CustomDashboardBuilder:
    ...
    def _init_widget_templates(self) -> Dict[str, Dict]:
        ...
        return {
            ...
            "geo_map": {
                ...
                "config": {"map_type": "choropleth", "center": "auto", "zoom": "auto"},
                "size": {"w": 6, "h": 4},
            },
            "metrics_card": {
                "type": "metric",
    ...
    def create_dashboard(
        self, user_id: str, name: str, template: str = "default"
    ) -> str:
        """Create new dashboard configuration"""
        dashboard_id = str(uuid.uuid4())

        layout_template = self.layout_templates.get(
            template, self.layout_templates["default"]
        )
    ...
    def add_widget(
        self, dashboard_id: str, widget_type: str, position: Dict[str, int] = None
    ) -> Optional[str]:
        ...
        widget_layout = {
            ...
            "w": (
                position.get("w")
                if position
                else config["widgets"][widget_id]["size"]["w"]
            ),
            "h": (
                position.get("h")
                if position
                else config["widgets"][widget_id]["size"]["h"]
            ),
        }
    ...
    def remove_widget(self, dashboard_id: str, widget_id: str) -> bool:
        ...
        # Remove from layout
        config["layout"] = [item for item in config["layout"] if item["i"] != widget_id]
    ...
    def duplicate_dashboard(
        self, dashboard_id: str, user_id: str, new_name: str = None
    ) -> Optional[str]:
