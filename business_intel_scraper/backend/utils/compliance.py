...
class DataSubject:
    ...
    retention_period: timedelta = field(default=timedelta(days=365 * 2))  # 2 years default
...
class GDPRComplianceManager:
    ...
    def __init__(self, encryption_manager):
        ...
        self.retention_policies: Dict[DataCategory, timedelta] = self._init_retention_policies()
        self.anonymization_rules: Dict[DataCategory, Callable] = self._init_anonymization_rules()
    ...
    def _init_retention_policies(self) -> Dict[DataCategory, timedelta]:
        ...
        return {
            ...
            DataCategory.FINANCIAL: timedelta(days=365 * 7),              # 7 years (legal requirement)
    ...
    def record_consent(self, consent: ConsentRecord):
        ...
        if consent.subject_id in self.data_subjects:
            self.data_subjects[consent.subject_id].consent_records[consent.consent_type] = {
                'granted': consent.granted,
                'timestamp': consent.timestamp,
                'version': consent.version
            }
        
        logger.info(f"Consent recorded: {consent.consent_type.value} - {consent.granted}")
    ...
    def withdraw_consent(self, subject_id: str, consent_type: ConsentType):
        ...
        if subject_id in self.consent_records:
            for consent in reversed(self.consent_records[subject_id]):
                if consent.consent_type == consent_type and consent.granted and not consent.withdrawn_date:
        ...
        if subject_id in self.data_subjects:
            if consent_type in self.data_subjects[subject_id].consent_records:
                self.data_subjects[subject_id].consent_records[consent_type]['granted'] = False
                self.data_subjects[subject_id].consent_records[consent_type]['withdrawn_date'] = now
        
        logger.info(f"Consent withdrawn: {subject_id} - {consent_type.value}")
        
        # Trigger data processing review
        asyncio.create_task(self._review_data_processing_after_withdrawal(subject_id, consent_type))
    
    async def _review_data_processing_after_withdrawal(self, subject_id: str, consent_type: ConsentType):
    ...
    async def delete_subject_data(self, subject_id: str):
        ...
        try:
            ...
            # 4. Third-party service data removal
            # 5. Backup and archive cleanup
        ...
        try:
            ...
            for category in subject.data_categories:
                if category in self.anonymization_rules:
                    # TODO: Apply anonymization to actual data
                    # This would involve updating database records, files, etc.
                    pass
    ...
    def export_subject_data(self, subject_id: str) -> Dict[str, Any]:
        ...
        export_data = {
            ...
            'data_processing_activities': [
                activity for activity in self.data_processing_register
        ...
        # TODO: Include actual user data from all systems
        # This would gather data from:
        # - User profiles and preferences
        # - Historical activity data
        # - Generated content
        # - Analytics data (if consent given)
    ...
    def generate_privacy_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'data_subjects': {
                ...
                'anonymized': sum(1 for s in self.data_subjects.values() if s.anonymized),
                'deletion_requested': sum(1 for s in self.data_subjects.values() if s.deletion_requested)
            },
            'consent_management': {
...
class ThirdPartyIntegrations:
    ...
    async def analytics_integration(self, event_data: Dict[str, Any], user_id: str = None):
    ...
    async def crm_integration(self, customer_data: Dict[str, Any], user_id: str):
        ...
        if user_id not in self.gdpr_manager.data_subjects:
            logger.warning(f"CRM integration attempted for unregistered user: {user_id}")
            return
    ...
    async def cleanup_third_party_data(self, user_id: str):
        ...
        for service_name, integration in self.integrations.items():
            ...
            try:
                ...
                logger.error(f"Error cleaning up {service_name} for user {user_id}: {e}")
    ...
    def get_integration_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'active_integrations': sum(1 for i in self.integrations.values() if i['active']),
            'integrations': {
                name: {
...
gdpr_manager = GDPRComplianceManager(None)  # Will be initialized with encryption_manager
cookie_consent_manager = CookieConsentManager()

# Export for easy import
__all__ = [
    'GDPRComplianceManager', 'ThirdPartyIntegrations', 'CookieConsentManager',
    'DataSubject', 'ConsentRecord', 'DataCategory', 'ConsentType', 'DataProcessingPurpose',
    'gdpr_manager', 'cookie_consent_manager'
]
