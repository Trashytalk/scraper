...
class CacheManager:
    ...
    async def delete_pattern(self, pattern: str):
        ...
        try:
            if self.redis_client:
                ...
                keys_to_delete = [
                    k for k in self.local_cache.keys() if pattern.replace("*", "") in k
                ]
    ...
    async def _cleanup_local_cache(self):
        ...
        now = datetime.utcnow()
        expired_keys = [
            key for key, entry in self.local_cache.items() if entry["expires"] <= now
        ]
    ...
    def decorator(func: Callable):
        ...
        async def wrapper(*args, **kwargs):
            ...
            cache_key = cache_manager._generate_cache_key(
                f"{key_prefix}:{func.__name__}", cache_data
            )
    ...
    try:
        ...
        for operation in warm_up_operations:
            ...
            await cache_manager.set(
                f"warmup:{operation}", {"status": "ready"}, ttl=7200
            )
...
async def cleanup_expired_data():
    ...
    try:
        ...
        if len(performance_monitor.metrics_history) > 10000:
            performance_monitor.metrics_history = performance_monitor.metrics_history[
                -5000:
            ]
