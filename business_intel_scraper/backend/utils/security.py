...
class SecurityAuditLogger:
    ...
    async def log_security_event(self, event: SecurityEvent):
        ...
        for key, value in event.metadata.items():
            if key in ["password", "token", "session_id", "api_key"]:
                encrypted_metadata[key] = self.encryption_manager.encrypt_data(
                    str(value)
                )
            else:
        ...
        if self.redis_client:
            try:
                await self.redis_client.lpush(
                    "security_events", json.dumps(event.to_dict())
                )
    ...
    async def _send_security_alert(self, event: SecurityEvent):
        ...
        # Implement actual alerting system
        await self._send_security_alert(alert_message, event)
        logger.critical(alert_message)

    async def _send_security_alert(self, message: str, event):
        """Send security alerts through multiple channels"""
        try:
            # Log to dedicated security log file
            ...
            # Create alert record - will be saved by calling code if db session available
            alert_data = {
            ...
            # Log structured alert data for external systems to pick up
            logger.info(f"STRUCTURED_SECURITY_ALERT: {alert_data}")
...
class SecurityMiddleware(BaseHTTPMiddleware):
    ...
    async def _check_rate_limit(
        self, ip_address: str, limit: int = 100, window: int = 60
    ) -> bool:
