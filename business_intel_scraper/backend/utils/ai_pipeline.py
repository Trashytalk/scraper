...
class AIEnhancementPipeline:
    ...
    def __init__(self, settings: Optional[Any] = None) -> None:
        ...
        if settings:
            ...
            self.process_classification = settings.getbool(
        ...
        if self.enabled:
            try:
                ...
                if status["ai_enabled"]:
                    ...
                    self.logger.info(
                        f"Available capabilities: {list(status['capabilities'].keys())}"
                    )
                else:
                    self.logger.warning(
                        "AI models not available, disabling AI pipeline"
                    )
                    self.enabled = False
    ...
    def _merge_ai_data(
        self, original_item: Any, enhanced_data: ProcessedData
    ) -> Dict[str, Any]:
        ...
        if self.process_entities and enhanced_data.entities:
            ...
            enhanced_item["ai_people"] = [
                e.text for e in enhanced_data.entities if e.label == "PERSON"
            ]
            enhanced_item["ai_organizations"] = [
                e.text for e in enhanced_data.entities if e.label == "ORG"
            ]
            enhanced_item["ai_locations"] = [
                e.text for e in enhanced_data.entities if e.label in ["GPE", "LOC"]
            ]
            enhanced_item["ai_emails"] = [
                e.text for e in enhanced_data.entities if e.label == "EMAIL"
            ]
            enhanced_item["ai_urls"] = [
                e.text for e in enhanced_data.entities if e.label == "URL"
            ]
        ...
        if self.process_classification and enhanced_data.classification:
            ...
            enhanced_item["ai_category_confidence"] = (
                enhanced_data.classification.confidence
            )
        ...
        if self.process_sentiment and enhanced_data.sentiment:
            ...
            if sentiment_dict:
                dominant_sentiment = max(
                    sentiment_dict, key=lambda k: sentiment_dict[k]
                )
...
class AIFilterPipeline:
    ...
    def __init__(self, settings: Optional[Any] = None) -> None:
        ...
        if settings:
            ...
            self.min_sentiment_confidence = settings.getfloat(
                "AI_MIN_SENTIMENT_CONFIDENCE", 0.0
            )
    ...
    def process_item(self, item: Any, spider: Spider) -> Any:
        ...
        if quality_score < self.min_quality_score:
            ...
            raise DropItem(
                f"Quality score {quality_score} below threshold {self.min_quality_score}"
            )
        ...
        if self.required_entities:
            ...
            if missing_entities:
                self.logger.info(
                    f"Dropping item missing required entities: {missing_entities}"
                )
        ...
        if sentiment_score < self.min_sentiment_confidence:
            self.logger.info(
                f"Dropping item with low sentiment confidence: {sentiment_score}"
            )
