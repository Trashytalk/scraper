...
from gui.components.advanced_parsing import AdvancedParser, ParsingRule, FileFormat, ParsingMode
from business_intel_scraper.backend.nlp.pipeline import extract_entities, preprocess
...
class EnhancedDataProcessor:
    ...
    async def process_task(self, task: ProcessingTask) -> Dict[str, Any]:
        ...
        try:
            ...
            task.progress = 0.3
            rule_results = await self._apply_parsing_rules(parsed_data, task.parsing_rules)
            ...
            task.progress = 0.7
            cleaned_data = await self._clean_and_normalize(parsed_data, rule_results, nlp_results)
            ...
            self.processing_stats['entities_extracted'] += len(nlp_results.get('entities', []))
    ...
    async def _apply_parsing_rules(self, parsed_data: Dict[str, Any], rules: List[ParsingRule]) -> Dict[str, Any]:
        ...
        try:
            ...
            if raw_content:
                for rule in rules:
                    if rule.enabled:
                        extracted = self.advanced_parser._extract_by_rule(raw_content, rule)
                        if extracted:
    ...
    async def _clean_and_normalize(self, parsed_data: Dict[str, Any], 
    ...
    async def _store_results(self, task: ProcessingTask, results: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if task.destination:
                await self._export_results(results, task.destination, task.output_format)
    ...
    async def _export_results(self, results: Dict[str, Any], destination: str, format: str):
