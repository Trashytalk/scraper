...
class BusinessEventDetector:
    ...
    async def detect_events(self, data_sources: List[Dict[str, Any]], 
                          entities: Optional[List[str]] = None) -> List[BusinessEvent]:
        ...
        logger.info(f"Event detection completed. Found {len(unique_events)} unique events")
        return unique_events
    
    async def _detect_events_in_source(self, source: Dict[str, Any], 
                                     entities: Optional[List[str]]) -> List[BusinessEvent]:
        ...
        if source_type == 'news_articles':
            events.extend(await self._detect_news_events(source_data, source_url, entities))
        elif source_type == 'filings':
            events.extend(await self._detect_filing_events(source_data, source_url, entities))
        elif source_type == 'contracts':
            events.extend(await self._detect_contract_events(source_data, source_url, entities))
        elif source_type == 'social_media':
            events.extend(await self._detect_social_events(source_data, source_url, entities))
        elif source_type == 'structured_data':
            events.extend(await self._detect_structured_events(source_data, source_url, entities))
        
        return events
    
    async def _detect_news_events(self, articles: List[Dict], source_url: Optional[str], 
                                entities: Optional[List[str]]) -> List[BusinessEvent]:
    ...
    async def _detect_filing_events(self, filings: List[Dict], source_url: Optional[str],
                                  entities: Optional[List[str]]) -> List[BusinessEvent]:
        ...
        for filing in filings:
            ...
            regulatory_patterns = {k: v for k, v in self.event_patterns.items() 
    ...
    async def _detect_contract_events(self, contracts: List[Dict], source_url: Optional[str],
                                    entities: Optional[List[str]]) -> List[BusinessEvent]:
        ...
        for contract in contracts:
            ...
            contract_patterns = {k: v for k, v in self.event_patterns.items() 
    ...
    async def _detect_social_events(self, posts: List[Dict], source_url: Optional[str],
                                  entities: Optional[List[str]]) -> List[BusinessEvent]:
        """Detect events from social media posts"""
        events = []
        
        # Social media detection would focus on reputation events
        reputation_patterns = {k: v for k, v in self.event_patterns.items() 
    ...
    async def _detect_structured_events(self, records: List[Dict], source_url: Optional[str],
                                      entities: Optional[List[str]]) -> List[BusinessEvent]:
    ...
    async def _check_pattern_match(self, text: str, source_data: Dict, pattern: EventPattern,
                                 source_url: Optional[str], entities: Optional[List[str]]) -> Optional[BusinessEvent]:
        ...
        if context_matches > 0:
            confidence_score += 0.2 * (context_matches / len(pattern.context_requirements))
    ...
    def _deduplicate_events(self, events: List[BusinessEvent]) -> List[BusinessEvent]:
        ...
        for event in events:
            ...
            signature = f"{event.entity_id}_{event.event_type}_{event.event_date.date()}"
    ...
    def unsubscribe_from_events(self, subscriber_id: str):
        ...
        if subscriber_id in self.subscribers:
            ...
            logger.info(f"Subscriber {subscriber_id} unregistered from event notifications")
    ...
    def get_recent_events(self, hours: int = 24, severity: Optional[EventSeverity] = None) -> List[BusinessEvent]:
