...
class AdvancedEntityResolver:
    ...
    def _load_business_suffixes(self) -> Set[str]:
        ...
        return {
            ...
            'ltd', 'ltd.', 'limited', 'llc', 'l.l.c.', 'pte', 'pte.',
            'plc', 'p.l.c.', 'gmbh', 'sa', 's.a.', 'bv', 'b.v.',
            'ag', 'co', 'co.', 'company', 'group', 'holdings',
            'partnership', 'lp', 'l.p.', 'llp', 'l.l.p.',
    ...
    async def resolve_entities(self, entities: List[Dict[str, Any]], 
                              entity_type: str = "company") -> List[ResolvedEntity]:
        """Main entity resolution pipeline"""
        logger.info(f"Starting entity resolution for {len(entities)} {entity_type} entities")
        ...
        candidate_pairs = await self._generate_candidate_pairs(normalized_entities, entity_type)
        logger.info(f"Generated {len(candidate_pairs)} candidate pairs for matching")
        
        # Step 3: Score candidate pairs
        matches = await self._score_candidate_pairs(candidate_pairs, normalized_entities)
        ...
        resolved_entities = await self._create_canonical_entities(entity_clusters, entity_type)
        
        # Step 6: Store results
        await self._store_resolution_results(resolved_entities, matches)
        
        logger.info(f"Entity resolution completed. Created {len(resolved_entities)} canonical entities")
    ...
    def _normalize_entity(self, entity: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Normalize name
        if 'name' in entity:
            normalized['normalized_name'] = self._normalize_business_name(entity['name'])
        ...
        if 'address' in entity:
            normalized['normalized_address'] = self._normalize_address(entity['address'])
        ...
        for id_field in ['registration_number', 'tax_id', 'vat_number', 'duns_number']:
            if id_field in entity:
                normalized[f'normalized_{id_field}'] = re.sub(r'[^a-zA-Z0-9]', '', 
                                                            str(entity[id_field]).upper())
        
        # Normalize contact info
        if 'email' in entity:
            normalized['normalized_email'] = entity['email'].lower().strip()
            normalized['email_domain'] = normalized['normalized_email'].split('@')[-1] if '@' in normalized['normalized_email'] else ''
    ...
    def _normalize_address(self, address: str) -> str:
        ...
        abbreviations = {
            ...
            'suite': 'ste', 'apartment': 'apt', 'floor': 'fl', 'unit': 'unit',
    ...
    async def _generate_candidate_pairs(self, entities: List[Dict], entity_type: str) -> List[Tuple[int, int]]:
        ...
        for id_field in ['normalized_registration_number', 'normalized_tax_id', 'normalized_duns_number']:
    ...
    async def _score_candidate_pairs(self, candidate_pairs: List[Tuple[int, int]], 
                                   entities: List[Dict]) -> List[EntityMatch]:
        ...
        for i, j in candidate_pairs:
            ...
            overall_score, match_type, evidence = self._calculate_overall_score(similarities)
            
            if overall_score >= self.similarity_threshold:
                confidence_level = self._determine_confidence_level(overall_score, evidence)
    ...
    async def _calculate_field_similarities(self, entity1: Dict, entity2: Dict) -> Dict[str, float]:
        ...
        if entity1.get('normalized_address') and entity2.get('normalized_address'):
            addr_sim = self._calculate_address_similarity(
                entity1['normalized_address'],
                entity2['normalized_address']
            )
        ...
        if entity1.get('normalized_email') and entity2.get('normalized_email'):
            email_sim = 1.0 if entity1['normalized_email'] == entity2['normalized_email'] else 0.0
            similarities['email'] = email_sim
        
        # Domain similarity
        if entity1.get('email_domain') and entity2.get('email_domain'):
            domain_sim = 1.0 if entity1['email_domain'] == entity2['email_domain'] else 0.0
            similarities['email_domain'] = domain_sim
        
        if entity1.get('website_domain') and entity2.get('website_domain'):
            web_domain_sim = 1.0 if entity1['website_domain'] == entity2['website_domain'] else 0.0
            similarities['website_domain'] = web_domain_sim
        
        # Phone similarity
        if entity1.get('phone_suffix') and entity2.get('phone_suffix'):
            phone_sim = 1.0 if entity1['phone_suffix'] == entity2['phone_suffix'] else 0.0
    ...
    def _calculate_overall_score(self, similarities: Dict[str, float]) -> Tuple[float, str, Dict]:
        ...
        id_perfect = any(similarities.get(f, 0) == 1.0 for f in 
                        ['normalized_registration_number', 'normalized_tax_id', 'normalized_duns_number'])
        if id_perfect:
            overall_score = min(overall_score * 1.2, 1.0)
        
        # Boost score for domain matches
        domain_match = (similarities.get('website_domain', 0) == 1.0 or 
                       similarities.get('email_domain', 0) == 1.0)
        if domain_match and similarities.get('name', 0) > 0.7:
            overall_score = min(overall_score * 1.1, 1.0)
        
        # Determine match type
        if any(similarities.get(f, 0) == 1.0 for f in ['normalized_registration_number', 'normalized_tax_id']):
            match_type = 'exact_id_match'
        elif similarities.get('name', 0) > 0.95 and similarities.get('address', 0) > 0.8:
        ...
        evidence = {
            ...
            'highest_scoring_field': max(similarities, key=similarities.get) if similarities else None,
    ...
    async def _cluster_entities(self, matches: List[EntityMatch], 
                              entities: List[Dict]) -> List[List[str]]:
    ...
    async def _create_canonical_entities(self, clusters: List[List[str]], 
                                       entity_type: str) -> List[ResolvedEntity]:
        ...
        for cluster in clusters:
            if len(cluster) == 1:
                ...
                canonical_name, canonical_data, alternative_names = await self._determine_canonical_form(cluster)
    ...
    async def _determine_canonical_form(self, cluster: List[str]) -> Tuple[str, Dict, List[str]]:
    ...
    async def _store_resolution_results(self, resolved_entities: List[ResolvedEntity], 
        ...
        try:
            ...
            session.commit()
            logger.info(f"Stored {len(resolved_entities)} resolved entities and {len(matches)} matches")
