...
class EntityRelationshipMapper:
    ...
    async def map_relationships(
        self, entities: List[Dict], raw_data_sources: List[Dict]
        ...
        officer_rels = await self._extract_officer_relationships(
            entities, raw_data_sources
        )
        relationships.extend(officer_rels)
        logger.info(f"Extracted {len(officer_rels)} officer relationships")

        ownership_rels = await self._extract_ownership_relationships(
            entities, raw_data_sources
        )
        ...
        contract_rels = await self._extract_contract_relationships(
            entities, raw_data_sources
        )
    ...
    async def _extract_officer_relationships(
        self, entities: List[Dict], raw_data_sources: List[Dict]
    ) -> List[EntityRelationship]:
            ...
            if entity.get("type") == "company" and "officers" in entity:
                for officer in entity["officers"]:
                    ...
                    relationship = EntityRelationship(
                        ...
                        source_entity_id=officer.get(
                            "person_id", f"person_{officer.get('name', 'unknown')}"
                        ),
    ...
    def _classify_officer_role(self, role: str) -> str:
        ...
        if any(word in role for word in ["ceo", "chief executive"]):
            ...
            word in role for word in ["cto", "chief technology", "chief technical"]
        ):
            return "IS_CTO_OF"
    ...
    async def _extract_ownership_relationships(
        self, entities: List[Dict], raw_data_sources: List[Dict]
    ) -> List[EntityRelationship]:
            ...
            if "subsidiaries" in entity:
                for subsidiary in entity["subsidiaries"]:
                    relationship = EntityRelationship(
                        ...
                        relationship_subtype="subsidiary",
                        relationship_data={
                            "ownership_percentage": subsidiary.get(
            ...
            if "shareholders" in entity:
                for shareholder in entity["shareholders"]:
                    ...
                    relationship = EntityRelationship(
                        ...
                        evidence_sources=[entity.get("source_url", "")],
                        strength=min(
                            ownership_pct / 100.0 + 0.3, 1.0
                        ),  # Scale by ownership percentage
                    )
                    relationships.append(relationship)

        return relationships

    async def _extract_address_relationships(
            ...
            if len(entity_group) > 1:
                for i in range(len(entity_group)):
                    for j in range(i + 1, len(entity_group)):
                        ...
                        strength = self._calculate_address_strength(
                            address, entity1, entity2
                        )
                        ...
                        relationship = EntityRelationship(
                            ...
                            relationship_data={
                                ...
                                "full_address_match": entity1.get("address")
                                == entity2.get("address"),
                            },
    ...
    def _calculate_address_strength(
        self, address: str, entity1: Dict, entity2: Dict
    ) -> float:
    ...
    async def _extract_contact_relationships(
        self, entities: List[Dict]
    ) -> List[EntityRelationship]:
    ...
    async def _extract_domain_relationships(
        self, entities: List[Dict]
    ) -> List[EntityRelationship]:
    ...
    async def _extract_contract_relationships(
        self, entities: List[Dict], raw_data_sources: List[Dict]
    ) -> List[EntityRelationship]:
    ...
    def analyze_entity_network(
        ...
        for source, target, edge_data in subgraph.edges(data=True):
            if (
                edge_data.get("strength", 0) > 0.7
                and edge_data.get("confidence", 0) > 0.7
            ):
        ...
        return {
            ...
            "network_density": (
                nx.density(subgraph.to_undirected()) if subgraph_nodes else 0
            ),
            "subgraph_data": {
    ...
    def _calculate_network_metrics(
        self, subgraph: nx.MultiDiGraph, entity_id: str
    ) -> Dict[str, Any]:
        ...
        if entity_id in subgraph:
            ...
            if len(subgraph.nodes) > 1:
                ...
                if len(subgraph.nodes) > 2:
                    ...
                    metrics["betweenness_centrality"] = betweenness_centrality.get(
                        entity_id, 0
                    )
                ...
                if nx.is_connected(undirected_graph):
                    ...
                    metrics["closeness_centrality"] = closeness_centrality.get(
                        entity_id, 0
                    )
    ...
    def detect_communities(self, min_community_size: int = 3) -> List[List[str]]:
        ...
        try:
            ...
            logger.warning(
                "python-louvain not installed. Using simple connected components."
            )
