...
@router.get(
    "/overview", response_model=AnalyticsOverview, dependencies=[Depends(require_token)]
)
async def get_analytics_overview() -> AnalyticsOverview:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get analytics overview: {str(e)}"
        )
...
async def get_performance_charts(
    hours: int = Query(
        default=6, ge=1, le=168, description="Hours of historical data (1-168)"
    )
) -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get performance charts: {str(e)}"
        )


@router.get(
    "/alerts", response_model=AlertsResponse, dependencies=[Depends(require_token)]
)
...
@router.get(
    "/jobs", response_model=JobAnalyticsResponse, dependencies=[Depends(require_token)]
)
async def get_job_analytics() -> JobAnalyticsResponse:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get job analytics: {str(e)}"
        )
...
@router.get(
    "/insights", response_model=InsightsResponse, dependencies=[Depends(require_token)]
)
                ...
                "recent_alerts": [
                    {
                        ...
                        "timestamp": "2025-07-22T11:55:00Z",
                    },
                    {
                        "level": "warning",
                        "message": "High memory usage detected",
                        "timestamp": "2025-07-22T11:50:00Z",
                    },
                ],
                "total_alerts": 5,
            },
...
async def get_historical_metric(
    metric_name: str,
    hours: int = Query(
        default=24, ge=1, le=168, description="Hours of historical data"
    ),
) -> Dict[str, Any]:
    """Get historical data for a specific metric."""
    try:
        historical_data = await analytics_engine.get_historical_data(metric_name, hours)
        return {"metric_name": metric_name, "hours": hours, "data": historical_data}
    except Exception as e:
        raise HTTPException(
...
async def get_endpoint_statistics() -> Dict[str, Any]:
    ...
    try:
        ...
        return {"timestamp": analytics_engine._start_time, "endpoints": stats}
    except Exception as e:
        raise HTTPException(
...
async def cleanup_analytics_data(
    days: int = Query(
        default=30, ge=1, le=365, description="Delete data older than this many days"
    )
) -> Dict[str, str]:
...
async def flush_metrics_buffer() -> Dict[str, str]:
    ...
    try:
        ...
        return {"status": "success", "message": "Metrics buffer flushed to storage"}
    except Exception as e:
        raise HTTPException(
