...
async def get_job_stats() -> JobStats:
    ...
    try:
        ...
        return JobStats(
            ...
            completed_jobs=len([j for j in valid_jobs if j.get("status") == "completed"]),
            failed_jobs=len([j for j in valid_jobs if j.get("status") == "failed"]),
            total_items_collected=sum(j.get("items_collected", 0) for j in valid_jobs if isinstance(j.get("items_collected"), int))
        )
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to calculate job statistics: {str(e)}")
...
async def create_job(job_data: JobCreate) -> Job:
    ...
    try:
        ...
        if not (job_data.url.startswith('http://') or job_data.url.startswith('https://')):
            raise HTTPException(status_code=422, detail="URL must start with http:// or https://")
        
        # Validate scraper type
        valid_scraper_types = ['news', 'ecommerce', 'social', 'business', 'general', 'test']
...
async def get_job(job_id: int) -> Job:
    ...
    try:
        if not isinstance(job_id, int) or job_id <= 0:
            raise HTTPException(status_code=422, detail="Invalid job ID: must be a positive integer")
...
async def get_job_logs(job_id: int, limit: int = Query(default=100, ge=1, le=1000)) -> Dict[str, Any]:
...
async def get_job_data(job_id: int, format: str = Query(default="json")) -> Dict[str, Any]:
    ...
    data_size = f"{job['items_collected'] * 2.4:.1f} MB" if job['items_collected'] > 0 else "0 MB"
...
async def download_job_data(job_id: int, format: str = Query(default="json")) -> Dict[str, Any]:
