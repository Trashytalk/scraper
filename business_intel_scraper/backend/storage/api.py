...
@router.post(
    "/raw-data", response_model=RawDataResponse, dependencies=[Depends(require_token)]
)
...
@router.post(
    "/entities",
    response_model=StructuredEntityResponse,
    dependencies=[Depends(require_token)],
)
async def store_structured_entity(
    request: StructuredEntityRequest,
) -> StructuredEntityResponse:
...
async def get_entity_provenance(entity_id: str) -> ProvenanceResponse:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Provenance retrieval failed: {str(e)}"
        )
...
async def export_data_lineage(
    request: DataExportRequest, background_tasks: BackgroundTasks
):
    ...
    try:
        ...
        background_tasks.add_task(
            _perform_data_export, export_id, storage_manager, request
        )
...
async def _perform_data_export(
    export_id: str, storage_manager: AdvancedStorageManager, request: DataExportRequest
):
    ...
    try:
        ...
        logger.info(
            f"Export {export_id} completed: {export_result['total_size_mb']} MB exported"
        )
