...
class StorageConfig(BaseSettings):
    ...
    raw_storage_access_key: Optional[str] = Field(
        default=None, env="STORAGE_RAW_ACCESS_KEY", description="Storage access key"
    )

    raw_storage_secret_key: Optional[str] = Field(
        default=None, env="STORAGE_RAW_SECRET_KEY", description="Storage secret key"
    )

    raw_storage_region: str = Field(
        default="us-east-1", env="STORAGE_RAW_REGION", description="Storage region"
    )
    ...
    database_url: Optional[str] = Field(
        default=None, env="DATABASE_URL", description="Database connection URL"
    )

    database_host: str = Field(
        default="localhost", env="STORAGE_DB_HOST", description="Database host"
    )

    database_port: int = Field(
        default=5432, env="STORAGE_DB_PORT", description="Database port"
    )

    database_name: str = Field(
        default="business_intel", env="STORAGE_DB_NAME", description="Database name"
    )

    database_user: str = Field(
        default="postgres", env="STORAGE_DB_USER", description="Database user"
    )

    database_password: Optional[str] = Field(
        default=None, env="STORAGE_DB_PASSWORD", description="Database password"
    )
    ...
    enable_caching: bool = Field(
        default=True, env="STORAGE_ENABLE_CACHING", description="Enable caching layer"
    )
    ...
    enable_alerts: bool = Field(
        default=True, env="STORAGE_ENABLE_ALERTS", description="Enable storage alerting"
    )
