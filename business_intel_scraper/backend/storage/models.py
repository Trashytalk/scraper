...
class RawDataModel(Base):
    ...
    # Relationships
    structured_mappings = relationship(
        "RawToStructuredMappingModel", back_populates="raw_data"
    )
...
class RawToStructuredMappingModel(Base):
    ...
    entity_id = Column(
        String, ForeignKey("structured_entities.entity_id"), nullable=False, index=True
    )
    ...
    __table_args__ = (
        ...
        UniqueConstraint(
            "raw_id", "entity_id", "extractor_name", name="uq_raw_entity_extractor"
        ),
    )
...
class DataQualityMetricsModel(Base):
    ...
    )  # entity, domain, job, global
    scope_id = Column(
        String, index=True
    )  # ID of the scope (entity_id, domain, job_id, etc.)
...
class DataLineageModel(Base):
    ...
    transformation_type = Column(
        String, nullable=False, index=True
    )  # extraction, enrichment, aggregation
    ...
    __table_args__ = (
        ...
        Index(
            "idx_lineage_transformation", "transformation_type", "transformation_name"
        ),
        Index("idx_lineage_path", "path_length", "dependency_count"),
        Index(
            "idx_lineage_quality", "transformation_confidence", "data_quality_impact"
        ),
    )
