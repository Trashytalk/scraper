...
class CrawlOrchestrator:
    ...
    async def run_intelligence_gathering(
        self, operation_name: str = "business_intel"
    ) -> Dict:
        ...
        try:
            async for page in self.crawl_manager.start_discovery_operation(
                operation_name
            ):
                ...
                if len(discovered_pages) % 100 == 0:
                    ...
                    logger.info(
                        f"Discovery progress: {len(discovered_pages)} pages - {metrics}"
                    )

                # Safety limit to prevent runaway crawling
                if len(discovered_pages) >= 10000:
                    logger.warning(
                        "Reached maximum pages limit (10,000), stopping operation"
                    )
                    break
        ...
        logger.info(
            f"Discovery operation completed in {operation_duration:.2f}s: {final_metrics}"
        )
        ...
        return {
            ...
            "high_value_pages": [
                p for p in discovered_pages if p.classification_score > 0.8
            ],
            "summary": self._generate_operation_summary(
                discovered_pages, final_metrics
            ),
        }

    def _generate_operation_summary(
        self, pages: List[DiscoveredPage], metrics: Dict
    ) -> Dict:
        ...
        for page in pages:
            ...
            classification_types[page.classification_type] = (
                classification_types.get(page.classification_type, 0) + 1
            )
        ...
        success_rate = len([p for p in pages if p.crawl_status == "crawled"]) / len(
            pages
        )
        ...
        return {
            ...
            "crawl_rate_pages_per_minute": round(
                metrics.get("crawl_rate_per_minute", 0), 2
            ),
            "top_domains": self._extract_top_domains(pages),
            "quality_breakdown": {
                "high_quality": len([p for p in pages if p.classification_score > 0.8]),
                "medium_quality": len(
                    [p for p in pages if 0.5 < p.classification_score <= 0.8]
                ),
    ...
    def _extract_top_domains(
        self, pages: List[DiscoveredPage], limit: int = 10
    ) -> List[Dict]:
...
class EnhancedAdaptiveLinkClassifier(AdaptiveLinkClassifier):
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        logger.info(
            "Enhanced AdaptiveLinkClassifier initialized with business patterns"
        )
    ...
    def enhanced_rule_based_classification(
        ...
        high_value_matches = sum(
            1
            for pattern in self.business_patterns["high_value_patterns"]
        ...
        financial_matches = sum(
            1
            for pattern in self.business_patterns["financial_patterns"]
            if pattern.search(all_text)
        )
        ...
        medium_value_matches = sum(
            1
            for pattern in self.business_patterns["medium_value_patterns"]
            if pattern.search(all_text)
        )
        ...
        industry_matches = sum(
            1
            for pattern in self.business_patterns["industry_patterns"]
            if pattern.search(all_text)
        )
        ...
        navigation_matches = sum(
            1
            for pattern in self.business_patterns["navigation_patterns"]
        ...
        exclude_matches = sum(
            1
            for pattern in self.business_patterns["exclude_patterns"]
            if pattern.search(all_text)
        )
        ...
        context_score = sum(
            0.05 for keyword in business_keywords if keyword in context.lower()
        )
        ...
        url_score = sum(
            0.1
            for indicator in business_url_indicators
    ...
    def classify_link(
        ...
        base_result.priority = self._assign_priority(
            base_result.category, base_result.confidence
        )
    ...
    def get_pattern_stats(self) -> Dict:
        ...
        return {
            ...
            "total_patterns": sum(
                len(patterns) for patterns in self.business_patterns.values()
            ),
            "pattern_breakdown": {
