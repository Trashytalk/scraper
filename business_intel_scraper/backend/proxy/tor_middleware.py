...
from business_intel_scraper.backend.proxy.tor_provider import TORProxyProvider, proxy_manager
...
class TORProxyMiddleware:
    ...
    def __init__(self, settings):
        ...
        self.new_identity_on_failure = settings.getbool('TOR_NEW_IDENTITY_ON_FAILURE', True)
        self.circuit_change_interval = settings.getint('TOR_CIRCUIT_CHANGE_INTERVAL', 10)
    ...
    def process_request(self, request: Request, spider) -> Optional[Request]:
        ...
        if proxies:
            ...
            if (self.circuit_change_interval > 0 and 
                self.request_count % self.circuit_change_interval == 0):
    ...
    def process_exception(self, request: Request, exception: Exception, spider) -> Optional[Request]:
        ...
        if isinstance(exception, (ConnectionRefusedError, DNSLookupError, TimeoutError)):
            if self.new_identity_on_failure:
                success = self._handle_connection_failure(spider)
                if success:
                    # Retry request with new identity
                    retry_request = request.copy()
                    retry_request.meta['tor_retry_count'] = request.meta.get('tor_retry_count', 0) + 1
                    
                    # Limit retry attempts
                    if retry_request.meta['tor_retry_count'] <= 3:
                        spider.logger.info(f"Retrying TOR request with new identity: {request.url}")
                        if self.stats:
                            self.stats.inc_value('tor_middleware/retries')
                        return retry_request
                    else:
                        spider.logger.error(f"TOR request retry limit exceeded: {request.url}")
                        if self.stats:
