...
def test_backend_data():
    ...
    jobs_response = requests.get(
        "http://localhost:8000/api/jobs", headers={"Authorization": f"Bearer {token}"}
    )
    ...
    if results_response.status_code == 200:
        ...
        print(
            f"   - Content-Type: {results_response.headers.get('content-type', 'unknown')}"
        )
                    ...
                    if key in results_data:
                        ...
                        if isinstance(nested_data, list):
                            ...
                            if nested_data and isinstance(nested_data[0], dict):
                                print(
                                    f"   - Sample keys in {key}: {list(nested_data[0].keys())}"
                                )
            else:
