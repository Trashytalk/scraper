...
def test_fixed_crawling():
    ...
    create_response = requests.post(f"{BASE_URL}/api/jobs", 
                                   json=job_data, 
                                   headers=headers)
    ...
    start_response = requests.post(f"{BASE_URL}/api/jobs/{job_id}/start", 
                                  headers=headers)
    ...
    for i in range(30):  # Wait up to 30 seconds
        ...
        if status_response.status_code == 200:
            ...
            if status == "completed":
                ...
                # Get results
                print("\n📊 Fetching results...")
                results_response = requests.get(f"{BASE_URL}/api/jobs/{job_id}/results", 
                                              headers=headers)
