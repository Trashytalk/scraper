...
async def test_database_models():
    ...
    async with session_maker() as session:
        ...
        result = await session.execute(
            sa.select(Entity)
            .where(Entity.properties.op('->>')('industry') == 'Technology')
        )
...
async def test_database_performance():
    ...
    async with session_maker() as session:
        ...
        print(f"✅ Top organizations query: {len(top_orgs)} results in {query_time:.3f}s")
        ...
        for stat in stats:
            print(f"   - {stat.entity_type}: {stat.count} entities, avg confidence: {stat.avg_confidence:.3f}")
...
async def test_real_world_scenario():
    ...
    async with session_maker() as session:
        ...
        print(f"🌐 Network Analysis (Key People):")
        ...
        for event in funding_events:
            ...
            print(f"   • {event.entity.label}: ${amount:,} ({event.properties.get('round', 'N/A')})")
