...
def test_basic_imports():
    ...
    try:
        from business_intel_scraper.backend.crawling import (
...
def test_seed_sources():
    ...
    for source_name, source in seed_sources.items():
        print(
            f"  📂 {source.name}: {len(source.urls)} URLs (priority: {source.priority})"
        )
...
def test_domain_rules():
    ...
    test_urls = [
        ...
        (
            "https://bloomberg.com/company/profile",
            True,
            "Bloomberg company - should be allowed",
        ),
        ("https://example.com/login", False, "Login page - should be excluded"),
        (
            "https://manta.com/business/directory",
            True,
            "Business directory - should be allowed",
        ),
...
def test_enhanced_link_classifier():
    ...
    for i, link in enumerate(test_links, 1):
        classified = classifier.classify_link(
            url=link["url"], anchor_text=link["anchor_text"], context=link["context"]
        )
    ...
    print(
        f"✅ Classification accuracy: {accuracy:.1%} ({correct_classifications}/{len(test_links)})"
    )
...
async def test_integration():
    ...
    manager = AdvancedCrawlManager(
        db_url=None, max_concurrent=2, max_depth=1  # Limit depth for testing
    )
...
async def main():
    ...
    print(f"\n{'='*60}")
    print("ADVANCED CRAWLING TEST SUMMARY")
    print("=" * 60)
