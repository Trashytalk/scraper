...
from business_intel_scraper.backend.db.centralized_data import (
    Base, DataConstants, CentralizedDataRecord, DataAnalytics, 
    SystemMetrics, AlertRecord, DataDeduplication, DataRepository,
    create_tables, get_model_summary
)
...
class TestDataAnalytics:
    ...
    def test_computed_properties(self, test_session):
        ...
        limited_analytics = DataAnalytics(
            news_records=100,
            ecommerce_records=50
        )
        assert limited_analytics.content_diversity_score == (2/6) * 100  # Only 2 types
...
class TestDataRepository:
    ...
    def test_get_high_quality_records(self, test_repository, sample_data_record):
        ...
        for i in range(5):
            ...
            data['extracted_text'] = "Excellent content " * (10 + i * 5)  # Varying quality
    ...
    def test_alert_operations(self, test_repository):
        ...
        # Resolve alert
        resolved = test_repository.resolve_alert(alert.id, "test_user", "Test resolution")
...
class TestPerformance:
    ...
    def test_query_performance(self, test_session):
        ...
        domain_results = test_session.query(CentralizedDataRecord).filter(
            CentralizedDataRecord.source_domain == "domain1.com"
        ).all()
        
        # Query by data type (indexed)
        type_results = test_session.query(CentralizedDataRecord).filter(
            CentralizedDataRecord.data_type == "news"
        ).all()
        
        # Query by quality score (indexed)
        quality_results = test_session.query(CentralizedDataRecord).filter(
            CentralizedDataRecord.data_quality_score >= 80.0
        ).all()
...
class TestDatabaseConstraints:
    ...
    def test_foreign_key_constraints(self, test_session):
        ...
        original_record = CentralizedDataRecord(source_url="https://example.com/original")
        test_session.add(original_record)
        test_session.commit()
        ...
        invalid_dedup = DataDeduplication(
            ...
            canonical_record_id=999999  # Non-existent ID
        )
...
class TestUtilityFunctions:
    ...
    def test_create_tables_function(self):
        ...
        with test_engine.connect() as conn:
            ...
            expected_tables = [
                'centralized_data', 'data_analytics', 'system_metrics',
                'alert_records', 'data_deduplication'
            ]
    ...
    def test_model_summary_function(self):
        ...
        expected_models = [
            'CentralizedDataRecord', 'DataAnalytics', 'SystemMetrics',
            'AlertRecord', 'DataDeduplication'
        ]
