...
from business_intel_scraper.backend.db.centralized_data import (
    Base, CentralizedDataRecord, DataAnalytics, SystemMetrics, 
    AlertRecord, DataRepository
)
...
class PerformanceMetrics:
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'operations_per_second': len(self.execution_times) / sum(self.execution_times) if sum(self.execution_times) > 0 else 0
        }
...
class TestQueryPerformance:
    ...
    def create_test_data(self, session, record_count=500):
        ...
        for i in range(record_count):
            record = CentralizedDataRecord(
                ...
                content_category=["technology", "business", "sports", "entertainment"][i % 4],
    ...
    def test_indexed_query_performance(self, performance_session):
        ...
        query_tests = [
            ...
            lambda: performance_session.query(CentralizedDataRecord).filter(
            ...
            lambda: performance_session.query(CentralizedDataRecord).filter(
                CentralizedDataRecord.scraped_at >= datetime.utcnow() - timedelta(days=1)
            ).all()
        ]
        ...
        for i, query_func in enumerate(query_tests):
            ...
            print(f"Query {i+1}: {len(results)} results in {execution_time:.3f} seconds")
