    ...
    return {
        ...
        "sensitive_data": {
            ...
            "encryption_key": base64.b64encode(
                b"32-byte-encryption-key-for-test"
            ).decode(),
            "personal_info": {
                "ssn": "123-45-6789",
...
class TestAuthenticationSecurity:
    ...
    def test_brute_force_protection(self, security_middleware):
        ...
        assert final_result.get("locked", False) or final_result.get(
            "rate_limited", False
        )
...
class TestAuthorizationSecurity:
    ...
    def test_role_based_access_control(self, security_middleware, security_test_data):
        ...
        assert not security_middleware.has_permission(
            user_data["role"], "admin:delete_all"
        )
        assert not security_middleware.has_permission(
            user_data["role"], "admin:system_config"
        )

        # Regular user should access user functions
        assert security_middleware.has_permission(
            user_data["role"], "user:read_own_data"
        )
        assert security_middleware.has_permission(
            user_data["role"], "user:update_profile"
        )

        # Admin should access all functions
        assert security_middleware.has_permission(
            admin_data["role"], "admin:delete_all"
        )
        assert security_middleware.has_permission(
            admin_data["role"], "user:read_own_data"
        )

    def test_resource_ownership_validation(
        self, security_middleware, security_repository
    ):
        ...
        assert not security_middleware.can_access_resource(
            "user1", user2_record.id, "modify"
        )

        # Admin should access all records
        assert security_middleware.can_access_resource(
            "admin", user1_record.id, "modify"
        )
        assert security_middleware.can_access_resource(
            "admin", user2_record.id, "delete"
        )
    ...
    def test_privilege_escalation_prevention(self, security_middleware):
        ...
        admin_change = security_middleware.validate_role_change(admin_context, "user")
...
class TestInputValidationSecurity:
    ...
    def test_sql_injection_prevention(self, security_repository, malicious_payloads):
        ...
        for payload in sql_payloads:
            ...
            try:
                ...
                assert (
                    payload in malicious_record.title
    ...
    def test_command_injection_prevention(
        self, security_middleware, malicious_payloads
    ):
    ...
    def test_special_character_handling(self, security_middleware):
        ...
        for char in dangerous_chars:
            assert char not in sanitized or sanitized.count(
                char
            ) <= special_chars.count(char)
...
class TestDataProtectionSecurity:
    ...
    def test_database_encryption_at_rest(self, security_repository, security_test_data):
        ...
        stored_record = (
            security_repository.session.query(CentralizedDataRecord)
            .filter_by(id=sensitive_record.id)
            .first()
        )
...
class TestVulnerabilityAssessment:
    ...
    def test_information_disclosure_prevention(self, security_middleware):
        ...
        error_msg = security_middleware.generate_error_message(
    ...
    def test_session_fixation_prevention(self, security_middleware):
        ...
        post_login_session = security_middleware.regenerate_session(
            initial_session["id"]
        )
