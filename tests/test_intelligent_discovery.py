...
def test_basic_imports():
    ...
    try:
        from business_intel_scraper.backend.discovery.classifier import AdaptiveLinkClassifier, LinkInfo, LinkCategory
    ...
    try:
        from business_intel_scraper.backend.extraction.schema_detector import SchemaDetector, DetectedSchema
...
def test_scheduler():
    ...
    config = {'max_concurrent_crawls': 5, 'enable_ml_scoring': False}  # Disable ML for basic test
...
def test_classifier():
    ...
    test_links = [
        ...
        ("https://example.com/investor-relations", "Investor Relations", "https://example.com"),
    ...
    for link_info in classified_links:
        print(f"✅ Classified: {link_info.url} -> {link_info.category.value} (confidence: {link_info.confidence:.2f}, priority: {link_info.priority.name})")
...
def test_schema_detector():
    ...
    if schema:
        print(f"✅ Detected schema: {schema.name} (confidence: {schema.confidence:.2f})")
        print(f"   Fields detected: {len(schema.fields)}")
        for field in schema.fields:
            print(f"     - {field.name}: {field.data_type.value} (confidence: {field.confidence:.2f})")
    else:
...
def test_integration():
    ...
    from business_intel_scraper.backend.discovery.scheduler import IntelligentCrawlScheduler, CrawlRequest
