...
try:
    from gui.components.advanced_entity_graph import (
        ...
        EntityNode, EntityRelationship, GraphAnalytics, NetworkXConnector,
        GraphAnalyticsEngine, GraphQueryEngine, GraphVisualizationEngine
    )
    from gui.components.data_integration_bridge import (
        DataIntegrationBridge, IntegrationMapping
    )
...
class TestGraphAnalyticsEngine:
    ...
    async def test_centrality_calculation(self):
        ...
        relationships = [
            EntityRelationship(
                "NODE_0", "NODE_1", RelationshipType.OWNS
            ),
            EntityRelationship(
                "NODE_1", "NODE_2", RelationshipType.OWNS
            ),
            EntityRelationship(
                "NODE_2", "NODE_3", RelationshipType.OWNS
            ),
            EntityRelationship(
                "NODE_0", "NODE_4", RelationshipType.OWNS
            )
        ]
    ...
    async def test_anomaly_detection(self):
        ...
        for i in range(8):
            rel = EntityRelationship(
                "HUB_NODE", f"NORMAL_{i}", RelationshipType.OWNS
            )
            await connector.create_relationship(rel)
...
class TestAdvancedEntityGraphSystem:
    ...
    async def test_query_execution(self):
        ...
        # Add test data
        await graph_system.add_entity("QUERY_TEST", NodeType.COMPANY, {"name": "Query Corp"})
...
class TestDataIntegrationBridge:
    ...
    def test_entity_similarity_calculation(self):
        ...
        node1 = EntityNode(
            "ENTITY_1", NodeType.COMPANY,
            {"name": "Test Corporation", "industry": "Technology"}
        )
        
        node2 = EntityNode(
            "ENTITY_2", NodeType.COMPANY,
            {"name": "Test Corporation", "industry": "Technology"}
        )
