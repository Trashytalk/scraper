...
from business_intel_scraper.backend.db.centralized_data import (
    Base, DataRepository, CentralizedDataRecord, DataAnalytics,
    SystemMetrics, AlertRecord, DataDeduplication, create_tables
)
...
def pytest_collection_modifyitems(config, items):
    ...
    for item in items:
        ...
        if any(keyword in item.name.lower() for keyword in ['load', 'performance', 'stress', 'bulk']):
...
def database_test_utils(test_session):
    ...
    class DatabaseTestUtils:
        ...
        def verify_record_count(self, expected_count: int, model_class=CentralizedDataRecord):
