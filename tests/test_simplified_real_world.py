...
async def test_database_connectivity():
    ...
    try:
        ...
        print(
            f"   📊 Found {len(table_names)} existing tables: {', '.join(table_names)}"
        )
...
async def test_real_world_scenario():
    ...
    try:
        ...
        for industry, count, avg_revenue in industry_results:
            print(
                f"      {industry}: {count} companies, avg revenue ${avg_revenue/1e9:.1f}B"
            )
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for endpoint in endpoints:
                    try:
                        resp = requests.get(
                            f"http://localhost:8000{endpoint}", timeout=2
                        )
