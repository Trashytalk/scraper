...
from business_intel_scraper.backend.discovery.scheduler import IntelligentCrawlScheduler, CrawlRequest, CrawlPriority
from business_intel_scraper.backend.discovery.classifier import AdaptiveLinkClassifier, LinkCategory
...
class IntelligentDiscoveryDemo:
    ...
    def __init__(self):
        ...
        self.graph_analyzer = CrawlGraphAnalyzer({
            'enable_networkx': False,  # Fallback mode
            'max_depth': 5
        })
    ...
    def demo_intelligent_scheduling(self):
        ...
        demo_requests = [
            ...
            ("https://example-corp.com/investor-relations", "financial_spider", CrawlPriority.HIGH, 1),
            ("https://example-corp.com/contact", "contact_spider", CrawlPriority.NORMAL, 1),
            ("https://example-corp.com/privacy", "legal_spider", CrawlPriority.LOW, 2),
            ("https://example-corp.com/careers", "general_spider", CrawlPriority.BACKGROUND, 2),
        ]
        ...
        for url, spider, priority, depth in demo_requests:
            ...
            print(f"  {'✅' if success else '❌'} {priority.name:10} | {spider:15} | {url}")
        ...
        print(f"  • Success Rate: {list(stats['top_domains'].values())[0]['success_rate']:.1%}")
        print()
    ...
    def demo_adaptive_link_classification(self):
        ...
        discovered_links = [
            ("https://acme-corp.com/about-us", "About ACME Corporation", "https://acme-corp.com"),
            ("https://acme-corp.com/leadership", "Executive Leadership Team", "https://acme-corp.com"),
            ("https://acme-corp.com/investor-relations", "Investor Relations", "https://acme-corp.com"),
            ("https://acme-corp.com/quarterly-earnings", "Q3 2024 Earnings Report", "https://acme-corp.com"),
            ("https://acme-corp.com/press-releases", "Press Releases", "https://acme-corp.com"),
            ("https://acme-corp.com/contact-us", "Contact Information", "https://acme-corp.com"),
            ("https://acme-corp.com/products", "Our Products & Services", "https://acme-corp.com"),
            ("https://acme-corp.com/partnerships", "Strategic Partnerships", "https://acme-corp.com"),
            ("https://acme-corp.com/legal/privacy", "Privacy Policy", "https://acme-corp.com"),
            ("https://twitter.com/acme_corp", "@acme_corp on Twitter", "https://acme-corp.com"),
            ("mailto:info@acme-corp.com", "info@acme-corp.com", "https://acme-corp.com"),
            ("https://acme-corp.com/random-page", "Random Page", "https://acme-corp.com"),
        ]
        ...
        for category, links in sorted(by_category.items()):
            ...
            for link in links:
                priority_icon = {"CRITICAL": "🔴", "HIGH": "🟡", "NORMAL": "🟢", "LOW": "🔵", "BACKGROUND": "⚪"}
    ...
    def demo_schema_detection(self):
        ...
        for page_type, html_content in test_pages.items():
            ...
            schema = self.schema_detector.detect_schema(html_content, f"https://acme-corp.com/{page_type.lower().replace(' ', '-')}")
            
            if schema:
                detected_schemas.append(schema)
                print(f"  ✅ Schema: {schema.name} (confidence: {schema.confidence:.2f})")
                print(f"  📋 Fields detected: {len(schema.fields)}")
                
                for field in schema.fields[:3]:  # Show first 3 fields
                    print(f"     • {field.name}: {field.data_type.value} ({field.confidence:.2f})")
        ...
        print(f"📊 Schema Detection Stats:")
    ...
    def demo_template_extraction(self):
        ...
        for template_id in templates_to_test:
            ...
            if result and result["data"]:
                print(f"  ✅ Extraction successful (confidence: {result['confidence']:.2f})")
    ...
    def demo_graph_analysis(self):
        ...
        crawl_data = [
            ("https://company.com/", ["https://company.com/about", "https://company.com/contact", "https://company.com/investors"], 0.9, 1.2),
            ("https://company.com/about", ["https://company.com/leadership", "https://company.com/history"], 0.8, 1.8),
            ("https://company.com/contact", ["mailto:info@company.com"], 0.7, 1.1),
            ("https://company.com/investors", ["https://company.com/earnings", "https://company.com/sec-filings"], 0.9, 2.1),
            ("https://company.com/leadership", ["https://company.com/ceo-bio", "https://company.com/board"], 0.8, 1.5),
        ...
        for url, links, data_value, response_time in crawl_data:
            success = len(links) > 0 or "earnings" in url  # Assume success if has links or is earnings
            ...
            print(f"  📄 {url}")
            print(f"     Links: {len(links)} | Quality: {data_value:.1f} | Time: {response_time:.1f}s")
    ...
    async def demo_adaptive_scraper_integration(self):
        ...
        for result in simulated_results:
            ...
            print(f"     Schema: {result['schema']} (confidence: {result['confidence']:.2f})")
            print(f"     Data: {len(result['data'])} fields | Links: {result['links_found']}")
