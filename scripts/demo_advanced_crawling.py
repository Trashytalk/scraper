...
class MockDiscoveredPage:
    ...
    def __init__(self, url, parent_url=None, anchor_text=None, depth=0, 
                 classification_score=0.5, classification_type="unknown", 
                 source_type="unknown", metadata=None):
        self.url = url
...
class AdvancedCrawlingDemo:
    ...
    def _compile_business_patterns(self) -> Dict:
        ...
        return {
            'high_value': [
                r'company[-_]profile', r'business[-_]profile', r'executive[-_]team',
                r'financial[-_]statements', r'annual[-_]report', r'investor[-_]relations'
            ],
            'medium_value': [
                r'contact[-_]us', r'company[-_]info', r'business[-_]directory',
                r'leadership', r'press[-_]release'
            ],
            'navigation': [
                r'next[-_]page', r'more[-_]results', r'view[-_]all', r'page[-_]\d+'
            ],
            'exclude': [
                r'login', r'signup', r'privacy[-_]policy', r'terms', r'careers'
            ]
        }
    ...
    def demo_domain_filtering(self):
        ...
        test_urls = [
            ...
            ("https://facebook.com/company/apple", False, "Blocked social media domain"),
            ("https://bloomberg.com/company/profile/apple", True, "Financial site company profile"),
            ("https://example.com/login", False, "Excluded login page"),
            ("https://manta.com/business/directory/apple-corp", True, "Business directory listing"),
            ("https://youtube.com/watch?v=abc123", False, "Blocked video platform"),
            ("https://crunchbase.com/organization/apple", True, "Business intelligence platform")
        ]
        ...
        print(f"✅ URL Filtering Accuracy: {passed}/{len(test_urls)} ({passed/len(test_urls)*100:.1f}%)")
        print()
    ...
    def demo_content_deduplication(self):
        ...
        content_samples = [
            ("Page 1", "<html><body><h1>ACME Corp</h1><p>Technology leader</p></body></html>"),
            ("Page 2", "<html><body><h1>ACME Corp</h1><p>Technology leader</p></body></html>"),
            ("Page 3", "<html>  <body>  <h1>ACME Corp</h1>  <p>Technology leader</p>  </body>  </html>"),
            ("Page 4", "<html><body><h1>XYZ Inc</h1><p>Different company</p></body></html>"),
            ("Page 5", "<html><body><h1>ACME Corp</h1><p>Leading technology company</p></body></html>")
        ]
    ...
    def demo_crawl_scheduling(self):
        ...
        sorted_pages = sorted(
            discovered_pages,
            key=lambda p: (-p.classification_score, p.depth)
        )
        ...
        for i, page in enumerate(sorted_pages, 1):
            ...
            print(f"  {i}. {priority} | Depth {page.depth} | Score {page.classification_score:.2f}")
            print(f"     {page.url}")
    ...
    def demo_metadata_extraction(self):
        ...
        for page in sample_pages:
            print(f"📄 {page['url']}")
            print(f"   Classification: {page['classification_type']} (score: {page['classification_score']:.2f})")
            print(f"   Source: {page['source_type']} | Depth: {page['depth']}")
            print(f"   Response: {page['metadata']['status_code']} | Time: {page['metadata']['response_time']}s")
    ...
    def demo_crawl_metrics(self):
        ...
        print(f"   ⏱️  Runtime: {mock_metrics['runtime_seconds']//60}m {mock_metrics['runtime_seconds']%60}s")
        print(f"   📄 Pages Crawled: {mock_metrics['pages_crawled']:,}")
        print(f"   🔗 Links Discovered: {mock_metrics['links_discovered']:,}")
        print(f"   ⚡ Crawl Rate: {mock_metrics['crawl_rate_per_minute']:.1f} pages/min")
        print()
        
        print("🎯 Quality Distribution:")
        total_quality = mock_metrics['high_value_pages'] + mock_metrics['medium_value_pages'] + mock_metrics['low_value_pages']
        print(f"   🔴 High Value: {mock_metrics['high_value_pages']} ({mock_metrics['high_value_pages']/total_quality*100:.1f}%)")
        print(f"   🟡 Medium Value: {mock_metrics['medium_value_pages']} ({mock_metrics['medium_value_pages']/total_quality*100:.1f}%)")
        print(f"   🔵 Low Value: {mock_metrics['low_value_pages']} ({mock_metrics['low_value_pages']/total_quality*100:.1f}%)")
        print()
        
        print("🌐 Top Domains by Page Count:")
        for domain in mock_metrics['top_domains']:
            print(f"   {domain['domain']}: {domain['page_count']} pages (avg score: {domain['avg_score']:.2f})")
        print()
        ...
        error_rate = sum(mock_metrics['errors'].values()) / mock_metrics['pages_crawled'] * 100
        print(f"⚠️  Error Rate: {error_rate:.1f}% ({sum(mock_metrics['errors'].values())} errors)")
        print()
    ...
    def classify_link(self, url: str, anchor_text: str, context: str) -> Tuple[float, str]:
        ...
        high_value_matches = sum(1 for pattern in self.business_patterns['high_value'] 
        ...
        medium_value_matches = sum(1 for pattern in self.business_patterns['medium_value']
                                 if re.search(pattern, all_text))
        ...
        nav_matches = sum(1 for pattern in self.business_patterns['navigation']
        ...
        exclude_matches = sum(1 for pattern in self.business_patterns['exclude']
                            if re.search(pattern, all_text))
    ...
    def calculate_content_hash(self, content: str) -> str:
        ...
        return hashlib.md5(normalized.encode()).hexdigest()[:12]  # Shortened for display
