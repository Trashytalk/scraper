...
class TestValidator:
    ...
    def test_api_endpoints(self):
        ...
        try:
            response = requests.get(
                f"{self.base_url}/api/jobs", headers=headers, timeout=10
            )
            self.log_test(
                "Jobs API",
                response.status_code == 200,
        ...
        try:
            response = requests.get(
                f"{self.base_url}/api/analytics/dashboard", headers=headers, timeout=10
            )
            self.log_test(
                "Analytics API",
                response.status_code == 200,
        ...
        try:
            response = requests.get(
                f"{self.base_url}/api/performance/summary", headers=headers, timeout=10
            )
            self.log_test(
                "Performance API",
                response.status_code == 200,
    ...
    def test_job_creation(self):
        ...
        try:
            response = requests.post(
                f"{self.base_url}/api/jobs", json=job_data, headers=headers, timeout=10
            )
            self.log_test(
                "Job Creation",
                response.status_code == 200,
    ...
    def test_database_connectivity(self):
        ...
        try:
            ...
            if not os.path.exists(db_path):
                self.log_test(
                    "Database File", False, f"Database file not found: {db_path}"
                )
                return
            ...
            self.log_test(
                "Database Connectivity", True, f"Found {cache_count} cached entries"
            )

            # Check for recent entries
            cursor.execute(
                "SELECT COUNT(*) FROM crawl_cache WHERE crawled_at > datetime('now', '-1 hour')"
            )
    ...
    def test_enhanced_crawling_features(self):
        ...
        try:
            ...
            if success and (
                "success" in result.stdout.lower() or "✅" in result.stdout
            ):
                self.log_test(
                    "Enhanced Crawling Test", True, "Enhanced crawling features working"
                )
            else:
                self.log_test(
                    "Enhanced Crawling Test", False, f"Return code: {result.returncode}"
                )
                if result.stdout:
    ...
    def test_data_centralization(self):
        ...
        try:
            ...
            if success and "successful" in result.stdout.lower():
                ...
                self.log_test(
                    "Data Centralization", False, f"Return code: {result.returncode}"
                )
