    ...
    async def dispatch(self, request: Request, call_next):
        ...
        for key, value in request.query_params.items():
            if self._contains_sql_injection(value) or self._contains_xss(value):
                logger.warning(f"Malicious query parameter detected from {request.client.host}: {key}={value}")
...
class RequestLoggingMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if response.status_code >= 400:
            logger.warning(f"Error response {response.status_code} for {request.method} {request.url.path} from {client_ip}")
            ...
            if isinstance(selectors, dict):
                ...
                for key, selector in selectors.items():
                    if isinstance(selector, str) and len(selector) <= 200:
                        ...
                        if re.match(r'^[a-zA-Z0-9\s\.\#\[\]\:\-\,\>\+\~\*\(\)=\"\']+$', selector):
                            validated_selectors[sanitize_string(key, 50)] = sanitize_string(selector, 200)
