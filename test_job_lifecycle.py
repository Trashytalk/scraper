...
def test_job_lifecycle():
    ...
    job_data = {
        ...
        "config": {
            "timeout": 30,
            "user_agent": "Business Intelligence Scraper Test"
        }
    }
    ...
    try:
        response = requests.get(f"http://localhost:8000/api/jobs/{job_id}", headers=headers)
    ...
    try:
        response = requests.post(f"http://localhost:8000/api/jobs/{job_id}/start", headers=headers)
    ...
    try:
        response = requests.get(f"http://localhost:8000/api/jobs/{job_id}/results", headers=headers)
        ...
        if response.status_code == 200:
            ...
            if isinstance(results, dict):
                if 'results' in results:
                    ...
                    print(f"   Results count: {len(results_data) if isinstance(results_data, list) else 'Not a list'}")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for job in jobs[-5:]:  # Show last 5 jobs
                ...
                print(f"   Job {job_id}: {name[:30]}... | Status: {status} | Results: {results_count}")
        else:
            ...
            for job in jobs[-3:]:  # Check last 3 jobs
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        if isinstance(data, dict) and 'results' in data:
                            ...
                            print(f"   Results format: {type(data)} - {str(data)[:100]}...")
                    else:
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        print(f"   Details keys: {list(details.keys()) if isinstance(details, dict) else 'Not dict'}")
                    else:
                        print(f"   Error: {response.text[:100]}...")
                except Exception as e:
                    print(f"   Exception: {e}")
