...
class GraphSystemInstaller:
        ...
        try:
            if platform == 'linux':
                ...
                if self._command_exists('apt-get'):
                    ...
                    for dep in dependencies:
                        subprocess.run(['sudo', 'yum', 'install', '-y', dep], check=True)
    ...
    def setup_neo4j_database(self, install_docker: bool = True) -> bool:
        ...
        try:
            ...
            logger.warning("Docker not found. Please install Docker manually for Neo4j support.")
            return False
...
def main():
    ...
    parser.add_argument("--neo4j", action="store_true",
                       help="Setup Neo4j database using Docker")
